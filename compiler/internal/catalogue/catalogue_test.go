...
func TestCompleteInventoryAndMirrors(t *testing.T) {
	c := Builtin()
	inv := c.Inventory()
	if len(inv.Packages) != 37 || len(inv.Types) != 102 || len(inv.Errors) != 109 || len(inv.Operations) != 285 || len(inv.NativeDeclarations) != 10 {
	...
	for _, p := range strings.Fields("action ai asset browser bytes checks cli clock codec collections crypto env files html htmx http io json llm log number option path process random s3 sql stream text time url ws") {
