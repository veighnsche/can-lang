...
`)
	...
	lock, err := json.Marshal(map[string]any{"dependencies": map[string]any{"vendor": map[string]any{"path": "vendor", "manifest_sha256": project.Digest(vendorManifest), "source_sha256": digest, "error_registry": registry}}})
	if err != nil {
		t.Fatal(err)
	}
