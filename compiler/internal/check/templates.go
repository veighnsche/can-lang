...
func (c *programChecker) checkTemplate(program *Program, file *resolve.File, d *syntax.FixtureDecl, callables map[string]CallableDeclaration) error {
	...
	context := CompletionContext{Identity: region.ID, Kind: ir.FunctionRegion, Package: file.Package.ID, File: file.Source.Syntax.Source, Scope: scope, Result: target.Type.Result(), Registry: program.Registry, Expressions: c.expressions(file, scope), Callables: callables, Variadic: c.variadic, Raw: c.rawScope(file), Warn: c.warn}
