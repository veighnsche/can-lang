...
func (e *RegionEmitter) coordination(node *ir.Coordination) (LoweredExpression, error) {
	...
	fmt.Fprintf(&out, "const %s = await $canCoordinateSettle(%s,%s);\n", selection, quote(node.Mode), participants)
