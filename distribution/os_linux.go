...
func hostOSVersion() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("verify bundle: cannot read OS release: %w", err)
	}
	...
	if fields["ID"] != "debian" {
		return "", fmt.Errorf("verify bundle: requires Debian, found %q", fields["ID"])
	}
	if fields["VERSION_ID"] == "" {
		return "", fmt.Errorf("verify bundle: Debian VERSION_ID missing")
	}
