...
Checker regressions exercise inferred and explicit fixed transitions with and without prior cache population. Staged integration executes the independent review reproducers through a freshly built offline distribution. The genuinely growing recursion test now handles the nested array result before continuing, so its body is otherwise type-correct and its failure specifically exercises the growth guard.
