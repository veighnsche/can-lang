"""LF01 bounded G1-G4 control rerun. Scratch copy of implementation-gaps/run-probes.py.

Reads evidence projects, writes results ONLY to the scratch out path.
Runs G1/G2 (passing, failing-assertion), G3 (pending-assertion + cpu negative
control), G4 (diagnostic-semantic/resolve/parse + passing LSP control).
G5 resource projects are intentionally NOT rerun (preserved regression constraint).
"""
import json, os, pathlib, signal, subprocess, time, sys, shutil
HERE = pathlib.Path('/Users/vince/Projects/can-lang/docs/syntax-taste/evidence/2026-09-22/implementation-gaps')
...
def run(label, args, timeout=15, input_bytes=None):
    ...
    p = subprocess.Popen([str(launcher), *args], stdin=subprocess.PIPE,
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE, start_new_session=True)
    killed = False
    try:
        out_b, err = p.communicate(input_bytes, timeout=timeout)
    except subprocess.TimeoutExpired:
        killed = True
        os.killpg(p.pid, signal.SIGKILL)
        out_b, err = p.communicate(timeout=3)
    row = {'label': label, 'argv': args, 'external_limit_seconds': timeout,
           'externally_terminated': killed, 'exit_code': p.returncode,
           'elapsed_seconds': round(time.monotonic() - start, 3),
           'stdout': out_b.decode(errors='replace'), 'stderr': err.decode(errors='replace')}
    results.append(row)
...
for name in ['passing', 'failing-assertion', 'pending-assertion', 'cpu-assertion',
             'diagnostic-semantic', 'diagnostic-resolve', 'diagnostic-parse']:
    row = run(name + ' build', ['build', str(work / name)])
    row['current_after_build'] = current(name)
for name in ['passing', 'failing-assertion', 'pending-assertion', 'cpu-assertion']:
    before = current(name)
    row = run(name + ' assert', ['assert', str(work / name)],
              timeout=4 if name in ['pending-assertion', 'cpu-assertion'] else 15)
    row['current_before_assert'] = before
    row['current_after_assert'] = current(name)
def frame(obj):
