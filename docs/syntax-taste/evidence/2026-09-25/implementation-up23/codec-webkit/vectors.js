...
var nativeErrorPrototypes = new Set([
  Error.prototype,
  TypeError.prototype,
...
// ../../../runtime/data.ts
var nominal = Symbol("can.nominal");
var nominalRecords = new WeakMap;
var opaqueChildren = new WeakMap;
function record(identity, fields) {
...
function dataArray(value) {
  ...
  if (dataKeys(value).length !== length + 1)
    throw new TypeError("expected dense data array");
  const out = [];
  for (let index = 0;index < length; index++)
    out.push(dataProperty(value, String(index)));
  return out;
}
...
var catalogueTypeShapes = freeze([
  {
    name: "choice_option",
    identity: "can.prelude@1::choice_option",
    kind: "record",
...
var faults = new WeakMap;

// ../../../runtime/failure.ts
var standard = new WeakMap;
var nextOccurrence = 1n;
function allocateOccurrenceID() {
...
function freezeOrigin(origin) {
  return Object.freeze({
    ...
    invocation: Object.freeze([...origin.invocation])
  });
}
...
function createStandard(kind, message, cause, origin) {
  const occurrence = Object.freeze(Object.create(null));
  standard.set(occurrence, Object.freeze({
    occurrenceID: allocateOccurrenceID(),
    kind,
    message,
    cause,
    origin: freezeOrigin(origin)
  }));
  return occurrence;
}
function resourceStateFailure(cause, origin) {
  return createStandard("resource_state", "resource_state: invalid resource use", cause, origin);
}
var assertionFailureClasses = Object.freeze([
  "missing fixture",
  "argument mismatch",
  "ambiguous fixture",
  "malformed fixture",
...
var completions = new WeakSet;
var elements = new WeakSet;

// ../../../runtime/bytes.ts
var storage = new WeakMap;
function ownBytes(fresh) {
...
function copyBytes(value, origin) {
  if (!isBytes(value))
...
// ../../../runtime/codec/budget.ts
var standaloneBytes = 8388608;
var maxDepth = 64;
var maxNodes = 1e6;
...
class Budget {
  ...
  remaining;
  nodes = 0;
  constructor(bytes = standaloneBytes) {
    this.bytes = bytes;
    if (!Number.isSafeInteger(bytes) || bytes < 1 || bytes > 67108864)
      throw new TypeError("invalid private codec budget");
    this.remaining = bytes;
  }
  charge(size, path) {
    if (!Number.isSafeInteger(size) || size < 0)
      throw new TypeError("invalid codec charge");
  ...
  visit(depth, path) {
    if (depth > maxDepth)
      reject(path, "depth_limit");
    if (++this.nodes > maxNodes)
      reject(path, "node_limit");
  }
}
...
// ../../../runtime/codec/numbers.ts
function compareExponent(source, bound) {
...
function decodeInteger(token, budget, path) {
  if (token.length > budget.bytes)
    reject(path, "byte_limit");
  const parts = /^(-?)(\d+)(?:\.(\d+))?(?:[eE]([+-]?\d+))?$/.exec(token);
  if (!parts)
    throw new TypeError("missing native numeric token evidence");
  ...
  let trailing = 0;
  while (coefficient[coefficient.length - 1 - trailing] === "0")
    trailing++;
  const exponent = parts[4] ?? "0";
  if (compareExponent(exponent, fraction.length - trailing) < 0)
    reject(path, "integer_token");
...
function encodeInteger(value, budget, path) {
  const sign = value < 0n ? 1 : 0;
  const digits = budget.remaining - sign;
  if (digits < 1)
    reject(path, "byte_limit");
  if (!(budget.remaining >= 20 && value > -10000000000000000000n && value < 10000000000000000000n)) {
...
function scanJSON(text) {
  ...
  function valuePath() {
    ...
    if (!parent)
      return "";
    if (parent.kind === "[")
      return childPath(parent.path, parent.index++);
...
`;
  for (let i = 0;i < text.length; ) {
    ...
    if (ch === "{" || ch === "[") {
      ...
      stack.push(ch === "{" ? { kind: ch, path, index: 0, keys: new Set } : { kind: ch, path, index: 0 });
      i++;
      continue;
    }
    if (ch === "}" || ch === "]") {
    ...
    valuePath();
    budget.visit(stack.length, "");
    while (i < text.length && !whitespace(text[i]) && !'{}[],:"'.includes(text[i]))
      i++;
  }
  return duplicate;
}

// ../../../runtime/codec/document.ts
var origin2 = Object.freeze({
  source: "can:codec",
  start: 0,
  end: 0,
  invocation: Object.freeze([])
});
function decodeText(input, bytes = standaloneBytes) {
  if (byteLength(input) > BigInt(bytes))
    reject("", "byte_limit");
  try {
    return new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(copyBytes(input, origin2));
  } catch (cause) {
...
function parseJSONText(text) {
  ...
  let parsed;
  try {
    parsed = JSON.parse(text, function(key, value, context) {
  ...
  if (duplicate !== undefined)
    reject(duplicate, "duplicate_member");
  return { parsed, rootHolder, tokens };
}
function parseDocument(input, bytes = standaloneBytes) {
  return parseJSONText(decodeText(input, bytes));
}

// ../../../runtime/codec/project.ts
var encoder2 = new TextEncoder;
function graph(schema) {
  const nodes = new Map(schema.nodes.map((node) => [node.identity, node]));
  if (nodes.size !== schema.nodes.length)
    throw new TypeError("duplicate codec schema node");
  return (id) => {
    const node = nodes.get(id);
    if (!node)
      throw new TypeError("missing codec schema node");
    return node;
  };
}
function scalar(text, path) {
  if (new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(encoder2.encode(text)) !== text)
    reject(path, "unicode_scalar");
}
function checkedData(read, path) {
  try {
    return read();
  } catch (cause) {
    if (cause instanceof TypeError)
...
function required(value, key, path) {
  if (!Object.hasOwn(value, key))
    reject(childPath(path, key), "missing_member");
  return checkedData(() => dataProperty(value, key), childPath(path, key));
}
function extras(value, fields, path) {
  const extra = Object.keys(value).filter((key) => !fields.includes(key)).sort();
  if (extra.length)
    reject(childPath(path, extra[0]), "extra_member");
}
var exactInt = (tokens) => (value, holder, key, path, budget) => {
  if (typeof value !== "number")
    reject(path, "type");
  return decodeInteger(tokens.get(holder)?.get(key) ?? "", budget, path);
};
function projectValue(schema, parsed, holder, budget, intOf) {
  const get = graph(schema), active = new Set;
  function visit(node, value, site, key, path, depth) {
      ...
      switch (node.name) {
        ...
        case "float":
          ...
          if (!Number.isFinite(value))
            reject(path, "nonfinite");
          return value;
        case "int":
          return intOf(value, site, key, path, budget);
        default:
    ...
    try {
      ...
      const entries = fields.map((field) => [
        field.name,
        visit(get(field.type), required(data, field.name, path), data, field.name, childPath(path, field.name), depth + 1)
      ]);
...
// ../../../runtime/codec/jsonl.ts
var origin3 = Object.freeze({
  source: "can:codec",
  start: 0,
  end: 0,
  invocation: Object.freeze([])
...
var ownerContexts = new WeakMap;
// ../../../runtime/transport/stream/lifecycle.ts
var origin5 = Object.freeze({
  source: "can:stream-lifecycle",
  start: 0,
  end: 0,
  invocation: Object.freeze([])
});
var cells = new WeakMap;

// ../../../runtime/codec/json.ts
var origin6 = Object.freeze({
  source: "can:codec",
  start: 0,
  end: 0,
  invocation: Object.freeze([])
...
var nativeJSON = JSON;
function scalar2(text, path) {
  if (new TextDecoder("utf-8", { fatal: true, ignoreBOM: true }).decode(encoder3.encode(text)) !== text)
    reject(path, "unicode_scalar");
}
function checkedData2(read, path) {
  try {
    return read();
  } catch (cause) {
    if (cause instanceof TypeError)
...
function object2(value, path) {
  if (value === null || typeof value !== "object" || checkedData2(() => Array.isArray(value), path))
    reject(path, "type");
  checkedData2(() => dataKeys(value), path);
  return value;
}
function required2(value, key, path) {
  if (!Object.hasOwn(value, key))
    reject(childPath(path, key), "missing_member");
  return checkedData2(() => dataProperty(value, key), childPath(path, key));
}
function extras2(value, fields, path) {
  const extra = Object.keys(value).filter((key) => !fields.includes(key)).sort();
  if (extra.length)
    reject(childPath(path, extra[0]), "extra_member");
}
function decodeJSON(schema, input, bytes = standaloneBytes) {
...
function encodeJSON(schema, input, bytes = standaloneBytes) {
  const get = graph(schema), budget = new Budget(bytes), active = new Set;
  const integers = new Map;
  function textCost(value, path) {
    if (value.length > budget.remaining)
      reject(path, "byte_limit");
    scalar2(value, path);
    budget.charge(encoder3.encode(JSON.stringify(value)).length, path);
  }
  function keys(names, path) {
    budget.charge(2 + Math.max(0, names.length - 1) + names.length, path);
    for (const name of names)
      textCost(name, path);
  }
  function leaf(node, value, path) {
  ...
  function visit(node, value, path, depth) {
    ...
    try {
      if (node.kind === "array") {
        if (!checkedData2(() => Array.isArray(value), path))
          reject(path, "type");
        const length = checkedData2(() => dataProperty(value, "length"), path);
        budget.charge(2 + Math.max(0, length - 1), path);
        const items = checkedData2(() => dataArray(value), path);
      ...
      if (node.kind !== "record" && node.kind !== "error")
        throw new TypeError("unsupported codec node");
      if (recordIdentity(value) !== node.identity)
        reject(path, "type");
      const data = object2(value, path), fields = node.fields ?? [];
      keys(fields.map((field) => field.name), path);
      for (const field of fields)
        visit(get(field.type), required2(data, field.name, path), childPath(path, field.name), depth + 1);
      extras2(data, fields.map((field) => field.name), path);
    } finally {
      active.delete(value);
    }
  }
  visit(get(schema.root), input, "", 0);
  function view(node, value) {
    if (node.kind === "primitive") {
      if (node.name === "int")
        return nativeJSON.rawJSON(integers.get(value));
      if (node.name === "float" && Object.is(value, -0))
  ...
  const encoded = encoder3.encode(JSON.stringify(view(get(schema.root), input)));
...
// ../../../runtime/test/browser-wire-vectors.ts
var primitives = ["int", "str", "bool", "float"].map((name) => ({
  identity: name,
  kind: "primitive",
  name
}));
var schema = (root, nodes = []) => ({
  root,
  nodes: [...primitives, ...nodes]
});
var origin7 = { source: "test:browser-wire", start: 0, end: 0, invocation: [] };
var from = (text) => ownBytes(new TextEncoder().encode(text));
var text = (bytes) => new TextDecoder().decode(copyBytes(bytes, origin7));
function issue(run) {
  try {
    run();
  } catch (e) {
    if (e instanceof CodecIssue)
      return { reason: e.reason, path: e.path };
    throw e;
  }
  throw new Error("accepted invalid JSON");
}
function runBrowserWireVectors() {
  const results = [];
  ...
  check("int64-beyond-safe", () => {
    const decoded = decodeJSON(schema("int"), from("9007199254740993.0"));
  ...
  check("int64-huge-exponent", () => {
    const decoded = decodeJSON(schema("int"), from("1e309"));
  ...
  check("float-finite", () => {
    const decoded = decodeJSON(schema("float"), from("1.25"));
    if (decoded !== 1.25)
      throw new Error(`decoded ${String(decoded)}`);
    const negativeZero = decodeJSON(schema("float"), from("-0"));
    if (!Object.is(negativeZero, -0))
      throw new Error("lost negative zero");
  ...
  check("float-nonfinite-rejected", () => {
    ...
    if (got.reason !== "nonfinite")
      throw new Error(`reason ${got.reason}`);
  ...
  check("variant-canonical-tag", () => {
    ...
    if (identity !== "some")
      throw new Error(`identity ${String(identity)}`);
    return encoded;
  });
  check("variant-wrong-tag", () => {
    const got = issue(() => decodeJSON(variant, from('{"case":"p::some<str>","value":{"value":1}}')));
  ...
  check("option-some-none", () => {
    ...
    if (some !== '{"case":"p::some<int>","value":{"value":7}}')
      throw new Error(`some ${some}`);
    const none = text(encodeJSON(option, record("absent", [])));
    if (none !== '{"case":"p::none","value":{}}')
      throw new Error(`none ${none}`);
    if (recordIdentity(decodeJSON(option, from(some))) !== "present")
      throw new Error("some lost");
    if (recordIdentity(decodeJSON(option, from(none))) !== "absent")
      throw new Error("none lost");
    return `${some} ${none}`;
  });
  ...
  check("nested-roundtrip-stable", () => {
    ...
    if (twice !== once)
      throw new Error(`unstable ${twice}`);
    return once;
  });
  return results;
}

// codec-vectors-entry.ts
globalThis.__canWireResults = runBrowserWireVectors();
