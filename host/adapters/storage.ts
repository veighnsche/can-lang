// D02 delivered reviewed-adapter tier, Op A (PENDING-REVIEW — not admitted).
//
...
// per-origin, quota-bound. Validators and failure leaves below are
// inherited verbatim from the D01 prototype
// (`tests/host-discrimination/adapters/storage.ts`); the only addition
// is the native host binding at the end of this file, which the
// live-browser conformance leg exercises. Native-call fidelity is
// pinned to the living standard (string keys/values, `null` for
// missing keys, `QuotaExceededError` on quota breach,
// `SecurityError`/opaque origin when storage is blocked).
//
// Audit contract: declared I/O types, immutable copying, failure
// mapping, target availability, callback/reentrancy rules (none: the
// surface is synchronous), ownership/disposal (none: stateless).
...
// NativeShape is the minimal surface `createNativeStorageHost` needs
// from a scope: whatever `scope.localStorage` holds, if anything.
// Accessing `localStorage` itself can throw (opaque origin), so the
// binding probes inside a guard and fails closed to `available: false`
// instead of letting a native throw escape.
export type NativeStorageShape = {
  getItem(key: string): string | null;
  setItem(key: string, value: string): void;
  removeItem(key: string): void;
};

function closedNativeHost(): StorageHost {
  const closed = (): never => {
    throw new Error("unreachable: adapter gates on available before calling the host");
  };
  return { available: false, getItem: closed, setItem: closed, removeItem: closed };
}

export function createNativeStorageHost(scope: unknown): StorageHost {
  let candidate: unknown;
  try {
    candidate =
      scope !== null && typeof scope === "object" && "localStorage" in scope
        ? (scope as { localStorage?: unknown }).localStorage
        : undefined;
    ...
    // must read as unavailable, not as an escaping native throw. The
    // probe key cannot collide with adapter traffic: keys are validated
    // before contact, and the probe never runs through the adapter.
    (shape as NativeStorageShape).getItem("__d02_probe__");
    const native = shape as NativeStorageShape;
    return {
      available: true,
      // Closures keep the native receiver: detached Storage methods throw.
      getItem: (key: string): string | null => native.getItem(key),
      setItem: (key: string, value: string): void => {
