// D02 delivered companion tier, Widget C (PENDING-REVIEW — not admitted).
//
...
// The reference renderer below stands in for any real chart engine;
// its output shape (opaque bytes plus typed table) is the contract,
// not its pixels.
//
// Validators, strict codecs, and failure leaves are inherited verbatim
// from the D01 prototype
// (`tests/host-discrimination/companions/chart-widget.ts`). Deliberate
...
export const CHART_PROTOCOL = "d02.chart/1";
export const CHART_TITLE_MAX_CHARS = 128;
export const CHART_LABEL_MAX_CHARS = 64;
export const CHART_POINTS_MAX = 366;
// CHART_SELECT_ROUNDTRIP_BUDGET_MS pins the X-R01-1 packaging latency
// trip wire: a D02-measured select roundtrip above this budget on the
// loopback conformance leg reactivates the tier choice (T2 first).
// 250ms is orders of magnitude above measured loopback time yet below
// the perception floor for occasional-selection analytics widgets, so
// only a structural regression can trip it. Production network
// latency is an operator concern outside this leg; the budget guards
// the payload contract, not the WAN.
export const CHART_SELECT_ROUNDTRIP_BUDGET_MS = 250;
...
export type CompanionTransport = {
  send(url: string, authorization: string, body: Uint8Array): Promise<Uint8Array>;
};

// createFetchTransport is the production-shaped transport: one HTTP
// POST per chart op with the Bearer credential. Non-2xx statuses and
// network failures surface as transport_failed with no native text;
// the response bytes still go through the strict response codec, so a
// confused or tampered server cannot inject phantom artifacts.
export function createFetchTransport(): CompanionTransport {
  return {
    async send(url: string, authorization: string, body: Uint8Array): Promise<Uint8Array> {
      ...
      if (!response.ok) throw new Error(`companion status ${response.status}`);
...
export function createChartCompanionClient(init: Readonly<{
  ...
  const roundtrip = async (request: Omit<ChartRequest, "v" | "tenant" | "pool" | "correlation">): Promise<CompanionChartResult<ChartResponse & { ok: true }>> => {
    ...
    let raw: Uint8Array;
    try {
      raw = await init.transport.send(init.endpoint, `Bearer ${secret}`, body);
    } catch {
...
export function chartCompanionPolicy(): DestinationPolicy {
  return destinationPolicy({
    version: "2026-09-26.d02-chart",
    rules: [{ scheme: "https", host: "companion.example", pathPrefix: "/v1/", credential: "D02_CHART_COMPANION_TOKEN" }],
    redirect: "deny",
    maxRedirectHops: 0,
    loopback: "deny",
