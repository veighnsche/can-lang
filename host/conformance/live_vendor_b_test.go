...
// liveVendorBDebtCommand is the exact command that runs the Vendor B live
// legs once C01 provisions the pinned browsers. It is the skip message
// below and the debt ledger in live/README.md — one string, three places.
const liveVendorBDebtCommand = "CAN_D03_LIVE=1 go test ./host/conformance/ -run TestLiveVendorBLegs -count=1 -v"

// TestLiveVendorBLegs is the CI-shaped gate for the pinned Vendor B
// live-browser legs (the delivered D02 client driving Vendor B from real
// page context on Chromium/WebKit/Firefox). While C01 is blocked-open it
// skips with the precise unblock command; it never claims an unrun leg as
// passing. With CAN_D03_LIVE=1 it pre-bundles the page entry and the
// Vendor B server, resolves the pinned Playwright harness, runs the
...
func TestLiveVendorBLegs(t *testing.T) {
	...
	if _, err := os.Stat(playwright); err != nil {
		t.Fatalf("pinned playwright missing at %s; provision per live/README.md, then: %s", playwright, liveVendorBDebtCommand)
	}
	bun, err := exec.LookPath("bun")
	if err != nil {
		t.Fatal("bun not on PATH; live legs need bun build for the page and server bundles")
	}
	...
	for _, entry := range [][2]string{
		{"host/conformance/live/vendor-b-page-entry.ts", "vendor-b-client.bundle.js"},
		{"host/companions/chart-vendor-b.ts", "vendor-b-server.bundle.mjs"},
	} {
