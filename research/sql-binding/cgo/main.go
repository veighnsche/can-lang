// Command cgo runs the shared SQL corpus through the official CGo binding,
// github.com/pganalyze/pg_query_go/v6. It is the reference implementation:
// libpg_query is the actual PostgreSQL parser, so its raw outputs pin the
// expected corpus values. Modes: run the corpus, --once for cold timing,
// --batch for memory. Keep in sync with ../wasm/main.go by structure; the
...
func runOne(sql string) Record {
	...
	tree, parseErr := pg_query.Parse(sql)
	...
	scan, scanErr := pg_query.Scan(sql)
...
func main() {
	...
	fmt.Fprintf(os.Stderr, "cgo: %d cases, params per case: %s\n", len(records), summarize(records, corpus))
}
