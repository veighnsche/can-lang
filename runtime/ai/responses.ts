...
export class ResponseIssue extends Error {
  constructor(
    ...
    super("invalid generation response");
  }
}
...
function encodeProtocol(value: unknown, bytes: number) {
  ...
  const nodes: SchemaNode[] = [
    { identity: prefix + "str", kind: "primitive", name: "str" },
    { identity: prefix + "bool", kind: "primitive", name: "bool" },
    { identity: prefix + "int", kind: "primitive", name: "int" },
    { identity: prefix + "strings", kind: "array", name: "strings", element: prefix + "str" },
  ];
  let serial = 0;
  function project(value: unknown): Readonly<{ type: string; value: unknown }> {
    ...
    if (Array.isArray(value) && value.every((item) => typeof item === "string"))
      return { type: prefix + "strings", value: array(value) };
    if (!object(value)) throw new TypeError("unsupported compiler protocol value");
    const identity = prefix + serial++,
      fields = Object.entries(value).map(([name, child]) => ({ name, ...project(child) }));
    nodes.push({
      identity,
      kind: "record",
      name: "protocol",
    ...
    return {
      ...
      value: record(
        identity,
        fields.map(({ name, value }) => [name, value] as const),
      ),
    };
  }
  const root = project(value);
  return encodeJSON({ root: root.type, nodes }, root.value, bytes);
}
export function encodeResponseRequest(
  model: string,
  instructions: string,
  stateSchema: Schema,
  state: unknown,
  ...
  const outputFormat =
    format === undefined
      ? { type: "text" }
      : { type: "json_schema", name: format.name, strict: true, schema: JSON.parse(format.schema) };
  return encodeProtocol(
    {
      model,
      instructions,
      input,
      max_output_tokens: BigInt(maxOutputTokens),
      store: false,
      stream: false,
      background: false,
      truncation: "disabled",
...
export function decodeResponse(
  ...
  const invalid = (reason: string): never => {
    throw new ResponseIssue("invalid", reason);
  };
  if (
    !object(parsed) ||
    !text(parsed.id) ||
    ...
    !["completed", "incomplete", "failed", "cancelled", "queued", "in_progress"].includes(
      parsed.status,
    ) ||
  ...
  if (parsed.status === "queued" || parsed.status === "in_progress")
    return invalid("nonterminal_response");
  ...
  for (const part of message.content as unknown[]) {
    ...
    if (part.type === "refusal") {
      if (typeof part.refusal !== "string") return invalid("output_shape");
      refused = true;
    } else if (part.type === "output_text") {
      if (typeof part.text !== "string") return invalid("output_shape");
  ...
  if (format !== undefined)
    return decodeJSON(format.codec, ownBytes(new TextEncoder().encode(output)), bytes);
  return output;
}
...
export function createResponses(
  ...
  function issue(
    ...
    if (cause instanceof CodecIssue) {
      ...
      identity =
        cause.kind === "instructions"
          ? types.invalid
          : cause.kind === "invalid"
            ? types.invalidResponse
            : cause.kind === "refused"
              ? types.refused
              : types.truncated;
    ...
    return failure(
      domain.create(identity, record(identity, fields), where, undefined, { boundary, operation }),
    );
  }
    ...
    async generate<T>(
      ...
      return invoke(async () => {
        ...
        try {
          ...
          return issue(cause, where, operation, connection.maxBodyBytes);
        }
