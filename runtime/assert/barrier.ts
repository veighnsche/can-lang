...
type Request={frame:FrameState;sequence:bigint;deliver:()=>Completion|Promise<Completion>;resolve:(value:Completion)=>void;reject:(cause:unknown)=>void};
type State={root:InvocationIdentity;frames:Set<FrameState>;identities:WeakSet<object>;active:number;pending:Set<Request>;incoming:Request[];heap:Request[];sequence:bigint;queued:boolean;compare:(a:InvocationIdentity,b:InvocationIdentity)=>number};
...
function blocking(phase:Phase):number{return phase==="running"||phase==="reserved"?1:0;}
function phase(entry:FrameState,next:Phase):void{
 entry.barrier.active+=blocking(next)-blocking(entry.phase);entry.phase=next;
}
...
function insert(state:State,request:Request):void{
 const heap=state.heap;let index=heap.length;heap.push(request);
 while(index>0){
...
function take(state:State):Request{
 const heap=state.heap,request=heap[0],last=heap.pop()!;
 if(heap.length){
  let index=0;
...
function changed(state:State):void{
 ...
 queueMicrotask(()=>{
  ...
  try{
   ...
   state.heap=[];state.incoming=[];
   for(const pending of state.pending){phase(pending.frame,"running");pending.reject(cause)}
...
export function createBarrier(root:InvocationIdentity,compare=compareInvocations):Barrier{
 ...
 barriers.set(value,{root,frames:new Set(),identities:new WeakSet(),active:0,pending:new Set(),incoming:[],heap:[],sequence:0n,queued:false,compare});
 return value;
}
export function reserveFrame(owner:Barrier,identity:InvocationIdentity):Frame{
 ...
 frames.set(value,entry);state.frames.add(entry);state.active++;return value;
}
...
export function fixtureEvent(value:Frame,deliver:()=>Completion|Promise<Completion>):Promise<Completion>{
 ...
 const pending=new Promise<Completion>((resolve,reject)=>{
  const state=entry.barrier,request={frame:entry,sequence:state.sequence++,deliver,resolve,reject};
  state.pending.add(request);state.incoming.push(request);
 });
 changed(entry.barrier);return pending;
}
export function barrierState(owner:Barrier){
 const state=barrier(owner);
 return Object.freeze({pending:state.pending.size,frames:Object.freeze([...state.frames].map(value=>Object.freeze({path:invocationPath(value.identity),phase:value.phase})))});
}
