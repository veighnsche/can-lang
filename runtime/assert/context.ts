...
export function fixtureIndex(context: AssertionContext, table: string, total: number, origin: FailureOrigin): number {
  ...
  const current = state(context), queue = current.tables.get(table)!;
  if (queue.used >= queue.total) throw violation(context, "missing fixture", origin);
  return queue.used++;
}
export function registerFixtureTable(context: AssertionContext, table: string, total: number, origin: FailureOrigin): void {
  const current = state(context);
