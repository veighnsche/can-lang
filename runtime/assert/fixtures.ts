...
export async function withFixture(
  ...
  const sameArguments = row.raw !== undefined ? completionValueEqual(actualArguments, expectedArguments.value) : assertionEqual(actualArguments, expectedArguments.value);
  ...
  if (row.raw !== undefined) {
    ...
    if (!completionEqual(actual, expected)) throw fixtureMismatch(context, allocation, "outcome mismatch", origin);
    return actual;
  }
  const expected = await invoke(row.expected, origin);
  if (expected.kind === "standard") throw fixtureMismatch(context, allocation, "malformed fixture", origin);
