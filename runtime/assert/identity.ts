...
export function callableIdentity(
  ...
  const parent = frame(parentIdentity),
    visit = occurrence(parent, site),
    identity = token<CallableIdentity>();
  ...
  const key = createHash("sha256")
    .update("can-callable-instance-v1\0" + JSON.stringify([parent.path, site, visit]))
    .digest("hex");
  instances.set(identity, {
    family: parent.family,
    path: Object.freeze({ site, occurrence: visit, instance: key }),
