...
import{contextOwner,fixtureIndex,policyFixtureEvidence,registerFixtureTable,violation,type AssertionContext}from"./context.ts";
...
export function provideInjection(context:AssertionContext,operation:string,origin:string,identity:string,payload:unknown,failed:string,domain:Domain,where:FailureOrigin):void{
  if(origin!=="native"&&origin!=="emitted")throw violation(context,"malformed fixture",where);
  const owner=contextOwner(context);
  let table=pending.get(owner);
  if(table===undefined){table=new Map();pending.set(owner,table);}
  if(table.has(operation))throw violation(context,"malformed fixture",where);
  const completion=origin==="native"
    ?failure(domain.create(failed,record(failed,[["detail",payload]]),where,domain.create(identity,payload,where,undefined,{boundary:"native",operation}),{boundary:"native",operation}))
    :failure(domain.create(identity,payload,where,undefined,{boundary:"emitted",operation}));
  table.set(operation,completion);
  registerFixtureTable(context,"can:policy:"+operation,1,where);
}
export async function policyBase<T>(context:AssertionContext|undefined,operation:string,where:FailureOrigin,invoke:()=>Promise<Completion<T>>):Promise<Completion<T>>{
...
export function policyKey(completion:Completion<unknown>,failed:string):PolicyKey{
  ...
  if(details.typeIdentity===failed&&details.provenance.boundary==="native"){
    if(!isDomainFailure(details.cause))return{origin:"native",identity:details.typeIdentity};
    return{origin:"native",identity:domainFailureDiagnostics(details.cause).typeIdentity};
  }
  if(details.provenance.boundary==="native")return{origin:"native",identity:details.typeIdentity};
