...
const origin: FailureOrigin = Object.freeze({source: "can:bytes", start: 0, end: 0, invocation: Object.freeze([])});
function backing(value: unknown): Uint8Array {
  if (!isBytes(value)) throw resourceStateFailure(undefined, origin);
  return storage.get(value)!;
}
export function byteLength(value: unknown): bigint { return BigInt(backing(value).length); }
...
export function createBytes(domain: ReturnType<typeof createDomainRuntime>, invalidData: string) {
  function invalid(path: string, reason: string) {
    return failure(domain.create(invalidData, record(invalidData, [["path", path], ["reason", reason]]), origin));
  }
  return Object.freeze({
    async empty(_context?: AssertionContext): Promise<Completion<Bytes>> { return success(ownBytes(new Uint8Array(0))); },
    async fromInts(values: readonly bigint[], _context?: AssertionContext): Promise<Completion<Bytes>> {
    ...
    async toInts(value: unknown, _context?: AssertionContext): Promise<Completion<readonly bigint[]>> {
    ...
    async fromUTF8(text: string, _context?: AssertionContext): Promise<Completion<Bytes>> {
      ...
      if (new TextDecoder("utf-8", {fatal: true, ignoreBOM: true}).decode(encoded) !== text) return invalid("", "unicode_scalar");
    ...
    async toUTF8(value: unknown, _context?: AssertionContext): Promise<Completion<string>> {
      ...
      try { return success(new TextDecoder("utf-8", {fatal: true, ignoreBOM: true}).decode(bytes)); }
      catch (cause) {
