...
export function scanJSON(text: string): string | undefined {
  ...
  for (let i = 0; i < text.length;) {
    ...
    if (ch === '"') {
      ...
      if (frame?.keys && text[next] === ":") {
        try {
          ...
          if (typeof key === "string") {
            if (frame.keys.has(key) && duplicate===undefined) duplicate = childPath(frame.path,key);
            frame.key=key;
            frame.keys.add(key);
          }
        } catch (cause) {
      ...
      } else {valuePath();budget.visit(stack.length, "");}
      continue;
    }
    ...
    valuePath();budget.visit(stack.length, "");
