...
function projectFormat(schema:Schema,input:unknown,parse:(text:string)=>unknown,reason:"invalid_toml"|"invalid_yaml"|"invalid_json5",bytes:number):unknown {
  const text=decodeText(input,bytes);
  const budget=new Budget(bytes);
  let parsed:unknown;
  try{parsed=parse(text);}
  catch(cause){if(cause instanceof Error)reject("",reason);throw cause;}
