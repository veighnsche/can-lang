...
export function projectJsonlRecord(
  schema: Schema,
  line: Uint8Array,
  index: number,
  budget: Budget,
  ...
  try {
    ...
    if (cause instanceof TypeError) reject(path, "utf8");
    throw cause;
  }
...
function projectIndexed(
  ...
  parsed: unknown,
  holder: object,
  tokens: WeakMap<object, Map<string, string>>,
  path: string,
  budget: Budget,
): unknown {
  try {
    return projectValue(schema, parsed, holder, budget, exactInt(tokens));
  } catch (cause) {
    if (cause instanceof CodecIssue) throw new CodecIssue(path + cause.path, cause.reason);
    throw cause;
  }
...
// every record.
export function decodeJsonlRecords(
  schema: Schema,
  input: unknown,
  ...
  for (const line of framer.push(raw))
    records.push(projectJsonlRecord(schema, line, index++, budget));
