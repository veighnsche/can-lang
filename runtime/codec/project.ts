import { array, dataKeys, dataProperty, record } from "../data.ts";
...
export function graph(schema: Schema) {
  ...
  return (id: string): SchemaNode => {
    const node = nodes.get(id);
    if (!node) throw new TypeError("missing codec schema node");
    return node;
  };
}
...
export const exactInt =
  (tokens: WeakMap<object, Map<string, string>>): IntPolicy =>
  ...
  function visit(
    ...
    try {
      if (node.kind === "variant") {
        ...
        if (typeof tag !== "string") reject(childPath(path, "case"), "type");
