...
export function forEach<T>(
  ...
  return boundary(
    () =>
      source.reduce<Promise<Completion<void>>>(
        (prior, item) =>
          prior.then(async () => {
...
export function some<T>(
  ...
  return boundary(async () => {
    ...
    return result.kind === "ok" ? success(result.value !== -1) : result;
  }, trace);
}
...
export function find<T>(
  ...
  return boundary(async () => {
    ...
    return success(
      result.value === -1
        ? record(identities.none, [])
        : record(identities.some, [["value", source[result.value]]]),
    );
  }, trace);
}
...
export function concat<T>(left: readonly T[], right: readonly T[]): readonly T[] {
  return array(left.concat(right));
}
export function toReversed<T>(source: readonly T[]): readonly T[] {
  return array(source.toReversed());
}
export function append<T>(source: readonly T[], item: T): readonly T[] {
