...
const identity = createHash("sha256").update("can-concrete-type-v1\0"+JSON.stringify(["error",declaration.identity])).digest("hex");
const domain = createDomainRuntime({declarations:[declaration],shapes:[{identity,kind:"error",declaration:declaration.identity,arguments:[],fields:[],leaves:[],inputs:[],errors:[]}]});
...
test("async calls, continuations, elements and native Promise bridges never assimilate then data", async () => {
  let invoked=0;
  const payload=record("then-data",[["then",()=>{invoked++;throw new Error("assimilated data")}],["value",3n]]);
  ...
  for(const settled of await Promise.allSettled(promises)) {
    expect(settled.status).toBe("fulfilled"); if(settled.status==="fulfilled")expect(value(settled.value)).toBe(payload);
  }
  expect(invoked).toBe(0);
  for(const carrier of [nested,...mapped]) {
    expect(Object.getPrototypeOf(carrier)).toBeNull(); expect(Object.isFrozen(carrier)).toBe(true);
    expect("then" in carrier).toBe(false);
    for(const descriptor of Object.values(Object.getOwnPropertyDescriptors(carrier))) {
      expect("value" in descriptor).toBe(true);expect(descriptor.writable).toBe(false);expect(descriptor.configurable).toBe(false);
    }
  }
});
test("void and distinct failure categories round trip with original occurrences", async()=>{
  ...
  const error=fresh(), standard=captureStandard(new Error("broken"),origin);
  for(const occurrence of [error,standard]) {
    ...
    expect(recovered).toBe(completion);expect(recovered.value).toBe(occurrence);
  }
  expect(domainFailureDiagnostics(error).occurrenceID).not.toBe(standardFailureOccurrenceID(standard));
  const winner=await Promise.any([toPromise(Promise.resolve(failure(error))),toPromise(Promise.resolve(success(4n)))]);
  expect(value(winner)).toBe(4n);
  const caught=await invoke(()=>{throw standard},origin);expect(caught.value).toBe(standard);
});
test("selected handler failure is never redispatched and nested handlers complete locally",async()=>{
  const first=fresh(),second=fresh();let visits=0;
  const handlers={ok:()=>success(0n),domain:new Map([[identity,()=>{visits++;return failure(second)}]])};
  const result=await dispatch(failure(first),handlers,origin);
  expect(result.kind).toBe("domain");expect(result.value).toBe(second);expect(visits).toBe(1);
  const standard=captureStandard("handler failed",origin);
  const failed=await dispatch(success(1n),{ok:()=>{throw standard},domain:new Map(),standard:()=>{throw new Error("redispatched")}},origin);
  expect(failed.value).toBe(standard);
  const nested=await dispatch(success(2n),{ok:async n=>{
    const local=await dispatch(success(n),{ok:v=>success(v+1n),domain:new Map()},origin);
    return success(value(local)+1n);
  },domain:new Map()},origin);
  expect(value(nested)).toBe(4n);
});
test("forged carriers and immediate unboxed thenables are refused without executing data",async()=>{
  let invoked=0;const raw={then(){invoked++}};
  const result=await invoke((()=>raw) as any,origin);expect(result.kind).toBe("standard");expect(invoked).toBe(0);
