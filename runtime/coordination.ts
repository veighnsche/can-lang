...
export async function settle(mode:Mode,participants:readonly Participant[]):Promise<Selection>{
 ...
 try{
  ...
  if(mode==="any"){
   ...
   if(outcomes.length!==participants.length||outcomes.some(value=>value===undefined||value.kind==="ok"))throw new TypeError("invalid all-failed settlement");
   owner.publish(all());
...
export async function handle(selection:Selection,handlers:Handlers,collect:boolean,origin:FailureOrigin):Promise<Completion>{
 if(selection.kind==="one")return invoke(()=>handlers.shared(selection.index,selection.completion),origin);
 if(selection.kind==="all-failed")return invoke(()=>handlers.allFailed(selection.outcomes),origin);
...
export function aggregate(outcomes:readonly Completion[],domain:ReturnType<typeof createDomainRuntime>,identity:string,origin:FailureOrigin):Completion{
 const failures=Object.freeze(outcomes.map(outcome=>{
