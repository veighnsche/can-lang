...
type Span = Readonly<{start:number;end:number;line:number;column:number;endLine:number;endColumn:number;operation:string}>;
type Source = Readonly<{id:string;path:string;spans:Record<string,Span>}>;
type Frame = Readonly<{source:string;file:string;line:number;column:number;endLine:number;endColumn:number;start:number;end:number;operation:string;synthetic:boolean}>;
...
export function configureDiagnostics(entryURL: string): void {
  ...
  if (index.schemaVersion!==1 || index.kind!=="can.source-index") throw new TypeError("invalid diagnostic index");
  ...
  sources=nextSources;modules=nextModules;
}
function frame(source:Source, span:Span, synthetic:boolean):Frame {
  ...
  return Object.freeze({source:source.id,file:source.path,line:span.line,column:span.column+1,
    endLine:span.endLine,endColumn:span.endColumn+1,start:span.start,end:span.end,operation:span.operation,synthetic});
}
function safeStack(cause:unknown):string|undefined {
  try {
    if ((typeof cause!=="object" && typeof cause!=="function") || cause===null || nativeTypes.isProxy(cause) || !nativeTypes.isNativeError(cause)) return;
...
export function diagnosticFrames(cause:unknown, origin:FailureOrigin):readonly Frame[] {
  ...
  if (stack) for (const text of stack.split("\n").slice(1,65)) {
    ...
    const mapped=originalPositionFor(map,{line:Number(match[2]),column:Number(match[3])-1});
    const source=sources.get(mapped.source ?? ""), span=source?.spans[mapped.name ?? ""];
    if (source && span) frames.push(frame(source,span,false));
  }
  const source=sources.get(origin.source);
  const span=source && Object.values(source.spans).find(s=>s.start===origin.start && s.end===origin.end);
  if (source && span && !frames.some(f=>f.source===source.id && f.start===span.start && f.end===span.end)) frames.unshift(frame(source,span,true));
