...
// never authored runtime registrations or untrusted wire schemas. This core is
// canonical for both hosts: identity verification takes an injected digest so
// Bun uses sync Node hashing while the browser verifies asynchronously with
// WebCrypto over the identical canonical input bytes and then constructs from
...
export function concreteTypeDigestInput(key: unknown): string {
  return "can-concrete-type-v1\0" + JSON.stringify(key);
}

export function shapeIdentityKey(
  shape: FailureShape,
  resolve: (id: string | undefined) => FailureShape,
): unknown {
  if (shape.kind === "array") return ["array", "", resolve(shape.element).identity];
  if (shape.kind === "callable" || shape.kind === "choice_arm")
    return [shape.kind, resolve(shape.result).identity, [...shape.inputs], [...shape.errors]];
  return [shape.kind, shape.declaration ?? "", ...shape.arguments];
}

export type IdentityDigest = (input: string, claimed: string) => string;
export type DomainRuntime = {
  accepts: (typeIdentity: string, value: unknown) => boolean;
  create: (
    typeIdentity: string,
    payload: unknown,
    origin: FailureOrigin,
  ...
  checkBound: (occurrence: DomainFailure, allowed: readonly string[]) => DomainFailure;
};

export function createDomainRuntimeWithDigest(
  plan: ErrorPlan,
  digestHex: IdentityDigest,
  ...
  for (const s of shapes.values()) {
    const key = shapeIdentityKey(s, get);
    const expected = digestHex(concreteTypeDigestInput(key), s.identity);
