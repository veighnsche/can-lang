...
import {
  ...
  assertionFailure,
  assertionFailureClasses,
  nonfiniteSortKeyFailure,
} from "./failure";
...
test("native descriptions never invoke application behavior", () => {
  ...
  const throwingToString = new TypeError("safe");
  throwingToString.toString = trap;
  ...
  const prototypeProxy = new Error("safe");
  Object.setPrototypeOf(prototypeProxy, proxy);
  expect(describeNativeFailure(prototypeProxy)).toBe("Error: safe");
  const circular: any = { credential: "secret" };
...
test("native scalar and Error descriptions follow fixed rules", () => {
  for (const [value, expected] of [
    ...
    [NaN, "NaN"],
    [-0, "0"],
    ["authored throw", "authored throw"],
    [Symbol("secret"), "native symbol"],
    [() => 0, "native function"],
  ] as const)
    expect(describeNativeFailure(value)).toBe(expected);
  ...
  Object.defineProperty(error, "name", { value: "Explicit" });
  Object.defineProperty(error, "credential", { value: "secret" });
  Object.defineProperty(error, "responseBody", { value: "secret" });
...
test("origins are distinct and propagation preserves an occurrence", () => {
  ...
  const first = captureStandard(cause, origin);
  const second = captureStandard(cause, origin);
  ...
  const occurrence = captureStandard("x", { ...origin, invocation });
  invocation[0] = "changed";
...
test("primitive and maintained categories use fixed messages", () => {
  ...
  const messages = [
    ...
    "arithmetic: integer remainder by zero",
    "arithmetic: negative integer exponent",
    "bounds: index out of range",
  ];
  actions.forEach((action, i) => {
    try {
      action();
      throw new Error("missing fault");
    } catch (cause) {
      const occurrence = captureStandard(cause, origin);
      expect(standardFailureMessage(occurrence)).toBe(messages[i]);
      expect(standardFailureDiagnostics(occurrence).cause).toBe(cause);
    }
  });
  expect(standardFailureMessage(resourceStateFailure("secret", origin))).toBe(
    "resource_state: invalid resource use",
  );
  expect(standardFailureMessage(cleanupFailure({ credential: "secret" }, origin))).toBe(
    "cleanup: automatic resource cleanup failed",
  );
  expect(standardFailureMessage(nonfiniteSortKeyFailure(Infinity, origin))).toBe(
    "arithmetic: nonfinite sort key",
  );
  for (const name of assertionFailureClasses)
    expect(standardFailureMessage(assertionFailure(name, origin))).toBe("assertion: " + name);
  expect(() => assertionFailure("secret" as any, origin)).toThrow(
    "unknown assertion failure class",
  );
});
...
test("description adapter defects retain the original cause with a fixed fallback", () => {
  ...
  try {
    Object.getOwnPropertyDescriptor = () => {
      throw new Error("private adapter defect");
    };
...
test("synthetic failures retain identity and original metadata when the first checked boundary is attached", () => {
  ...
  const failure = assertionFailure("missing fixture", synthetic),
    id = standardFailureOccurrenceID(failure);
