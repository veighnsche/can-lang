...
export type StandardKind =
  ...
  | "native_exception"
  | "cleanup";
export type FailureOrigin = Readonly<{
  source: string;
  start: number;
  end: number;
...
type StandardDetails = Readonly<{
  occurrenceID: bigint;
  kind: StandardKind;
  message: string;
  cause: unknown;
  origin: FailureOrigin;
  boundaryOrigin?: FailureOrigin;
}>;
...
export function freezeProvenance(provenance?: unknown): FailureProvenance {
  ...
  if (typeof provenance !== "object" || provenance === null)
    throw new TypeError("invalid failure provenance");
  const { boundary, operation } = provenance as Partial<FailureProvenance>;
  if ((boundary !== "native" && boundary !== "emitted") || typeof operation !== "string")
    throw new TypeError("invalid failure provenance");
  if (boundary === "native" && operation === "")
    throw new TypeError("native failure requires its producing operation");
...
export function describeNativeFailure(value: unknown): string {
  try {
    ...
    const name =
      nameDescriptor && "value" in nameDescriptor && typeof nameDescriptor.value === "string"
        ? nameDescriptor.value
        : prototypeErrorName(value);
    const message =
      messageDescriptor &&
...
export function captureStandard(cause: unknown, origin: FailureOrigin): StandardFailure {
  if (objectLike(cause) && standard.has(cause)) {
    ...
    if (
      details.origin.source.startsWith("can:") &&
      !origin.source.startsWith("can:") &&
      origin.source !== "" &&
      !details.boundaryOrigin
    ) {
...
export function isStandardFailure(value: unknown): value is StandardFailure {
  return objectLike(value) && standard.has(value);
}
...
export const assertionFailureClasses = Object.freeze([
  "missing fixture",
  "argument mismatch",
  "ambiguous fixture",
  "malformed fixture",
...
export function assertionFailure(
  failureClass: (typeof assertionFailureClasses)[number],
  origin: FailureOrigin,
): StandardFailure {
  if (!(assertionFailureClasses as readonly string[]).includes(failureClass))
    throw new TypeError("unknown assertion failure class");
...
export function standardFailureKind(value: StandardFailure): StandardKind {
  return details(value).kind;
}
export function standardFailureMessage(value: StandardFailure): string {
  return details(value).message;
}
export function standardFailureOccurrenceID(value: StandardFailure): bigint {
  return details(value).occurrenceID;
}
...
export function standardFailureDiagnostics(value: StandardFailure): StandardDetails {
