...
export type NumberErrors = Readonly<{
  inexact: string;
  invalidNumber: string;
  invalidTextBool: string;
  invalidIntBool: string;
}>;
...
export function createNumbers(
  ...
  return Object.freeze({
    async fromInt(value: bigint, _context?: AssertionContext): Promise<Completion<string>> {
      return success(String(value));
    },
    async fromFloat(value: number, _context?: AssertionContext): Promise<Completion<string>> {
      return success(String(value));
    },
    async fromBool(value: boolean, _context?: AssertionContext): Promise<Completion<string>> {
      return success(String(value));
    },
    async intToFloat(value: bigint, _context?: AssertionContext): Promise<Completion<number>> {
      ...
      if (!Number.isFinite(converted) || BigInt(converted) !== value)
        return invalid(errors.inexact, "reason", "not exactly representable as float");
      return success(converted);
    },
    async floatToInt(value: number, _context?: AssertionContext): Promise<Completion<bigint>> {
      if (!Number.isFinite(value) || !Number.isInteger(value))
        return invalid(errors.inexact, "reason", "not a finite integer");
      return success(BigInt(value));
    },
    async boolToInt(value: boolean, _context?: AssertionContext): Promise<Completion<bigint>> {
      return success(BigInt(Number(value)));
    },
    ...
    async floor(value: number, _context?: AssertionContext): Promise<Completion<number>> {
      return success(Math.floor(value));
    },
    async ceil(value: number, _context?: AssertionContext): Promise<Completion<number>> {
      return success(Math.ceil(value));
    },
    async trunc(value: number, _context?: AssertionContext): Promise<Completion<number>> {
      return success(Math.trunc(value));
    },
    async round(value: number, _context?: AssertionContext): Promise<Completion<number>> {
      return success(Math.round(value));
    },
    async isFinite(value: number, _context?: AssertionContext): Promise<Completion<boolean>> {
      return success(Number.isFinite(value));
    },
    async isNaN(value: number, _context?: AssertionContext): Promise<Completion<boolean>> {
      return success(Number.isNaN(value));
    },
    ...
    async toBool(value: string, _context?: AssertionContext): Promise<Completion<boolean>> {
      if (value !== "true" && value !== "false")
        return invalid(errors.invalidTextBool, "input", value);
...
export function createExactAmounts(
  domain: ReturnType<typeof createDomainRuntime>,
  identities: Readonly<{ zeroDivisor: string; division: string; rounded: string }>,
) {
  const zero = (): Completion<never> =>
    failure(domain.create(identities.zeroDivisor, record(identities.zeroDivisor, []), origin));
  const division = (quotient: bigint, remainder: bigint): Division =>
    record(identities.division, [
      ["quotient", quotient],
      ["remainder", remainder],
    ]) as Division;
  return Object.freeze({
    ...
    async roundRatioHalfEven(
      numerator: bigint,
      denominator: bigint,
      _context?: AssertionContext,
    ): Promise<Completion<Rounded>> {
      if (denominator === 0n) return zero();
      const n = denominator < 0n ? -numerator : numerator,
        d = denominator < 0n ? -denominator : denominator;
      let quotient = n / d;
      const remainder = n % d,
        twice = 2n * (remainder < 0n ? -remainder : remainder);
      if (twice > d || (twice === d && quotient % 2n !== 0n)) quotient += n < 0n ? -1n : 1n;
      return success(
        record(identities.rounded, [
          ["value", quotient],
          ["remainder_numerator", n - quotient * d],
          ["denominator", d],
        ]) as Rounded,
      );
    },
