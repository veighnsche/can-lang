...
function expectedWriteFailure(cause: unknown): boolean {
  ...
  return (
    ...
    writeCodes.has(code.value)
  );
}
export function createCLI(domain: Domain, contracts: Contracts) {
  return Object.freeze({
    async stdoutWrite(bytes: unknown, context?: AssertionContext): Promise<Completion<bigint>> {
      denyLiveBoundary(context, origin);
      return write(bytes, "stdout_write", Bun.stdout);
    },
    async stderrWrite(bytes: unknown, context?: AssertionContext): Promise<Completion<bigint>> {
      denyLiveBoundary(context, origin);
      return write(bytes, "stderr_write", Bun.stderr);
    },
  });
  async function write(
    bytes: unknown,
    operation: string,
    destination: typeof Bun.stdout,
  ): Promise<Completion<bigint>> {
    const data = copyBytes(bytes, origin);
    try {
      return success(BigInt(await Bun.write(destination, data)));
    } catch (cause) {
      if (!expectedWriteFailure(cause)) throw cause;
      return failure(
        domain.create(
          contracts.writeFailed,
          record(contracts.writeFailed, [["operation", operation]]),
          origin,
          cause,
        ),
      );
