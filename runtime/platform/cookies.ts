...
export function createCookies(domain: ReturnType<typeof createDomainRuntime>, ids: Ids) {
  const invalid = (reason: string) =>
    failure(domain.create(ids.invalid, record(ids.invalid, [["reason", reason]]), origin));
  const mint = (jar: Bun.Cookie): object => {
    const handle = Object.freeze(Object.create(null));
    jars.set(handle, jar);
    return handle;
  };
  const project = (value: unknown): Bun.Cookie | undefined =>
    object(value) ? jars.get(value) : undefined;
  ...
  const checkPath = (path: string): ReturnType<typeof invalid> | undefined => {
    try {
      new Bun.Cookie("can-cookie-probe", "", { path });
    } catch {
      return invalid("path");
    }
    return undefined;
  ...
  const checkDomain = (domainName: string): ReturnType<typeof invalid> | undefined => {
    try {
      new Bun.Cookie("can-cookie-probe", "", { domain: domainName });
    } catch {
  ...
  return Object.freeze({
    async parse(header: unknown, _context?: AssertionContext): Promise<Completion<unknown>> {
      ...
      const pairs: unknown[] = [];
      for (const entry of new Bun.CookieMap(header) as unknown as Iterable<
        readonly [unknown, unknown]
      >) {
        const text =
          typeof entry[1] === "string" ? entry[1] : (entry[1] as { value?: unknown } | null)?.value;
        if (typeof entry[0] !== "string" || typeof text !== "string")
          throw new TypeError("invalid native cookie pair");
        pairs.push(
          record(ids.pair, [
            ["name", entry[0]],
            ["value", text],
    ...
    async get(
      ...
      for (const entry of dataArray(dataProperty(collection, "pairs"))) {
        if (dataProperty(entry, "name") === name)
          return success(record(ids.some, [["value", dataProperty(entry, "value")]]));
      }
    ...
    async make(
      ...
      if (typeof name !== "string" || typeof value !== "string")
        throw new TypeError("invalid compiler cookie");
      const path = dataProperty(attributes, "path"),
        domainName = option(dataProperty(attributes, "domain"));
      const secure = dataProperty(attributes, "secure"),
        httpOnly = dataProperty(attributes, "http_only");
      if (
        typeof path !== "string" ||
        (domainName !== undefined && typeof domainName !== "string") ||
        typeof secure !== "boolean" ||
        typeof httpOnly !== "boolean"
      )
        throw new TypeError("invalid compiler cookie attributes");
      const site = sameSite(dataProperty(attributes, "same_site"));
      const maxAge = option(dataProperty(attributes, "max_age")),
        expiresMs = option(dataProperty(attributes, "expires_ms"));
      if (
        (maxAge !== undefined && typeof maxAge !== "bigint") ||
        (expiresMs !== undefined && typeof expiresMs !== "bigint")
      )
        throw new TypeError("invalid compiler cookie attributes");
      if (maxAge !== undefined && !Number.isSafeInteger(Number(maxAge))) return invalid("max_age");
      if (expiresMs !== undefined && (expiresMs > DATE_LIMIT || expiresMs < -DATE_LIMIT))
      ...
      return success(
        mint(
          new Bun.Cookie(name, value, {
            ...
            ...(expiresMs === undefined ? {} : { expires: new Date(Number(expiresMs)) }),
          }),
        ),
      );
    },
    ...
    async remove(
      ...
      if (typeof name !== "string" || typeof path !== "string")
        throw new TypeError("invalid compiler cookie tombstone");
      const domainValue = option(domainName);
      if (domainValue !== undefined && typeof domainValue !== "string")
        throw new TypeError("invalid compiler cookie tombstone");
      const badName = checkName(name);
      if (badName !== undefined) return badName;
      const badPath = checkPath(path);
