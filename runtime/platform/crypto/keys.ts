...
export function createCryptoKeys(
  domain: ReturnType<typeof createDomainRuntime>,
  ids: { keyMisuse: string; invalidKey: string; keypair: string },
) {
  ...
  const invalid = (reason: string) =>
    failure(domain.create(ids.invalidKey, record(ids.invalidKey, [["reason", reason]]), origin));
  return Object.freeze({
    ...
    async generateEd25519Keypair(context?: AssertionContext): Promise<Completion<unknown>> {
      ...
      try {
        ...
        return success(
          record(ids.keypair, [
            ["private_key", mint("Ed25519", ["sign"], pair.privateKey)],
            ["public_key", mint("Ed25519", ["verify"], pair.publicKey)],
          ]),
        );
      } catch (cause) {
        return caught(cause, origin);
    ...
    async exportEd25519Public(
      ...
      if (
        projected.algorithm !== "Ed25519" ||
        !projected.usages.includes("verify") ||
        projected.usages.includes("sign")
      )
        return misuse("export", projected.algorithm);
      try {
        return success(
          ownBytes(new Uint8Array(await crypto.subtle.exportKey("raw", projected.key))),
        );
      } catch (cause) {
        return caught(cause, origin);
      }
