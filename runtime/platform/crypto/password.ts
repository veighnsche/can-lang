...
const phc =
  /^\$argon2id\$v=19\$m=(\d{1,7}),t=(\d{1,2}),p=(\d)\$[A-Za-z0-9+/]{43}\$[A-Za-z0-9+/]{43}$/;
...
export function createPasswords(
  ...
  ids: { costRejected: string; invalidHash: string },
) {
  const cost = (profile: bigint) =>
    failure(
      domain.create(ids.costRejected, record(ids.costRejected, [["profile", profile]]), origin),
    );
  const malformed = () =>
    failure(
      domain.create(ids.invalidHash, record(ids.invalidHash, [["reason", "format"]]), origin),
    );
  return Object.freeze({
    async hash(
      ...
      try {
        ...
        return success(
          ...
          } as Parameters<typeof Bun.password.hash>[1]),
        );
      } catch (cause) {
    ...
    async verify(
      ...
      if (typeof password !== "string" || typeof encoded !== "string")
        throw new TypeError("invalid compiler password input");
