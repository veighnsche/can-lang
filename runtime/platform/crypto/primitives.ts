...
const use = (handle: unknown, algorithm: KeyAlgorithm, usage: string): KeyUse => {
  ...
  if (projected.algorithm !== algorithm || !projected.usages.includes(usage))
    return { reject: projected.algorithm };
  return { key: projected.key };
};
export function createCryptoPrimitives(
  ...
  const misuse = (operation: string, algorithm: string) =>
    failure(
      domain.create(
        ids.keyMisuse,
        record(ids.keyMisuse, [
          ["operation", operation],
          ["algorithm", algorithm],
        ]),
        origin,
  ...
  const badNonce = (length: bigint) =>
    failure(
      domain.create(ids.invalidNonce, record(ids.invalidNonce, [["length", length]]), origin),
    );
  const corrupt = () =>
    failure(domain.create(ids.decryptFailed, record(ids.decryptFailed, []), origin));
  return Object.freeze({
    async hmacSha256(
      ...
      const keyBytes = copyBytes(key, origin),
        messageBytes = copyBytes(message, origin);
      if (keyBytes.length === 0)
        return failure(
          domain.create(
            ids.invalidKey,
            record(ids.invalidKey, [["reason", "empty_hmac_key"]]),
            origin,
          ),
        );
      try {
        ...
        return success(
          ownBytes(
            new Uint8Array(await crypto.subtle.sign("HMAC", native, bufferView(messageBytes))),
          ),
        );
      } catch (cause) {
    ...
    async encryptAesGcm(
      ...
      const nonceBytes = copyBytes(nonce, origin),
        plainBytes = copyBytes(plaintext, origin),
        aadBytes = copyBytes(associated, origin);
      if (nonceBytes.length !== 12) return badNonce(BigInt(nonceBytes.length));
      try {
        const cipher = await crypto.subtle.encrypt(
          {
            name: "AES-GCM",
            iv: bufferView(nonceBytes),
            additionalData: bufferView(aadBytes),
            tagLength: 128,
          },
          got.key,
          bufferView(plainBytes),
        );
    ...
    async encryptAesGcmSealed(
      ...
      const got = use(handle, "AES-GCM", "encrypt");
      if ("reject" in got) return misuse("encrypt", got.reject);
      const plainBytes = copyBytes(plaintext, origin),
        aadBytes = copyBytes(associated, origin);
      const nonceBytes = crypto.getRandomValues(new Uint8Array(12));
      try {
        const cipher = await crypto.subtle.encrypt(
          { name: "AES-GCM", iv: nonceBytes, additionalData: bufferView(aadBytes), tagLength: 128 },
          got.key,
          bufferView(plainBytes),
        );
        return success(
          record(ids.sealed, [
            ["nonce", ownBytes(nonceBytes)],
            ["ciphertext", ownBytes(new Uint8Array(cipher))],
          ]),
        );
      } catch (cause) {
        return caught(cause, origin);
    ...
    async decryptAesGcm(
      ...
      const nonceBytes = copyBytes(nonce, origin),
        cipherBytes = copyBytes(ciphertext, origin),
        aadBytes = copyBytes(associated, origin);
      if (nonceBytes.length !== 12) return badNonce(BigInt(nonceBytes.length));
      try {
    ...
    async signEd25519(
      ...
      try {
        return success(
          ownBytes(
            new Uint8Array(await crypto.subtle.sign("Ed25519", got.key, bufferView(messageBytes))),
          ),
        );
      } catch (cause) {
        return caught(cause, origin);
    ...
    async verifyEd25519(
      ...
      const messageBytes = copyBytes(message, origin),
        signatureBytes = copyBytes(signature, origin);
      try {
        return success(
          await crypto.subtle.verify(
            "Ed25519",
            got.key,
            bufferView(signatureBytes),
            bufferView(messageBytes),
          ),
        );
      } catch (cause) {
        return caught(cause, origin);
