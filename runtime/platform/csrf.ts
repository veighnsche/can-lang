...
export function createCSRF(
  ...
  const duration = (value: unknown): number | ReturnType<typeof invalid> => {
    if (typeof value !== "bigint" || value < 0n || value > DURATION_LIMIT)
  ...
  const credential = (
    ...
    if (typeof secret !== "string" || typeof session !== "string")
      throw new TypeError("invalid compiler csrf credential");
  ...
  return Object.freeze({
    async generate(
      ...
      try {
        return success(
          Bun.CSRF.generate(secret as string, { sessionId: session as string, expiresIn: expires }),
        );
      } catch (cause) {
