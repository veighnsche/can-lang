...
const origin=Object.freeze({source:"can:time",start:0,end:0,invocation:Object.freeze([])});
...
type Civil={year:number;month:number;day:number;hour:number;minute:number;second:number;millisecond:number};
const civilOf=(value:unknown):Civil=>{
 const field=(name:string):unknown=>{try{return dataProperty(value,name);}catch{throw new TypeError("invalid compiler civil time");}};
...
const civilValid=(civil:Civil):boolean=>
 Number.isInteger(civil.year)&&civil.year>=-271821&&civil.year<=275760&&civil.month>=1&&civil.month<=12&&civil.day>=1&&civil.day<=31&&civil.hour>=0&&civil.hour<=23&&civil.minute>=0&&civil.minute<=59&&civil.second>=0&&civil.second<=59&&civil.millisecond>=0&&civil.millisecond<=999;
const partsFormatter=(zone:string)=>new Intl.DateTimeFormat("en-US",{timeZone:zone,hourCycle:"h23",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"});
const zonedParts=(zone:string,instant:number):Civil|undefined=>{
 try{
  ...
  return{year:get("year"),month:get("month"),day:get("day"),hour:get("hour"),minute:get("minute"),second:get("second"),millisecond:0};
...
const sameSecond=(left:Civil,right:Civil):boolean=>left.year===right.year&&left.month===right.month&&left.day===right.day&&left.hour===right.hour&&left.minute===right.minute&&left.second===right.second;
// Date.UTC maps years 0..99 to 19xx; civil years are literal.
const utcMillis=(civil:Civil):number=>{
 const guess=Date.UTC(civil.year,civil.month-1,civil.day,civil.hour,civil.minute,civil.second,civil.millisecond);
 if(civil.year>=0&&civil.year<=99)return new Date(guess).setUTCFullYear(civil.year);
 return guess;
};
export function createDateTimes(domain:ReturnType<typeof createDomainRuntime>,ids:{outOfRange:string;invalidZone:string;nonexistent:string;invalidOption:string}){
 const outOfRange=(millis:bigint)=>failure(domain.create(ids.outOfRange,record(ids.outOfRange,[["millis",millis]]),origin));
 const badZone=(zone:string)=>failure(domain.create(ids.invalidZone,record(ids.invalidZone,[["zone",zone]]),origin));
 const nonexistent=()=>failure(domain.create(ids.nonexistent,record(ids.nonexistent,[]),origin));
 const badOption=(reason:string)=>failure(domain.create(ids.invalidOption,record(ids.invalidOption,[["reason",reason]]),origin));
 return Object.freeze({
  async instantFromEpochMillis(millis:unknown,_context?:AssertionContext):Promise<Completion<object>>{
  ...
  async instantEpochMillis(handle:unknown,_context?:AssertionContext):Promise<Completion<bigint>>{
   return success(millisOf(handle));
  },
  async formatInZone(handle:unknown,locale:unknown,zone:unknown,dateStyle:unknown,timeStyle:unknown,_context?:AssertionContext):Promise<Completion<string>>{
   ...
   try{date=style(dateStyle,"date_style");}catch{return badOption("date_style");}
   try{time=style(timeStyle,"time_style");}catch{return badOption("time_style");}
   try{
    const formatter=new Intl.DateTimeFormat(locale,{timeZone:zone,dateStyle:date,timeStyle:time});
    return success(formatter.format(new Date(Number(millis))));
   }catch(cause){
    if(cause instanceof RangeError){
     try{new Intl.DateTimeFormat("en-US",{timeZone:zone});}catch{return badZone(zone);}
     return badOption("locale");
    }
    return caught(cause,origin);
   }
  },
  async resolveZonedTime(civil:unknown,zone:unknown,policy:unknown,_context?:AssertionContext):Promise<Completion<object>>{
   const parts=civilOf(civil);
   if(typeof zone!=="string")throw new TypeError("invalid compiler zone");
   if(typeof policy!=="bigint"||(policy!==0n&&policy!==1n))return badOption("policy");
   ...
   const pick=candidates.length===1?candidates[0]!:(policy===0n?candidates[0]!:candidates[candidates.length-1]!);
