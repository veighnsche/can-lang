...
export type FileErrorKind="not_found"|"denied"|"already_exists"|"not_empty"|"cross_device"|"unexpected_kind"|"not_directory"|"name_too_long"|"io_error";
const denied=new Set(["EACCES","EPERM","EROFS"]);
const ioCodes=new Set(["EIO","EBUSY","EFBIG","EINTR","EAGAIN","ENXIO","EBADF","ELOOP","EMFILE","ENFILE","ENOSPC","EDQUOT","ESTALE","ETXTBSY","EUCLEAN","ENOMEM","ENOTSUP"]);
...
export function classifyFileError(cause:unknown):{kind:FileErrorKind;code:string}|undefined{
...
export type EntryShape=Readonly<{isFile():boolean;isDirectory():boolean;isSymbolicLink():boolean}>;
