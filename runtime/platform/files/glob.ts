    ...
    async glob(
      ...
      try {
        ...
        for await (const match of scanner) {
          ...
          if (BigInt(collected.length) > maxEntries)
        ...
        switch (classified.kind) {
          case "not_found":
            return fail(types.notFound, [["path", base]], cause);
          case "denied":
            return fail(
              types.denied,
              [
          ...
          case "name_too_long":
            return fail(
              ...
              [
                ["path", base],
                ["reason", classified.kind === "not_directory" ? "not_directory" : "name_too_long"],
              ],
              cause,
            );
          default:
