...
export function createFileStreams(
  ...
  async function openReadable(path: string): Promise<Completion<undefined>> {
    try {
      ...
      return failureFor(cause, path, "open");
    }
  }
    ...
    async readStream(
      ...
      if (typeof maxChunk !== "bigint" || maxChunk < 1n)
        return fail(types.limitExceeded, [
          ["limit", typeof maxChunk === "bigint" ? maxChunk : -1n],
        ]);
      const ready = await openReadable(path);
      if (ready.kind !== "ok") return ready;
      return success(
        registerReader(openByteCell(Bun.file(path).stream(), maxChunk), fail, types.closeFailed),
      );
    },
    async readLinesStream(
      path: string,
      maxLine: bigint,
      context?: AssertionContext,
    ): Promise<Completion<object>> {
      ...
      if (typeof maxLine !== "bigint" || maxLine < 1n)
        return fail(types.limitExceeded, [["limit", typeof maxLine === "bigint" ? maxLine : -1n]]);
      const ready = await openReadable(path);
      if (ready.kind !== "ok") return ready;
      return success(
        registerReader(openLineCell(Bun.file(path).stream(), maxLine), fail, types.closeFailed),
      );
    },
