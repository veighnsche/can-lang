...
export function createFileWrites(domain: ReturnType<typeof createDomainRuntime>, types: Contracts) {
  ...
  async function missingSide(source: string, destination: string): Promise<string> {
    try {
      await lstat(dirname(destination));
    } catch (cause) {
  ...
  async function copyOrMove(
    ...
    if (kind === "move" && !overwrite) {
      try {
        ...
        return fail(types.alreadyExists, [["path", destination]]);
      } catch (cause) {
        if (classifyFileError(cause)?.kind !== "not_found")
          return failureFor(cause, destination, "move");
      }
    }
    if (kind === "copy") {
      ...
      try {
        if ((await stat(source)).isDirectory())
          return fail(types.unexpectedKind, [
            ["path", source],
            ["operation", "copy"],
          ]);
      } catch (cause) {
    ...
    try {
      if (kind === "copy")
        await copyFile(source, destination, overwrite ? undefined : constants.COPYFILE_EXCL);
      ...
      if (classified.kind === "not_found")
        return fail(types.notFound, [["path", await missingSide(source, destination)]], cause);
      if (classified.kind === "already_exists")
        return fail(types.alreadyExists, [["path", destination]], cause);
      if (classified.kind === "cross_device")
        return fail(
          types.crossDevice,
          [
            ["source", source],
  ...
  return Object.freeze({
    async writeBytes(
      ...
      try {
        ...
        return failureFor(cause, path, "write_bytes");
      }
    },
    async writeText(
      path: string,
      ...
      if (bad !== undefined) return bad;
      try {
        await writeFile(path, encoder.encode(value), { flag: overwrite ? "w" : "wx" });
        return success(undefined);
      } catch (cause) {
        return failureFor(cause, path, "write_text");
      }
    },
    async copy(
      source: string,
      ...
      denyLiveBoundary(context, origin);
      return copyOrMove("copy", source, destination, overwrite);
    },
    async move(
      source: string,
      destination: string,
      ...
      denyLiveBoundary(context, origin);
      return copyOrMove("move", source, destination, overwrite);
    },
  });
}
