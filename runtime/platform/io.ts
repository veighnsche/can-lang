...
const origin:FailureOrigin=Object.freeze({source:"can:io",start:0,end:0,invocation:Object.freeze([])});
const codes=new Set(["EPIPE","EBADF","EIO","ENOSPC","EACCES","EINVAL","EFBIG","EROFS","EINTR","EAGAIN","EISDIR","ENXIO"]);
export function expectedIOFailure(cause:unknown):boolean{
 ...
 return code!==undefined&&"value" in code&&typeof code.value==="string"&&codes.has(code.value);
}
type Contracts=Readonly<{readFailed:string;limit:string;invalidData:string}>;
export function createIO(domain:ReturnType<typeof createDomainRuntime>,types:Contracts,input:()=>ReadableStream<Uint8Array>=()=>Bun.stdin.stream()){
 ...
 async function read(limit:bigint,operation:string):Promise<Completion<Bytes>>{
  ...
  let complete=false;
  try{
   reader=input().getReader();const chunks:Uint8Array[]=[];let size=0n;
   for(;;){
  ...
  }catch(cause){if(!expectedIOFailure(cause))throw cause;return fail(types.readFailed,[["operation",operation]],cause);}
  finally{
   if(reader){
 ...
 return Object.freeze({
  async stdinBytes(limit:bigint,context?:AssertionContext):Promise<Completion<Bytes>>{denyLiveBoundary(context,origin);return read(limit,"stdin_bytes");},
  async stdinText(limit:bigint,context?:AssertionContext):Promise<Completion<string>>{
   denyLiveBoundary(context,origin);const result=await read(limit,"stdin_text");if(result.kind!=="ok")return result;
   const data=copyBytes(result.value,origin);
   try{return success(new TextDecoder("utf-8",{fatal:true,ignoreBOM:true}).decode(data));}
   catch(cause){if(!(cause instanceof TypeError))throw cause;return fail(types.invalidData,[["path",""],["reason","utf8"]]);}
  }
 });
}
