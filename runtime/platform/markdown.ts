...
const origin=Object.freeze({source:"can:markdown",start:0,end:0,invocation:Object.freeze([])});
...
type Attr=Readonly<{kind:"text";name:string;value:string}|{kind:"url";name:string;value:string}>;
...
export function createMarkdown(domain:ReturnType<typeof createDomainRuntime>,ids:Ids){
 const html=createHTML(domain,{structure:ids.htmlStructure,url:ids.htmlURL,target:"",interval:""});
 const fail=(identity:string,fields:readonly (readonly [string,unknown])[])=>failure(domain.create(identity,record(identity,fields),origin));
 const over=(limit:number,size:number)=>fail(ids.overLimit,[["limit",BigInt(limit)],["size",BigInt(size)]]);
 const invariant=(what:string):never=>{throw new TypeError(`unreachable markdown ${what}`);};
 return Object.freeze({
  async renderTextHTML(source:unknown,_context?:AssertionContext):Promise<Completion<string>>{
  ...
  async renderSafe(source:unknown,_context?:AssertionContext):Promise<Completion<unknown>>{
   ...
   const split=(children:string):(string|number)[]=>{
    ...
    for(let i=0;i<cells.length;i+=2){
     const text=cells[i]!;
     if(text!==""){if(text.includes("\0"))invariant("nul");out.push(text);}
     if(i+1<cells.length){
   ...
   const emit=(tag:string,attrs:readonly Attr[],children:string):string=>{
    if(capped)return "";
    if(trees.length>=maxNodes){capped=true;return "";}
   ...
   const textOf=(id:number):string=>{
    ...
    while(stack.length>0){
     ...
     for(let i=parts.length-1;i>=0;i--){const part=parts[i]!;if(typeof part==="string")out+=part;else stack.push(part);}
    }
    return out;
   };
   ...
   const cell=(tag:string,children:string,align:unknown):string=>{
    if(align!==undefined&&(typeof align!=="string"||!aligns.has(align.toLowerCase())))invariant("align");
    const attrs:Attr[]=typeof align==="string"?[{kind:"text",name:"align",value:align}]:[];
   ...
   const callbacks:NativeCallbacks={
    ...
    heading:(children,meta)=>{
     if(!Number.isInteger(meta.level)||meta.level<1||meta.level>6)invariant("heading");
     const id=meta.id;
    ...
    code:(children,meta)=>{
     ...
     const attrs:Attr[]=language!==undefined&&language!==""&&languageClass.test(language)?[{kind:"text",name:"class",value:`language-${language}`}]:[];
    ...
    image:(children,meta)=>{
     ...
     const attrs:Attr[]=[{kind:"url",name:"src",value:meta.src},{kind:"text",name:"alt",value:altText(children)}];
     if(title!==undefined)attrs.push({kind:"text",name:"title",value:title});
    ...
    list:(children,meta)=>{
     ...
     const attrs:Attr[]=typeof start==="number"&&Number.isSafeInteger(start)&&start>=0&&start!==1?[{kind:"text",name:"start",value:String(start)}]:[];
    ...
    table:children=>{
     ...
     for(const part of parts){
      if(typeof part==="string")invariant("table-text");
       ...
       if(tag==="thead"){if(head!=="")invariant("table");head=tokenOf(part);}
       else if(tag==="tbody"){if(body!=="")invariant("table");body=tokenOf(part);}
   ...
   const rendered=Bun.markdown.render(source,callbacks,{tables:true,strikethrough:true,tasklists:true,headings:{ids:true},noHtmlBlocks:true,noHtmlSpans:true,wikiLinks:false,underline:false,latexMath:false,autolinks:false});
