...
export class MultipartIssue extends Error {
  constructor(readonly reason: "multipart_boundary" | "multipart_frame" | "multipart_nested") {
    super("invalid multipart");
  }
}
...
export type MultipartDocument = Readonly<{
...
function asciiLine(line: Uint8Array): string {
  try {
    ...
    throw new MultipartIssue("multipart_frame");
  }
}
function unquote(value: string): string {
  ...
  for (let i = 1; i + 1 < value.length; i++) {
    ...
    if (char === "\\") {
      i++;
      if (i + 1 >= value.length) throw new MultipartIssue("multipart_frame");
      out += value[i]!;
    } else {
      if (char === "\r" || char === "\n") throw new MultipartIssue("multipart_frame");
      out += char;
...
function dispositionOf(header: string): Readonly<{ name: string; filename: string | undefined }> {
  ...
  for (const part of parts.slice(1)) {
    ...
    const key = part.slice(0, cut).trim().toLowerCase(),
      value = unquote(part.slice(cut + 1).trim());
    if (key === "name") {
      if (name !== undefined) throw new MultipartIssue("multipart_frame");
      name = value;
    } else if (key === "filename") {
      if (filename !== undefined) throw new MultipartIssue("multipart_frame");
      filename = value;
    }
  }
  if (name === undefined) throw new MultipartIssue("multipart_frame");
...
export function decodeMultipart(input: unknown, boundary: string): MultipartDocument {
  if (!/^[A-Za-z0-9'()+_,\-./:=?]+$/.test(boundary) || boundary.length > 70)
    throw new MultipartIssue("multipart_boundary");
  const body = new Uint8Array(copyBytes(input, origin));
  const lines = linesOf(body);
  const delimiter = encode.encode("--" + boundary),
    close = encode.encode("--" + boundary + "--");
  const same = (line: Uint8Array, marker: Uint8Array) =>
    line.byteLength === marker.byteLength && line.every((value, at) => value === marker[at]);
  const closedBy = (line: Uint8Array) =>
    same(line, close) ||
    (line.byteLength > close.byteLength &&
      close.every((value, at) => value === line[at]) &&
      line.subarray(close.byteLength).every((value) => value === 32 || value === 9));
  ...
  for (;;) {
    ...
    for (const header of headerLines) {
      ...
      if (cut <= 0) throw new MultipartIssue("multipart_frame");
      const key = header.slice(0, cut).trim().toLowerCase(),
        value = header.slice(cut + 1).trim();
      if (key === "content-disposition") {
        if (disposition !== undefined) throw new MultipartIssue("multipart_frame");
        disposition = value;
      } else if (key === "content-type") {
        if (contentType !== undefined) throw new MultipartIssue("multipart_frame");
        contentType = value;
      }
    }
    if (disposition === undefined) throw new MultipartIssue("multipart_frame");
    if (contentType !== undefined && contentType.toLowerCase().startsWith("multipart/"))
      throw new MultipartIssue("multipart_nested");
    ...
    if (contentType !== undefined) {
      try {
        ...
        if (cause instanceof CodecIssue) throw new MultipartIssue("multipart_frame");
        throw cause;
      }
    }
    ...
    const size =
      content.reduce((sum, line) => sum + line.byteLength, 0) +
      (content.length === 0 ? 0 : (content.length - 1) * 2);
    ...
    if (filename === undefined) {
      let value: string;
      try {
        value = text.decode(joined);
      } catch {
        throw new CodecIssue("multipart", "utf8");
      }
