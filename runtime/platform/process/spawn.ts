...
export function createProcesses(domain: ReturnType<typeof createDomainRuntime>, types: Contracts) {
  ...
  function configure(
    ...
    if (deadlineMs < 0n || graceMs < 0n)
      return invalid(deadlineMs < 0n ? "deadline_ms" : "grace_ms", "negative");
    if (deadlineMs > BigInt(maxDurationMs) || graceMs > BigInt(maxDurationMs))
      return invalid(deadlineMs > BigInt(maxDurationMs) ? "deadline_ms" : "grace_ms", "too_large");
    ...
    if (classified.kind === "not_found") {
      if (cwd !== "")
        try {
          ...
          if (classifySpawnError(nested)?.kind === "not_found")
            return fail(types.notFound, [["path", cwd]], cause);
        }
  ...
  type Settle = Readonly<
    | { kind: "exited" }
    | { kind: "timeout" }
    | { kind: "stream_failed"; stream: string }
    | { kind: "overflow"; stream: string; limit: bigint }
  >;
  async function execute(executable: string, config: RunConfig): Promise<Completion<unknown>> {
    let proc: ChildProcess;
    try {
      proc = Bun.spawn([executable, ...config.argv], {
        stdin: "pipe",
        stdout: "pipe",
        stderr: "pipe",
      ...
      return spawnFailure(cause, executable, config.cwd);
    }
    let settled = false;
      ...
      async (): Promise<Completion<void>> => {
        ...
        try {
          ...
          return failure(
            domain.create(
              types.ioError,
              record(types.ioError, [["operation", "kill"]]),
              origin,
              cause,
            ),
          );
    ...
    try {
      try {
        deliverStdin(proc, config.stdin);
      } catch (cause) {
        await terminateGroup(proc.pid, Number(config.graceMs), isExited);
        try {
          await proc.exited;
      ...
      const decided = new Promise<Settle>((resolve) => {
        decide = resolve;
      });
      let done = false;
      const finish = (settle: Settle) => {
        if (!done) {
          done = true;
          decide(settle);
        }
      ...
      if (config.deadlineMs > 0n)
        timer = setTimeout(() => finish({ kind: "timeout" }), Number(config.deadlineMs));
      stdoutDrain.then(
        (outcome) => {
          if (outcome.overflow)
            finish({ kind: "overflow", stream: "stdout", limit: config.stdoutLimit });
        },
        () => finish({ kind: "stream_failed", stream: "stdout" }),
      );
      stderrDrain.then(
        (outcome) => {
          if (outcome.overflow)
            finish({ kind: "overflow", stream: "stderr", limit: config.stderrLimit });
        },
        () => finish({ kind: "stream_failed", stream: "stderr" }),
      );
      try {
        ...
        if (outcome.kind === "stream_failed")
          return fail(types.ioError, [["operation", outcome.stream]]);
        const drains: readonly [DrainOutcome, DrainOutcome] = await Promise.all([
          stdoutDrain,
          stderrDrain,
        ]);
        if (outcome.kind === "timeout")
          return fail(types.timeout, [["deadline_ms", config.deadlineMs]]);
        if (outcome.kind === "overflow" || drains[0].overflow)
          return fail(types.outputLimit, [
            ["stream", "stdout"],
            ["limit", config.stdoutLimit],
          ]);
        if (drains[1].overflow)
          return fail(types.outputLimit, [
            ["stream", "stderr"],
            ["limit", config.stderrLimit],
          ]);
        if (proc.exitCode === null && proc.signalCode === null)
          return fail(types.ioError, [["operation", "wait"]]);
      ...
      try {
        await closeResource(token, "process");
      } catch {}
    }
  }
  return Object.freeze({
    async run(
      ...
      if (typeof (config as Completion<never>).kind === "string")
        return config as Completion<never>;
    ...
    async requireSuccess(
      ...
      const code = dataProperty(value, "code") as bigint,
        signal = dataProperty(value, "signal") as string;
      if (code === 0n && signal === "") return success(value);
      return fail(types.nonzero, [
        ["code", code],
        ["signal", signal],
      ]);
    },
