...
import {registerReader,useReader,type Fail,type ReaderCell} from "../transport/stream/lifecycle.ts";
...
export function createS3(domain:ReturnType<typeof createDomainRuntime>,ids:Ids){
  ...
  const service=(code:string,operation:string)=>fail(ids.service,[["code",code],["operation",operation]]);
  const closed=(operation:string,state:string)=>fail(ids.closed,[["operation",operation],["state",state]]);
  const overLimit=(limit:bigint,size:bigint)=>fail(ids.overLimit,[["limit",limit],["size",size]]);
  ...
  const contentType=(value:unknown):string|undefined|Completion<never>=>{
    ...
    if(typeof picked!=="string"||picked===""||/[\x00-\x1F\x7F]/.test(picked))return invalid("content_type");
    return picked;
  };
  const metadataOf=(stat:{size:number;etag:string;type:string;lastModified:Date}):unknown=>
    record(ids.metadata,[["size",BigInt(stat.size)],["etag",stat.etag],["content_type",stat.type],["last_modified",mintTimeInstant(BigInt(stat.lastModified.getTime()))]]);
  const statOf=async(client:NativeClient,key:string):Promise<unknown>=>metadataOf(await client.file(key).stat());
  ...
  return Object.freeze({
    async clientOpen(endpoint:unknown,region:unknown,bucket:unknown,accessKey:unknown,secretKey:unknown,_context?:AssertionContext):Promise<Completion<object>>{
      if(typeof endpoint!=="string"||typeof region!=="string"||typeof bucket!=="string"||typeof accessKey!=="string"||typeof secretKey!=="string")throw new TypeError("invalid compiler s3 client");
      try{
      ...
      try{
        return success(mintClient(new Bun.S3Client({endpoint,region,bucket,accessKeyId:accessKey,secretAccessKey:secretKey})));
      }catch(cause){return caught(cause,origin);}
    },
    async readBytes(handle:unknown,key:unknown,maxBytes:unknown,context?:AssertionContext):Promise<Completion<unknown>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      if(typeof maxBytes!=="bigint"||maxBytes<1n||maxBytes>READ_CAP)return invalid("limit");
      ...
      }catch(cause){return wire("read_bytes",checked,cause);}
    },
    async readRange(handle:unknown,key:unknown,offset:unknown,length:unknown,context?:AssertionContext):Promise<Completion<unknown>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      ...
      }catch(cause){return wire("read_range",checked,cause);}
    },
    async readStream(handle:unknown,key:unknown,maxBytes:unknown,context?:AssertionContext):Promise<Completion<object>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      ...
      try{
        ...
        let seen=0n;
        const counted=(file.stream() as ReadableStream<Uint8Array>).pipeThrough(new TransformStream<Uint8Array,Uint8Array>({
          transform(chunk,controller){
            seen+=BigInt(chunk.byteLength);
            if(seen>maxBytes){controller.error(Object.assign(new Error("s3 over byte budget"),{code:"over_limit"}));return;}
            controller.enqueue(chunk);
          },
        }));
        return success(registerReader(openByteCell(counted,STREAM_CHUNK),fail,ids.closeFailed,{scopeManaged:true}));
      }catch(cause){return wire("read_stream",checked,cause);}
    },
    async writeBytes(handle:unknown,key:unknown,body:unknown,options:unknown,context?:AssertionContext):Promise<Completion<unknown>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      ...
      try{
        await client.file(checked).write(copyBytes(body,origin),type===undefined?undefined:{type});
        return success(await statOf(client,checked));
      }catch(cause){return wire("write_bytes",checked,cause);}
    },
    async writeStream(handle:unknown,key:unknown,reader:unknown,options:unknown,maxBytes:unknown,deadlineMs:unknown,context?:AssertionContext):Promise<Completion<unknown>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
    ...
    async stat(handle:unknown,key:unknown,context?:AssertionContext):Promise<Completion<unknown>>{
      ...
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      try{return success(await statOf(client,checked));}
      catch(cause){return wire("stat",checked,cause);}
    },
    async exists(handle:unknown,key:unknown,context?:AssertionContext):Promise<Completion<boolean>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      try{return success(await client.file(checked).exists());}
      catch(cause){
    ...
    async remove(handle:unknown,key:unknown,context?:AssertionContext):Promise<Completion<void>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      try{await client.file(checked).delete();return success(undefined);}
      catch(cause){return wire("delete",checked,cause);}
    },
    async list(handle:unknown,options:unknown,context?:AssertionContext):Promise<Completion<unknown>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      ...
      try{
        const response:NativeList=await client.list({prefix,maxKeys:Number(limit),...(delimiterPick===undefined?{}:{delimiter:delimiterPick}),...(continuation===undefined?{}:{continuationToken:continuation})});
        const entries:unknown[]=[];
        for(const item of response.contents??[]){
          // Size, etag and timestamp always accompany real entries;
          // anything else is a malformed service response, not data.
          if(typeof item.key!=="string"||typeof item.size!=="number"||typeof item.eTag!=="string"||typeof item.lastModified!=="string")return service("malformed_listing","list");
          const millis=Date.parse(item.lastModified);
          if(Number.isNaN(millis))return service("malformed_listing","list");
          entries.push(record(ids.entry,[["key",item.key],["size",BigInt(item.size)],["etag",item.eTag],["last_modified",mintTimeInstant(BigInt(millis))]]));
        }
    ...
    async presign(handle:unknown,method:unknown,key:unknown,expiresIn:unknown,contentTypePick:unknown,context?:AssertionContext):Promise<Completion<object>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const verb=methodOf(method);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
      if(typeof expiresIn!=="bigint"||expiresIn<1n||expiresIn>EXPIRES_MAX)return invalid("expires");
      const type=contentType(contentTypePick);
      if(typeof type!=="string"&&type!==undefined)return type;
      try{
        const url=client.presign(checked,{method:verb,expiresIn:Number(expiresIn),...(type===undefined?{}:{type})});
        const token=Object.freeze(Object.create(null));
        presigned.set(token,{url,method:verb,expires:mintTimeInstant(BigInt(Date.now())+expiresIn*1000n)});
    ...
    async describe(token:unknown,_context?:AssertionContext):Promise<Completion<unknown>>{
      ...
      return success(record(ids.info,[["url",box.url],["method",record(methodLeaf(box.method),[])],["expires_at",box.expires]]));
    },
    async beginUpload(handle:unknown,key:unknown,options:unknown,context?:AssertionContext):Promise<Completion<object>>{
      denyLiveBoundary(context,origin);
      const client=projectClient(handle);
      const checked=keyOf(key);
      if(typeof checked!=="string")return checked;
    ...
    async uploadWrite(token:unknown,chunk:unknown,context?:AssertionContext):Promise<Completion<bigint>>{
      denyLiveBoundary(context,origin);
      ...
      }catch(cause){return wire("upload_write",box.key,cause);}
    },
