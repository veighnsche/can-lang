...
): { ok: true; max: number } | { ok: false; failure: Completion<never> } {
  if (typeof value !== "bigint") throw new TypeError("invalid compiler sql config");
  if (value < 1n || value > 2147483647n)
...
export function sqliteFileConfig(
  path: unknown,
  options: unknown,
  failures: SQLFailures,
): { ok: true; config: SQLiteFileConfig } | { ok: false; failure: Completion<never> } {
  ...
  if (timeout < 0n || timeout > 2147483647n)
    return { ok: false, failure: failures.connectionFailed("config") };
