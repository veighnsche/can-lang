...
export function isPostgresFailure(value: unknown): value is Error & { code: string; errno?: unknown; constraint?: unknown } {
  return value instanceof Error && value.name === "PostgresError" && typeof (value as { code?: unknown }).code === "string";
}

export function classifyPostgres(operation: string, cause: unknown, failures: SQLFailures, contracts: SQLCoreContracts): Completion<never> {
  ...
  if (constraint !== "" || (errno.length === 5 && errno.startsWith("23"))) {
    return failures.fail(contracts.constraintFailed, [["constraint", constraint !== "" ? constraint : errno]]);
  }
  return failures.queryFailed(operation, errno !== "" ? errno : cause.code !== "" ? cause.code : "unknown");
}

export function postgresAffectedRows(operation: string, result: unknown, failures: SQLFailures): Completion<bigint> {
...
export async function openPostgresClient(url: string, max: number, failures: SQLFailures): Promise<{ ok: true; client: InstanceType<typeof Bun.SQL> } | { ok: false; failure: Completion<never> }> {
