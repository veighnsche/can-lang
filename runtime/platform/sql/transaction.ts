...
class TxPrimary {
  ...
  constructor(completion: Completion<never>) {
...
const rollbackStrings = Object.freeze(
  Object.assign([rollbackText], { raw: Object.freeze([rollbackText]) }),
...
export function createSQLTransactions(
  ...
  const transactionFailed = (phase: string) =>
    fail(contracts.transactionFailed, [["phase", phase]]);
  ...
  function classifyBegin(
    phase: "begin" | "callback",
    cause: unknown,
    dialect: SQLDialect,
  ): Completion<never> {
    if (dialect === "sqlite") {
