...
// decoding, and native classification are the shared I35 core, never a
...
import { createSQLOperations, type SQLCoreContracts, type SQLNative, type SQLPlan } from "./sql.ts";
...
type Contracts = SQLCoreContracts & Readonly<{ transactionFailed: string; commitUnknown: string }>;
...
function isPostgresFailure(value: unknown): value is Error & { code: string } {
...
export function createSQLTransactions(
  ...
  function classifyBegin(phase: "begin" | "callback", cause: unknown): Completion<never> {
    ...
    if (cause.code === "ERR_POSTGRES_CONNECTION_REFUSED" || cause.code === "ERR_POSTGRES_CONNECTION_CLOSED") {
      return fail(contracts.connectionFailed, [["phase", phase]]);
    }
