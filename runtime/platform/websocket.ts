...
const origin = Object.freeze({
  source: "can:websocket",
  start: 0,
  end: 0,
  invocation: Object.freeze([]),
});
export const SESSION_KIND = "ws-session";
const MESSAGE_CAP = 67108864n,
  QUEUE_CAP = 1024n,
  SEND_CAP = 67108864n,
  DEADLINE_CAP = 2147483647n;
type EventIds = Readonly<{
  text: string;
  binary: string;
  drain: string;
  close: string;
...
}> & { socket: ClientSocket | ServerSocket | undefined; server?: object; cleanClose: boolean };
...
const sessions = new Map<number, Session>(),
  serverSessions = new Map<object, Set<number>>();
function unregister(session: Session): void {
  ...
  if (owned !== undefined) {
    ...
    if (owned.size === 0 && session.server !== undefined) serverSessions.delete(session.server);
  }
}
...
function createPump(maxQueued: number): Readonly<{
  pump: EventPump;
  push: (value: unknown) => void;
  pushDrain: (value: unknown) => void;
  pushClose: (value: unknown) => void;
  ...
  const queue: QueueEntry[] = [],
    waiters: ((take: EventTake) => void)[] = [];
  let terminal: EventTake | undefined,
    interrupted = false,
    disposed = false,
    drainPending = false;
  const wake = (take: EventTake): void => {
    let waiter;
    while ((waiter = waiters.shift()) !== undefined) waiter(take);
  };
  const settle = (take: Extract<EventTake, { kind: "end" } | { kind: "failed" }>): void => {
    if (terminal !== undefined) return;
    terminal = take;
    if (take.kind === "failed") queue.length = 0;
    wake(take);
  };
  return {
    pump: Object.freeze({
      take: (): Promise<EventTake> => {
        ...
        if (entry !== undefined) {
          if (entry.drain) drainPending = false;
          return Promise.resolve({ kind: "event", value: entry.value });
        }
        ...
        return new Promise<EventTake>((resolve) => {
          waiters.push(resolve);
        });
      },
      interrupt: (): void => {
        interrupted = true;
        wake({ kind: "interrupted" });
      },
      dispose: (): void => {
        disposed = true;
        queue.length = 0;
    ...
    push: (value: unknown): void => {
      ...
      if (waiter !== undefined) {
        waiter({ kind: "event", value });
        return;
      }
      if (queue.length >= maxQueued) {
        settle({ kind: "failed", reason: "queue_overrun" });
        return;
      }
    ...
    fail: (reason: string): void => {
      if (!disposed) settle({ kind: "failed", reason });
    },
  };
}
function utf8Bytes(text: string): bigint {
  return BigInt(new TextEncoder().encode(text).byteLength);
}
const tokenPattern = /^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/;
function validProtocol(value: unknown): value is string {
  return typeof value === "string" && value !== "" && tokenPattern.test(value);
}
function validCode(value: bigint): boolean {
  if (value < 1000n || value > 4999n) return false;
  if (value >= 1000n && value <= 1014n)
    return value !== 1004n && value !== 1005n && value !== 1006n;
  return value >= 3000n;
}
function messageIn(session: Session, message: unknown): void {
  if (typeof message === "string") {
    if (utf8Bytes(message) > session.maxMessage) {
      session.fail("message_too_large");
      return;
    }
  ...
  const bytes =
    message instanceof ArrayBuffer
      ...
      : ArrayBuffer.isView(message)
        ...
        : undefined;
  if (bytes === undefined) {
    session.fail("message_type");
    return;
  }
  if (BigInt(bytes.byteLength) > session.maxMessage) {
    session.fail("message_too_large");
    return;
  }
...
export function serverSocketOpen(socket: unknown): void {
  try {
    const session = lookup(socket);
    if (session !== undefined) session.socket = socket as ServerSocket;
  } catch {}
}
export function serverSocketMessage(socket: unknown, message: unknown): void {
  try {
    const session = lookup(socket);
    if (session !== undefined) messageIn(session, message);
  } catch {}
}
export function serverSocketClose(socket: unknown, code: unknown, reason: unknown): void {
  try {
    ...
    unregister(session);
    closeIn(session, code, reason);
  } catch {}
}
export function serverSocketDrain(socket: unknown): void {
  try {
    const session = lookup(socket);
    if (session !== undefined) session.pushDrain(record(session.ids.drain, []));
  } catch {}
}
...
export function closeServerSessions(server: object): void {
  ...
  // unregister mutates owned; snapshot the sessions before closing them.
...
export function isWebSocketValue(kind: string | undefined, value: unknown): boolean {
  ...
  try {
    return resourceStatus(value).kind === SESSION_KIND;
  } catch {
    return false;
  }
}
...
export function createWebSockets(domain: ReturnType<typeof createDomainRuntime>, types: Types) {
  ...
  function checkCaps(
    ...
    maxQueued: unknown,
    maxSend: unknown,
  ): Readonly<{ maxMessage: bigint; maxQueued: bigint; maxSend: bigint } | Completion<never>> {
    if (typeof maxMessage !== "bigint" || maxMessage < 1n || maxMessage > MESSAGE_CAP)
      return overLimit(typeof maxMessage === "bigint" ? maxMessage : -1n);
    if (typeof maxQueued !== "bigint" || maxQueued < 1n || maxQueued > QUEUE_CAP)
      return overLimit(typeof maxQueued === "bigint" ? maxQueued : -1n);
    if (typeof maxSend !== "bigint" || maxSend < 1n || maxSend > SEND_CAP)
      return overLimit(typeof maxSend === "bigint" ? maxSend : -1n);
    return { maxMessage, maxQueued, maxSend };
  }
  function register(
    caps: Readonly<{ maxMessage: bigint; maxQueued: bigint; maxSend: bigint }>,
    kind: "client" | "server",
    server?: object,
  ): Readonly<{ session: Session; sessionToken: object; readerToken: object }> {
    const pump = createPump(Number(caps.maxQueued));
    const session: Session = {
      id: ++nextSession,
      kind,
      maxMessage: caps.maxMessage,
      maxSend: caps.maxSend,
      ids: types,
      socket: undefined,
      server,
      cleanClose: false,
      push: pump.push,
      pushDrain: pump.pushDrain,
      pushClose: pump.pushClose,
      fail: pump.fail,
    };
    sessions.set(session.id, session);
    if (server !== undefined) {
      let owned = serverSessions.get(server);
      if (owned === undefined) {
        owned = new Set();
        serverSessions.set(server, owned);
      }
      owned.add(session.id);
    }
    const readerToken = registerReader(
      openEventCell(pump.pump),
      (identity, fields, cause) => fail(identity, fields, cause),
      types.closeFailed,
      { scopeManaged: true },
    );
    const sessionToken = registerResource(
      SESSION_KIND,
      session,
      async (): Promise<Completion<void>> => {
        ...
        const clean = session.cleanClose;
        session.cleanClose = false;
        try {
  ...
  async function teardown(
    request: unknown,
    held: Readonly<{ session: Session; sessionToken: object; readerToken: object }>,
  ): Promise<void> {
    try {
      await closeResource(held.readerToken, "stream-reader");
    } catch {}
    try {
      await closeResource(held.sessionToken, SESSION_KIND);
    } catch {}
  ...
  function sendOn(
    ...
    if (socket === undefined || socket.readyState !== 1)
      return fail(types.sendFailed, [["reason", "closed"]]);
    if (
      session.kind === "client" &&
      (socket as ClientSocket).bufferedAmount + Number(bytes) > Number(session.maxSend)
    )
      return fail(types.sendFailed, [["reason", "blocked"]]);
    try {
  ...
  return Object.freeze({
    async connect(
      ...
      try {
        ...
        return fail(types.invalidUrl, [["reason", "unparseable"]]);
      }
      if (parsed.protocol !== "ws:" && parsed.protocol !== "wss:")
        return fail(types.invalidUrl, [["reason", "scheme"]]);
      const offered: string[] = [];
      for (const entry of dataArray(protocols)) {
        if (!validProtocol(entry))
          return fail(types.invalidProtocol, [
            ["protocol", typeof entry === "string" ? entry : ""],
          ]);
        offered.push(entry);
      }
      const caps = checkCaps(maxMessage, maxQueued, maxSend);
      if ("kind" in caps) return caps;
      if (typeof deadlineMs !== "bigint" || deadlineMs < 1n || deadlineMs > DEADLINE_CAP)
        return overLimit(typeof deadlineMs === "bigint" ? deadlineMs : -1n);
      ...
      try {
        if (insecure)
          socket = new WebSocket(parsed.href, {
            ...(offered.length === 0 ? {} : { protocols: offered }),
            tls: { rejectUnauthorized: false },
          } as unknown as string[]);
        else
          socket =
            offered.length === 0 ? new WebSocket(parsed.href) : new WebSocket(parsed.href, offered);
      } catch {
        return fail(types.invalidUrl, [["reason", "unparseable"]]);
      }
      try {
        ...
        return fail(types.connectFailed, [["reason", "unreachable"]]);
      }
      const opened = await new Promise<
        Readonly<{ kind: "open" } | { kind: "failed"; reason: string }>
      >((resolve) => {
      ...
      socket.onmessage = (event: MessageEvent) => {
        try {
          messageIn(held.session, event.data);
        } catch {}
      };
      socket.onclose = (event: CloseEvent) => {
        try {
          closeIn(held.session, event.code, event.reason);
        } catch {}
      };
      socket.onerror = () => {};
      return success(
        record(types.connection, [
          ["session", held.sessionToken],
          ["events", held.readerToken],
    ...
    async accept(
      ...
      if (protocol !== "" && !validProtocol(protocol))
        return fail(types.invalidProtocol, [["protocol", protocol]]);
      if (protocol !== "" && !offeredProtocols(request).includes(protocol))
        return fail(types.unsupportedProtocol, [["protocol", protocol]]);
      const claimed = claimUpgrade(request);
      if (claimed === undefined) throw resourceStateFailure(undefined, origin);
      if (claimed === "upgraded")
        return fail(types.upgradeFailed, [["reason", "already_upgraded"]]);
      const held = register(caps, "server", claimed.server as object);
      let upgraded = false;
      try {
        upgraded = claimed.server.upgrade(
          claimed.native,
          protocol === ""
            ? { data: { session: held.session.id } }
            : {
      ...
      if (!upgraded) {
        await teardown(request, held);
        return fail(types.upgradeFailed, [["reason", "refused"]]);
      }
      return success(
        record(types.connection, [
          ["session", held.sessionToken],
    ...
    async close(
      ...
      denyLiveBoundary(context, origin);
      if (typeof code !== "bigint" || typeof reason !== "string")
        throw new TypeError("invalid compiler close");
      if (!validCode(code)) return fail(types.invalidClose, [["reason", "code"]]);
      if (!reason.isWellFormed() || utf8Bytes(reason) > 123n)
        return fail(types.invalidClose, [["reason", "reason"]]);
      ...
      await useResource(session, SESSION_KIND, async (native) => {
        const current = native as Session;
        try {
          current.socket?.close(Number(code), reason);
        } catch {}
