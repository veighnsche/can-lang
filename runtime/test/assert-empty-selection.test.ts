...
for (const inner of ["all", "settled", "any"] as Mode[])
  test(`empty ${inner} selection preserves lexical fixture order`, async () => {
    const events: string[] = [];
      ...
      actual: async (ctx) => {
        await settle(
          ...
          [0, 1].map((i) => ({
            ...
            run: async (child) => {
              if (i === 0) {
                await settle(inner, [], child, "p::empty#0", []);
                events.push("empty selected");
              }
              return callContext(child, "p::leaf#0", (leaf) =>
                withFixture(leaf, "table", rows, [i], () => success(-1), origin),
              );
            },
          })),
          ctx,
