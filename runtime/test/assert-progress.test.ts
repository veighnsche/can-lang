...
test("progress sink observes running barrier snapshots",async()=>{
 ...
 const report=await runAssertion({root,expected:async()=>success(1),actual:async context=>{
...
test("throwing progress sink never fails the root",async()=>{
 const report=await runAssertion({root,expected:async()=>success(1),actual:async()=>{
  await Bun.sleep(300);
  return success(1);
...
test("omitted sink keeps legacy report shape",async()=>{
 const report=await runAssertion({root,expected:async()=>success(1),actual:async()=>success(1)});
 expect(report.passed).toBe(true);
