...
test("supplied completion evidence cannot promote itself to provider or target coverage", () => {
  ...
  recordEvidence(value, "real-can");
  recordEvidence(value, "supplied-completion");
  recordEvidence(value, "supplied-completion");
  ...
  expect(evidenceSummary([value])).toEqual({
    "real-can": 1,
...
test("release evidence retains distinct categories and immutable snapshots", () => {
  const authored = createEvidence("assertion"),
    native = createEvidence("bun-conformance"),
    live = createEvidence("live-quality");
  recordEvidence(authored, "real-can");
  const before = evidenceReport(authored);
  recordEvidence(authored, "raw-provider-fixture");
  recordEvidence(native, "bun-conformance");
  recordEvidence(live, "live-quality");
  expect(before).toEqual(["real-can"]);
  expect(Object.isFrozen(before)).toBe(true);
  const report = evidenceSummary([authored, native, live]);
  expect(report).toEqual({
    "real-can": 1,
    "supplied-completion": 0,
    "raw-provider-fixture": 1,
    "policy-fixture": 0,
    "bun-conformance": 1,
    "live-quality": 1,
  });
...
test("evidence tokens and categories fail closed", () => {
  ...
  expect(() => recordEvidence(createEvidence("assertion"), "invented" as EvidenceLabel)).toThrow(
