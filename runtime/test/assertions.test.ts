...
const root = Object.freeze({
  ...
  declaration: "can.project.root/app::subject",
  name: "sample",
});
...
test("an assertion awaits and executes its real body exactly once", async () => {
  ...
  const pending = runAssertion({
    root,
    expected: async () => success(49n),
    actual: async () => {
      calls++;
      await gate;
      return success(7n * 7n);
    },
  });
  ...
  expect(
    await runAssertion({
      root,
      expected: async () => success(50n),
      actual: async () => success(7n * 7n),
    }),
  ).toMatchObject({ passed: false, reason: "outcome mismatch" });
});
...
test("caught live-boundary refusal is sticky and independent roots do not share it", async () => {
  const results = await Promise.all([
    ...
    runAssertion({
      root: { ...root, declaration: root.declaration + "_other" },
      expected: async () => success(undefined),
      actual: async () => {
        await Promise.resolve();
        return success(undefined);
...
test("basic fixture FIFO never searches later rows and reports leftovers", async () => {
  ...
  await expect(
    withFixture(context, "table", rows, [2n], async () => success(0n), origin),
  ).rejects.toBeDefined();
...
test("mismatch reports identify root, site and invocations without dumping values", async () => {
  const secret = "capture-secret-Zed",
    other = "leftover-Hank",
    actual = "actual-Quinn";
  const sited = {
    source: "can:test",
    ...
    invocation: ["can.project.root/app::subject"],
  };
  const context = assertionContext(root);
  const rows = [
    {
  ...
  await expect(
    withFixture(context, "table", rows, [actual], async () => success(0n), sited),
  ).rejects.toBeDefined();
  closeContext(context);
  ...
  expect(mismatch.actual.root).toEqual({
  ...
  expect(report.fixturePaths).toEqual([
    {
      ...
      expected: null,
      actual: report.fixturePaths[0].actual,
      origin: null,
    },
  ]);
...
test("pending invocation progress reports structure without values", async () => {
  const { coordinationContexts, scheduledFixture, contextProgress } =
    await import("../assert/context.ts");
  const context = assertionContext(root);
  const coord = coordinationContexts(context, "p::main#0", [[0], [1]], "all");
  coord.start(0);
  let delivered = false;
  const pending = scheduledFixture(coord.contexts[0], "table", 1, origin, async () => {
    delivered = true;
    return success(1n);
  });
