...
const html=createHTML(domain,{structure:errors[0]!.identity,url:urlID,target:"unused",interval:"unused"},[]);
const root=mkdtempSync(join(tmpdir(),"can-assets-"));
const script=await Bun.file(new URL("../../distribution/assets/htmx-4.0.0.min.js",import.meta.url)).bytes();
const css=new TextEncoder().encode("body{color:black}\n");
const digest=(bytes:Uint8Array)=>createHash("sha256").update(bytes).digest("hex");
const htmxDigest=digest(script),cssDigest=digest(css);
...
const table:AssetTable={
 htmx:{route:"/__can/assets/htmx-4.0.0.min.js",digest:htmxDigest,mediaType:"text/javascript",file:`assets/${htmxDigest}/htmx-4.0.0.min.js`,integrity:"sha384-BvJpBiO8Kh31EqtJe5DRIeWrHWnCGkwytKs9NKFi86Hhw96dEqdEMzZDeK9iEGTc"},
 project:[{route:cssRoute,digest:cssDigest,mediaType:"text/css; charset=utf-8",file:`assets/${cssDigest}/site.css`,integrity:""}]
};
const assets=createAssets(table,pathToFileURL(root+"/"));
const ask=(path:string,headers?:Record<string,string>,method="GET")=>assets.serve(new Request("http://127.0.0.1"+path,{method,headers}));
...
test("pinned htmx and project assets are immutable exact-path responses",async()=>{
 ...
 const cached=await ask("/__can/assets/htmx-4.0.0.min.js",{"if-none-match":`"${htmxDigest}"`});
...
test("path escapes, unknown routes, wrong bytes and foreign methods are refused",async()=>{
 for (const path of [`/__can/project/${cssDigest}/../${htmxDigest}/htmx-4.0.0.min.js`,`/__can/project/${htmxDigest}/htmx-4.0.0.min.js`,`/__can/assets/`,`/__can/assets/htmx-4.0.0.min.js.map`,`/__can`]) {
...
test("declared asset urls mint html provenance and foreign names stay reasons",async()=>{
 const declared=createHTML(domain,{structure:errors[0]!.identity,url:urlID,target:"unused",interval:"unused"},[cssRoute]);
 ...
 if(missing.kind==="domain")expect((domainFailureDiagnostics(missing.value).payload as {reason:string}).reason).toBe("missing");
 const unowned=await declared.rejectAsset("unowned");
 if(unowned.kind==="domain")expect((domainFailureDiagnostics(unowned.value).payload as {reason:string}).reason).toBe("unowned");
...
test("reserved routes and htmx navigation headers are not application-controlled",async()=>{
 ...
 const responses=createResponses(domain,{invalid:errors[0]!.identity,invalidData:"unused"});
