...
const error:FailureShape={identity:identity("error",declaration.identity),kind:"error",declaration:declaration.identity,arguments:[],fields:["path","reason"].map(name=>({name,type:text.identity})),leaves:[],inputs:[],errors:[]};
const domain=createDomainRuntime({declarations:[{identity:declaration.identity,name:declaration.name,id:declaration.id,parameters:0}],shapes:[text,error]});
...
test("all octets round-trip through new frozen arrays without mutable aliases",async()=>{
 ...
 expect(first).toEqual(original);expect(second).toEqual(original);expect(first).not.toBe(second);
 expect(Object.isFrozen(first)).toBe(true);expect(byteLength(bytes)).toBe(256n);
 expect(()=>{(first as bigint[])[0]=255n;}).toThrow();
 expect(byteLength(value(await api.empty()))).toBe(0n);
 expect(value(await api.toUTF8(value(await api.empty())))).toBe("");
 const native=new Uint8Array([1,2]);const owned=ownBytes(native);native.fill(3);
 const copy=copyBytes(owned,origin);copy.fill(4);
 ...
 const buffer=Buffer.from([5,6]);const ownedBuffer=ownBytes(buffer);buffer.fill(0);
...
test("integer bounds are checked before native numeric narrowing",async()=>{
 for(const bad of [-1n,256n,-(1n<<256n),1n<<256n]) invalid(await api.fromInts([0n,bad,255n]),"/1","byte_range");
...
test("fatal UTF-8 preserves BOM and scalar extremes and rejects malformed bytes",async()=>{
 for(const text of ["","\0","héllo 😀","\ufefftext","\ufffd","\ud7ff\ue000","\udbff\udfff"]){
 ...
 for(const bytes of [[0x80],[0xc0,0xaf],[0xc2],[0xe2,0x82],[0xed,0xa0,0x80],[0xf4,0x90,0x80,0x80],[0xff]])
...
test("forged opaque values fail with resource_state without executing traps",async()=>{
 let traps=0;const fake=new Proxy({}, {get(){traps++;throw Error("trap");},getPrototypeOf(){traps++;throw Error("trap");}});
 for(const fakeValue of [fake,{},new Uint8Array([1]),null]) {
  for(const operation of [()=>api.toInts(fakeValue as Bytes),()=>api.toUTF8(fakeValue as Bytes)]) {
   const result=await invoke<unknown>(operation,origin);expect(result.kind).toBe("standard");
   if(result.kind==="standard")expect(standardFailureKind(result.value)).toBe("resource_state");
  }
  try{byteLength(fakeValue);throw Error("accepted forged bytes");}catch(cause){if(!isStandardFailure(cause))throw cause;expect(standardFailureKind(cause)).toBe("resource_state");}
 }
 expect(traps).toBe(0);
});
...
test("raw transport fixture consumes copies and cannot mutate owned source bytes",async()=>{
 ...
 const request=new Request("https://fixture.invalid/echo",{method:"POST",body:new Uint8Array(copyBytes(original,origin))});
