...
test("native callable receipts retain immutable aliases and protected results", async () => {
  ...
  expect(value(await first())).toBe(data);
  expect(assimilated).toBe(0);
  expect(callableReceipt(() => {})).toBeUndefined();
  expect(callableReceipt({ site: "owner#0" })).toBeUndefined();
  expect(() => ownCallable("owner#0", "target", [], first)).toThrow(
    "invalid callable construction",
  );
});
...
test("creation receipts distinguish frozen captures and preserve initialization identity across roots", async () => {
  ...
  const root = makeRoot("first"),
    other = makeRoot("second");
  const initialized = ownCallable("p::initialized#0", "target", ["private setup"], async () =>
    success(undefined),
  );
  const captures = ["private near"];
  const first = ownCallable(
    "p::factory#0",
    "target",
    captures,
  ...
  const path = async (context: typeof root, fn: typeof first) =>
    callContext(
      context,
      "p::invoke#0",
      (child) => invocationPath(contextIdentity(child!)),
      callableInstance(fn),
    );
  const a = await path(root, first),
    b = await path(root, second);
  expect(a.segments[0].callable?.occurrence).toBe(0);
  expect(b.segments[0].callable?.occurrence).toBe(1);
  ...
  const x = await path(root, initialized),
    y = await path(other, initialized);
  ...
  for (const context of [root, other]) {
