...
const text=scalar("str"),boolean=scalar("bool");
const declarations=catalogue.errors.filter(error=>error.id===1010);
const errors=declarations.map(error=>({identity:identity("error",error.identity),kind:"error",declaration:error.identity,arguments:[],fields:error.fields.map(field=>({name:field.name,type:text.identity})),leaves:[],inputs:[],errors:[]}));
const domain=createDomainRuntime({declarations:declarations.map(error=>({identity:error.identity,name:error.name,id:error.id,parameters:0})),shapes:[text,boolean,...errors]});
const api=createChecks(domain,{failed:errors[0].identity});
const origin=Object.freeze({source:"checks.test.ts",start:11,end:42,invocation:Object.freeze(["can.project.root/checks::guard#0"])});
...
test("true completes void while false produces checks::failed with the exact authored reason",async()=>{
 ...
 expect(failed.kind).toBe("domain");if(failed.kind!=="domain")throw Error("expected domain failure");
...
test("invalid hostile reasons throw during validation without invoking user code",async()=>{
 let invoked=0;const trap=()=>{invoked++;return "pwned";};
 const hostile={toString:trap,valueOf:trap,toJSON:trap,[Symbol.toPrimitive]:trap} as unknown as string;
...
test("a recovered application check cannot clear a separate harness violation",async()=>{
 const root=Object.freeze({package:"can.project.root/positive_checks",declaration:"can.project.root/positive_checks::require_positive",name:"zero"});
 const report=await runAssertion({root,expected:async()=>success(undefined),actual:async context=>{
  const checked=await api.require(false,"recovered",origin);
  if(checked.kind!=="domain")return checked;
  try{await withFixture(context,"table",[{selector:"zero",arguments:async()=>success([1n]),expected:async()=>success(1n)}],[2n],async()=>success(0n),origin);}catch{}
