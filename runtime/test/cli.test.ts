...
import { copyBytes, ownBytes, isBytes } from "../bytes.ts";
...
const cli = createCLI(domain, {invalidData: identity("error", "can.std.codec@1::invalid_data"), writeFailed: identity("error", "can.std.io@1::write_failed")});
...
test("native UTF-8 conversion preserves scalar text, BOM and immutable byte ownership", async () => {
  for (const text of ["", "héllo 😀", "\ufefftext", "\0", "\uFFFD"]) {
    const bytes = value(await cli.fromUTF8(text));
...
test("unpaired surrogates yield the exact codec domain error", async () => {
  for (const text of ["\ud800", "\udfff", "a\ud800b"]) {
    const completion = await cli.fromUTF8(text);
    ...
    expect(details.payload).toMatchObject({path: "", reason: "unpaired_surrogate"});
  }
});
