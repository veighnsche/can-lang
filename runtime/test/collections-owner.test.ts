...
test("map-held resources lease before launch and remain usable by a losing participant", async () => {
  for (const nested of [false, true]) {
    const gate = Promise.withResolvers<void>(),
      selected = Promise.withResolvers<void>();
    let token: any,
      loser: Completion | undefined,
      constructionLeases = -1,
      captureLeases = -1;
    const events: string[] = [],
      diagnostics: unknown[] = [];
    const root = runOwnedRoot(
      async () => {
        token = registerResource("test", {}, () => {
          events.push("close");
          return success(undefined);
        });
        let payload: object = token;
        if (nested) {
          const callable = registerCallableCaptures(() => {}, [
            record("holder", [["resource", token]]),
          ]);
          payload = value(
            await maps.insert(value(await maps.empty()), 2n, Object.freeze([callable, token])),
          );
        }
        ...
        await settle("race", [
          {
            ...
            run: async () => {
              await gate.promise;
              const stored = nested ? token : value(await maps.get(map, 1n));
              loser = await invoke(
                () =>
                  useResource(stored, "test", () => {
                    events.push("use");
                    return success(undefined);
                  }),
                origin,
          ...
          { captures: [], run: () => success(undefined) },
        ]);
        captureLeases = resourceStatus(token).leases;
        const close = closeResource(token, "test");
    ...
    const pendingState = resourceStatus(token).state,
      pendingEvents = [...events];
    gate.resolve();
    const completed = await root;
    expect(constructionLeases).toBe(0);
    expect(captureLeases).toBe(1);
    expect(pendingState).toBe("closing");
    expect(pendingEvents).toEqual([]);
    expect(completed.completion.kind).toBe("ok");
    expect(loser?.kind).toBe("ok");
...
test("closed map-held resource rejects all participant preparation and rolls back prior leases", async () => {
  const root = await runOwnedRoot(async () => {
    const open = registerResource("open", {}, () => success(undefined)),
      closed = registerResource("closed", {}, () => success(undefined));
    await closeResource(closed, "closed");
    // Construction carries containment evidence even when resource state is closed.
    const map = value(await maps.insert(value(await maps.empty()), 1n, closed));
    let starts = 0;
    ...
    expect(resourceStatus(open).leases).toBe(0);
    await closeResource(open, "open");
    return success(undefined);
  });
...
test("replace and remove carry only new contents while older map aliases retain theirs", async () => {
  const root = await runOwnedRoot(async () => {
    const first = registerResource("first", {}, () => success(undefined)),
      second = registerResource("second", {}, () => success(undefined));
    const old = value(await maps.insert(value(await maps.empty()), 1n, first)),
      replaced = value(await maps.replace(old, 1n, second)),
      removed = value(await maps.remove(old, 1n));
    await closeResource(first, "first");
    let starts = 0;
    await settle("all", [
      {
        captures: [replaced, removed],
        run: () => {
          starts++;
          expect(resourceStatus(second).leases).toBe(1);
          return success(undefined);
        },
      },
    ]);
    expect(starts).toBe(1);
    expect(resourceStatus(second).leases).toBe(0);
    const bad = await invoke(async () => {
      await settle("all", [
        {
          captures: [old],
          run: () => {
    ...
    expect(bad.kind).toBe("standard");
    expect(starts).toBe(1);
    await closeResource(second, "second");
    return success(undefined);
  });
  expect(root.completion.kind).toBe("ok");
});
test("opaque containment traversal does not invoke proxies or getters", async () => {
  ...
  const root = await runOwnedRoot(async () => {
    ...
    await settle("all", [{ captures: [map, proxiedMap], run: () => success(undefined) }]);
    return success(undefined);
  });
  expect(root.completion.kind).toBe("ok");
  expect(traps).toBe(0);
