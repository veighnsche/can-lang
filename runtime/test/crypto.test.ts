...
const keyIds = {
  keyMisuse: id("can.std.crypto@1::key_misuse"),
  invalidKey: id("can.std.crypto@1::invalid_key"),
  keypair: "crypto::keypair",
};
const keys = createCryptoKeys(domain, keyIds);
const passwords = createPasswords(domain, {
  costRejected: id("can.std.password@1::cost_rejected"),
  invalidHash: id("can.std.password@1::invalid_hash"),
});
const primitives = createCryptoPrimitives(domain, {
  keyMisuse: keyIds.keyMisuse,
  invalidKey: keyIds.invalidKey,
  invalidNonce: id("can.std.crypto@1::invalid_nonce"),
  decryptFailed: id("can.std.crypto@1::decrypt_failed"),
...
describe("crypto hashes and HMAC", () => {
  test("sha256 matches fixed vectors", async () => {
    expect(hexOf(value(await sha256(utf8("abc"))))).toBe(
      "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad",
    );
    expect(hexOf(value(await sha256(ownBytes(new Uint8Array(0)))))).toBe(
      "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
    );
  });
  test("hmac matches RFC 4231 case 1 and node interop", async () => {
    const key = ownBytes(new Uint8Array(20).fill(0x0b));
    expect(hexOf(value(await primitives.hmacSha256(key, utf8("Hi There"))))).toBe(
      "b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7",
    );
    const raw = new Uint8Array([9, 9, 9]);
    expect(hexOf(value(await primitives.hmacSha256(ownBytes(raw), utf8("can"))))).toBe(
      createHmac("sha256", raw).update("can").digest("hex"),
    );
  });
  test("empty hmac key rejects without native use", async () => {
    expect(
      domainOutcome(
        await primitives.hmacSha256(ownBytes(new Uint8Array(0)), utf8("x")),
        "crypto::invalid_key",
      ),
    ).toEqual({ reason: "empty_hmac_key" });
  });
  test("hmac copies inputs at the edge", async () => {
    ...
    expect(first).toBe(
      createHmac("sha256", new Uint8Array([1, 2, 3]))
        .update("m")
        .digest("hex"),
    );
  });
...
describe("password hashing", () => {
  const envelope =
    /^\$argon2id\$v=19\$m=(\d+),t=(\d+),p=(\d+)\$[A-Za-z0-9+/]{43}\$[A-Za-z0-9+/]{43}$/;
  ...
  test("unknown presets reject distinctly", async () => {
    expect(domainOutcome(await passwords.hash("pw", 3n), "password::cost_rejected")).toEqual({
      profile: 3n,
    });
    expect(domainOutcome(await passwords.hash("pw", -1n), "password::cost_rejected")).toEqual({
      profile: -1n,
    });
  });
  test("malformed hashes reject instead of reading false or throwing", async () => {
    ...
    for (const encoded of bad) {
      expect(
        domainOutcome(await passwords.verify("pw", encoded), "password::invalid_hash"),
      ).toEqual({ reason: "format" });
    }
  });
});
...
describe("aes-gcm", () => {
  test("explicit-nonce round-trip is deterministic with a 16-byte tag", async () => {
    ...
    expect(hexOf(value(await primitives.decryptAesGcm(key, nonce, first, aad)))).toBe(
      hexOf(utf8("secret")),
    );
  });
  test("sealed encryption mints a fresh nonce alongside the ciphertext", async () => {
    ...
    expect(
      hexOf(
        value(await primitives.decryptAesGcm(key, nonceOf(first), cipherOf(first), utf8("aad"))),
      ),
    ).toBe(hexOf(utf8("sealed")));
  });
  test("tampered ciphertext, nonce, key, and aad collapse to decrypt_failed", async () => {
    ...
    const cipher = new Uint8Array(
      copyBytes(
        value(await primitives.encryptAesGcm(key, ownBytes(nonce), utf8("data"), utf8("aad"))),
        origin,
      ),
    );
    const flip = (at: number) => {
      const copy = new Uint8Array(cipher);
      copy[at] ^= 1;
      return ownBytes(copy);
    };
    expect(
      domainOutcome(
        await primitives.decryptAesGcm(key, ownBytes(nonce), flip(0), utf8("aad")),
        "crypto::decrypt_failed",
      ),
    ).toEqual({});
    expect(
      domainOutcome(
        await primitives.decryptAesGcm(key, ownBytes(nonce), flip(cipher.length - 1), utf8("aad")),
        "crypto::decrypt_failed",
      ),
    ).toEqual({});
    ...
    expect(
      domainOutcome(
        await primitives.decryptAesGcm(key, ownBytes(badNonce), ownBytes(cipher), utf8("aad")),
        "crypto::decrypt_failed",
      ),
    ).toEqual({});
    expect(
      domainOutcome(
        await primitives.decryptAesGcm(other, ownBytes(nonce), ownBytes(cipher), utf8("aad")),
        "crypto::decrypt_failed",
      ),
    ).toEqual({});
    expect(
      domainOutcome(
        await primitives.decryptAesGcm(key, ownBytes(nonce), ownBytes(cipher), utf8("other")),
        "crypto::decrypt_failed",
      ),
    ).toEqual({});
  });
  test("nonce lengths other than 12 reject before native use", async () => {
    ...
    for (const length of [0, 8, 11, 13, 16]) {
      ...
      expect(
        domainOutcome(
          await primitives.encryptAesGcm(key, nonce, utf8("x"), utf8("")),
          "crypto::invalid_nonce",
        ),
      ).toEqual({ length: BigInt(length) });
      expect(
        domainOutcome(
          await primitives.decryptAesGcm(key, nonce, utf8("x"), utf8("")),
          "crypto::invalid_nonce",
        ),
      ).toEqual({ length: BigInt(length) });
    }
...
describe("ed25519", () => {
  test("keypairs sign and verify with reimported public keys", async () => {
    ...
    const priv = dataProperty(pair, "private_key"),
      pub = dataProperty(pair, "public_key");
  ...
  test("signatures interoperate with node crypto both directions", async () => {
    ...
    expect(
      value(
        await primitives.verifyEd25519(pub, ownBytes(message), ownBytes(new Uint8Array(nodeSig))),
      ),
    ).toBe(true);
    const subtleSig = new Uint8Array(
      copyBytes(value(await primitives.signEd25519(priv, ownBytes(message))), origin),
    );
  ...
  test("non-32-byte public imports reject; all-zero keys admit but never verify", async () => {
    for (const length of [0, 31, 33, 64]) {
      expect(
        domainOutcome(
          await keys.importEd25519Public(ownBytes(new Uint8Array(length))),
          "crypto::invalid_key",
        ),
      ).toEqual({ reason: "bad_public_length" });
    }
    const zero = value(await keys.importEd25519Public(ownBytes(new Uint8Array(32))));
    expect(
      value(await primitives.verifyEd25519(zero, utf8("m"), ownBytes(new Uint8Array(64)))),
    ).toBe(false);
  });
});
...
describe("key misuse and redaction", () => {
  test("encryption-only keys cannot sign and signing keys cannot decrypt", async () => {
    ...
    const priv = dataProperty(pair, "private_key"),
      pub = dataProperty(pair, "public_key");
    expect(
      domainOutcome(await primitives.signEd25519(aes, utf8("m")), "crypto::key_misuse"),
    ).toEqual({ operation: "sign", algorithm: "AES-GCM" });
    expect(
      domainOutcome(
        await primitives.verifyEd25519(aes, utf8("m"), ownBytes(new Uint8Array(64))),
        "crypto::key_misuse",
      ),
    ).toEqual({ operation: "verify", algorithm: "AES-GCM" });
    expect(
      domainOutcome(
        await primitives.encryptAesGcm(priv, ownBytes(new Uint8Array(12)), utf8("m"), utf8("")),
        "crypto::key_misuse",
      ),
    ).toEqual({ operation: "encrypt", algorithm: "Ed25519" });
    expect(
      domainOutcome(
        await primitives.decryptAesGcm(pub, ownBytes(new Uint8Array(12)), utf8("m"), utf8("")),
        "crypto::key_misuse",
      ),
    ).toEqual({ operation: "decrypt", algorithm: "Ed25519" });
    expect(
      domainOutcome(await primitives.signEd25519(pub, utf8("m")), "crypto::key_misuse"),
    ).toEqual({ operation: "sign", algorithm: "Ed25519" });
    expect(
      domainOutcome(
        await primitives.verifyEd25519(priv, utf8("m"), ownBytes(new Uint8Array(64))),
        "crypto::key_misuse",
      ),
    ).toEqual({ operation: "verify", algorithm: "Ed25519" });
  });
  test("forbidden exports misuse without touching native", async () => {
    ...
    expect(domainOutcome(await keys.exportEd25519Public(aes), "crypto::key_misuse")).toEqual({
      operation: "export",
      algorithm: "AES-GCM",
    });
    expect(domainOutcome(await keys.exportEd25519Public(priv), "crypto::key_misuse")).toEqual({
      operation: "export",
      algorithm: "Ed25519",
    });
  });
  ...
  test("handles and pairs redact key material", async () => {
    ...
    const failure = domainOutcome(
      await primitives.signEd25519(aes, utf8("m")),
      "crypto::key_misuse",
    );
