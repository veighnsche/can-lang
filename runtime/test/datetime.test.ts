...
const api = createDateTimes(domain, {
  outOfRange: id("can.std.time@1::out_of_range"),
  invalidZone: id("can.std.time@1::invalid_zone"),
  nonexistent: id("can.std.time@1::nonexistent_time"),
  invalidOption: id("can.std.time@1::invalid_option"),
});
...
describe("instants", () => {
  test("epoch millis round-trip exactly inside the native span", async () => {
    ...
    expect(
      outcome(await api.instantFromEpochMillis(8640000000000001n), "time::out_of_range"),
    ).toEqual({ millis: 8640000000000001n });
    expect(
      outcome(await api.instantFromEpochMillis(-8640000000000001n), "time::out_of_range"),
    ).toEqual({ millis: -8640000000000001n });
...
describe("zoned formatting", () => {
  test("explicit locale and zone format deterministically on the pinned ICU", async () => {
    ...
    expect(value(await api.formatInZone(noon, "en-US", "Asia/Tokyo", "medium", "short"))).toBe(
      "Sep 23, 2026 at 9:00 PM",
    );
    expect(value(await api.formatInZone(noon, "en-US", "UTC", "full", "none"))).toBe(
      "Wednesday, September 23, 2026",
    );
    expect(value(await api.formatInZone(noon, "en-US", "UTC", "none", "medium"))).toBe(
      "12:00:00 PM",
    );
    expect(value(await api.formatInZone(noon, "de-DE", "Europe/Berlin", "short", "short"))).toBe(
      "23.09.26, 14:00",
    );
  });
  test("unknown zones, styles, and locales reject distinctly", async () => {
    ...
    expect(
      outcome(
        await api.formatInZone(instant, "en-US", "Mars/Olympus", "medium", "short"),
        "time::invalid_zone",
      ),
    ).toEqual({ zone: "Mars/Olympus" });
    expect(
      outcome(
        await api.formatInZone(instant, "en-US", "UTC", "sometimes", "short"),
        "time::invalid_option",
      ),
    ).toEqual({ reason: "date_style" });
    expect(
      outcome(
        await api.formatInZone(instant, "en-US", "UTC", "medium", "whenever"),
        "time::invalid_option",
      ),
    ).toEqual({ reason: "time_style" });
    expect(
      outcome(
        await api.formatInZone(instant, "!", "UTC", "medium", "short"),
        "time::invalid_option",
      ),
    ).toEqual({ reason: "locale" });
  });
});
...
describe("civil resolution", () => {
  ...
  test("ordinary civil times resolve exactly", async () => {
    ...
    expect(value(await api.instantEpochMillis(handle))).toBe(
      BigInt(Date.UTC(2026, 0, 15, 17, 30, 45)),
    );
  });
  test("spring gaps reject; fall overlaps resolve by explicit policy", async () => {
    expect(
      outcome(
        await api.resolveZonedTime(civil(2026, 3, 8, 2, 30, 0), zone, 0n),
        "time::nonexistent_time",
      ),
    ).toEqual({});
    ...
    expect(value(await api.instantEpochMillis(earlier))).toBe(
      BigInt(Date.UTC(2026, 10, 1, 5, 30, 0)),
    );
    expect(value(await api.instantEpochMillis(later))).toBe(
      BigInt(Date.UTC(2026, 10, 1, 6, 30, 0)),
    );
  });
  test("impossible dates reject as nonexistent; malformed parts and policies reject as options", async () => {
    expect(
      outcome(
        await api.resolveZonedTime(civil(2026, 2, 30, 0, 0, 0), zone, 0n),
        "time::nonexistent_time",
      ),
    ).toEqual({});
    expect(
      outcome(
        await api.resolveZonedTime(civil(2026, 13, 1, 0, 0, 0), zone, 0n),
        "time::invalid_option",
      ),
    ).toEqual({ reason: "parts" });
    expect(
      outcome(
        await api.resolveZonedTime(civil(2026, 1, 1, 25, 0, 0), zone, 0n),
        "time::invalid_option",
      ),
    ).toEqual({ reason: "parts" });
    expect(
      outcome(
        await api.resolveZonedTime(civil(2026, 1, 1, 0, 0, 0), zone, 2n),
        "time::invalid_option",
      ),
    ).toEqual({ reason: "policy" });
    expect(
      outcome(
        await api.resolveZonedTime(civil(2026, 1, 1, 0, 0, 0), "Mars/Olympus", 0n),
        "time::invalid_zone",
      ),
    ).toEqual({ zone: "Mars/Olympus" });
  });
