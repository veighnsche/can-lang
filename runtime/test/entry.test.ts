...
test("forged and non-void main results fail without invoking a then getter", async () => {
  let reads = 0;
  // oxlint-disable-next-line no-thenable -- The getter detects any attempt to assimilate an invalid main result.
  const unboxed = Object.defineProperty({}, "then", {
    get() {
      reads++;
