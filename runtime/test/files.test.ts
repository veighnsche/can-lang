...
const declarations=catalogue.errors.filter(e=>[1110,1300,1301,1302,1303,1304,1305,1306,1307,1308].includes(e.id));
...
const fileInfo=hash("record","can.std.files@1::file_info"),entry=hash("record","can.std.files@1::entry");
const reads=createFileReads(domain,{notFound:identity("can.std.files@1::not_found"),denied:identity("can.std.files@1::denied"),invalidPath:identity("can.std.files@1::invalid_path"),unexpectedKind:identity("can.std.files@1::unexpected_kind"),limitExceeded:identity("can.std.files@1::limit_exceeded"),invalidData:identity("can.std.codec@1::invalid_data"),ioError:identity("can.std.files@1::io_error")});
const writes=createFileWrites(domain,{notFound:identity("can.std.files@1::not_found"),alreadyExists:identity("can.std.files@1::already_exists"),denied:identity("can.std.files@1::denied"),invalidPath:identity("can.std.files@1::invalid_path"),unexpectedKind:identity("can.std.files@1::unexpected_kind"),notEmpty:identity("can.std.files@1::not_empty"),crossDevice:identity("can.std.files@1::cross_device"),ioError:identity("can.std.files@1::io_error")});
const dirs=createFileDirectory(domain,{notFound:identity("can.std.files@1::not_found"),alreadyExists:identity("can.std.files@1::already_exists"),denied:identity("can.std.files@1::denied"),invalidPath:identity("can.std.files@1::invalid_path"),unexpectedKind:identity("can.std.files@1::unexpected_kind"),limitExceeded:identity("can.std.files@1::limit_exceeded"),notEmpty:identity("can.std.files@1::not_empty"),ioError:identity("can.std.files@1::io_error"),fileInfo,entry});
const globs=createFileGlobs(domain,{notFound:identity("can.std.files@1::not_found"),denied:identity("can.std.files@1::denied"),invalidPath:identity("can.std.files@1::invalid_path"),limitExceeded:identity("can.std.files@1::limit_exceeded"),ioError:identity("can.std.files@1::io_error")});
...
test("binary round-trip preserves NUL and invalid UTF-8; text round-trips strictly",async()=>{
 await withTemp(async root=>{
  ...
  expect(await writes.writeBytes(file,ownBytes(new Uint8Array([0,255,1])),true)).toEqual(success(undefined));
  const back=await reads.readBytes(file,100n);expect(back.kind).toBe("ok");if(back.kind!=="ok")throw Error();
...
test("missing paths report not_found; denied paths never report absence",async()=>{
 await withTemp(async root=>{
  ...
  const locked=join(root,"locked");await mkdir(locked);await writeFile(join(locked,"secret.txt"),"x");
  await chmod(locked,0o000);
  try{
   check(await reads.readBytes(join(locked,"secret.txt"),10n),1301,{operation:"read_bytes"});
...
test("over-cap input fails; exact caps and huge limits succeed",async()=>{
 await withTemp(async root=>{
  ...
  const exact=await reads.readBytes(file,1000n);expect(exact.kind).toBe("ok");
  expect(await reads.readBytes(file,10n**100n)).toEqual(exact);
  check(await reads.readBytes(file,0n),1305,{limit:0n});
  const empty=join(root,"empty.txt");await writeFile(empty,"");
...
test("exclusive creation is atomic; concurrent creators race to one winner",async()=>{
 await withTemp(async root=>{
  ...
  const outcomes=await Promise.all(Array.from({length:10},(_,i)=>writes.writeText(racy,"writer"+i,false)));
...
test("missing parents fail; writes never create directories",async()=>{
 await withTemp(async root=>{
  check(await writes.writeText(join(root,"nodir","f.txt"),"x",true),1300,{path:join(root,"nodir","f.txt")});
  ...
  const src=join(root,"src.txt");await writeFile(src,"data");
  check(await writes.copy(join(root,"missing.txt"),join(root,"dst.txt"),true),1300,{path:join(root,"missing.txt")});
  check(await writes.copy(src,join(root,"nodir2","dst.txt"),true),1300,{path:join(root,"nodir2","dst.txt")});
  check(await writes.move(join(root,"missing.txt"),join(root,"dst.txt"),true),1300,{path:join(root,"missing.txt")});
  check(await writes.move(src,join(root,"nodir2","dst.txt"),true),1300,{path:join(root,"nodir2","dst.txt")});
 });
});
test("directory-as-file and file-as-directory are kind errors",async()=>{
 await withTemp(async root=>{
  await writeFile(join(root,"f.txt"),"x");await mkdir(join(root,"sub"));
  check(await reads.readBytes(join(root,"sub"),10n),1308,{path:join(root,"sub"),operation:"read_bytes"});
  check(await reads.readText(join(root,"sub"),10n),1308,{operation:"read_text"});
  check(await dirs.list(join(root,"f.txt"),10n),1308,{path:join(root,"f.txt"),operation:"list"});
  check(await writes.copy(join(root,"sub"),join(root,"c.txt"),true),1308,{operation:"copy"});
 });
});
test("stat follows or reports links; exists sees dangling links",async()=>{
 await withTemp(async root=>{
  await writeFile(join(root,"a.txt"),"hello");
  await symlink("a.txt",join(root,"link.txt"));await symlink("gone",join(root,"dangling"));await mkdir(join(root,"sub"));
  const file=await dirs.stat(join(root,"a.txt"),true);
  expect(file.kind).toBe("ok");if(file.kind!=="ok")throw Error();
  expect(recordIdentity(file.value)).toBe(fileInfo);
  expect(dataProperty(file.value,"kind")).toBe("file");expect(dataProperty(file.value,"size")).toBe(5n);
  const link=await dirs.stat(join(root,"link.txt"),false);
  expect(link.kind).toBe("ok");if(link.kind!=="ok")throw Error();
  expect(dataProperty(link.value,"kind")).toBe("symlink");
  const followed=await dirs.stat(join(root,"link.txt"),true);
  expect(followed.kind).toBe("ok");if(followed.kind!=="ok")throw Error();
  expect(dataProperty(followed.value,"kind")).toBe("file");
  const dangling=await dirs.stat(join(root,"dangling"),false);
  expect(dangling.kind).toBe("ok");if(dangling.kind!=="ok")throw Error();
...
test("list returns absolute sorted entries with kinds",async()=>{
 await withTemp(async root=>{
  await writeFile(join(root,"b.txt"),"b");await writeFile(join(root,"a.txt"),"a");
  await mkdir(join(root,"sub"));await symlink("a.txt",join(root,"link.txt"));
  const result=await dirs.list(root,100n);
  expect(result.kind).toBe("ok");if(result.kind!=="ok")throw Error();
  const rows=result.value as readonly unknown[];
  expect(rows.map(v=>dataProperty(v,"path"))).toEqual([join(root,"a.txt"),join(root,"b.txt"),join(root,"link.txt"),join(root,"sub")]);
  expect(rows.map(v=>dataProperty(v,"kind"))).toEqual(["file","file","symlink","directory"]);
...
test("copy and move round-trip; overwrite gates replacement",async()=>{
 await withTemp(async root=>{
  const src=join(root,"src.txt"),dst=join(root,"dst.txt");
...
test("remove deletes entries; trees need explicit recursion",async()=>{
 await withTemp(async root=>{
  const tree=join(root,"tree");await mkdir(join(tree,"sub"),{recursive:true});
  await writeFile(join(tree,"sub","f.txt"),"x");await writeFile(join(tree,"top.txt"),"t");
...
test("glob matches dotfiles and symlinks with pinned traversal",async()=>{
 await withTemp(async root=>{
  await mkdir(join(root,"sub"));await writeFile(join(root,"sub","a.txt"),"x");
  await writeFile(join(root,"top.txt"),"y");await writeFile(join(root,".dot"),"d");
  await symlink("top.txt",join(root,"link.txt"));await symlink("sub",join(root,"linkdir"));
  await symlink("gone",join(root,"dangling.txt"));
  expect(await globs.glob(root,"*.txt",false,100n)).toEqual(success([join(root,"dangling.txt"),join(root,"link.txt"),join(root,"top.txt")]));
  expect(await globs.glob(root,"*.txt",true,100n)).toEqual(success([join(root,"dangling.txt"),join(root,"link.txt"),join(root,"top.txt")]));
  expect(await globs.glob(root,"**/*.txt",false,100n)).toEqual(success([join(root,"dangling.txt"),join(root,"link.txt"),join(root,"sub","a.txt"),join(root,"top.txt")]));
  expect(await globs.glob(root,"**/*.txt",true,100n)).toEqual(success([join(root,"dangling.txt"),join(root,"link.txt"),join(root,"linkdir","a.txt"),join(root,"sub","a.txt"),join(root,"top.txt")]));
  expect(await globs.glob(root,"*",false,100n)).toEqual(success([join(root,"dangling.txt"),join(root,"link.txt"),join(root,"linkdir"),join(root,"sub"),join(root,"top.txt")]));
...
test("unsupplied assertions cannot touch the filesystem",async()=>{
 await withTemp(async root=>{
  ...
  for(const [name,actual] of calls){
   const result=await runAssertion({root:{package:"test",declaration:"boundary",name},actual,expected:async()=>success(undefined)});
...
test("unknown native failures stay standard; classification is exact",async()=>{
 ...
 expect(classifyFileError(Object.assign(new Error("denied"),{code:"EACCES"}))).toEqual({kind:"denied",code:"EACCES"});
 expect(classifyFileError(Object.assign(new Error("busy"),{code:"EBUSY"}))).toEqual({kind:"io_error",code:"EBUSY"});
 expect(classifyFileError(Object.assign(new Error("device"),{code:"EXDEV"}))).toEqual({kind:"cross_device",code:"EXDEV"});
