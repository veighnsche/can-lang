...
const primitives:SchemaNode[]=["int","str","bool","float"].map(name=>({identity:name,kind:"primitive",name}));
const schema=(root:string,nodes:SchemaNode[]=[]):Schema=>({root,nodes:[...primitives,...nodes]});
const row:SchemaNode={identity:"row",kind:"record",name:"t::row",fields:[{name:"name",type:"str"},{name:"n",type:"int"}]};
const rowSchema=schema("row",[row]);
const codec=createCodec(rowSchema,domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
const reads=createStreamReads(domain,{readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled"),closeFailed:identity("can.std.stream@1::close_failed"),limitExceeded:identity("can.std.codec@1::invalid_data")});
...
function fails(run:()=>unknown,reason:Reason,path?:string){try{run();throw Error("accepted invalid document");}catch(e){expect(e).toBeInstanceOf(CodecIssue);expect((e as CodecIssue).reason).toBe(reason);if(path!==undefined)expect((e as CodecIssue).path).toBe(path);}}
async function owned(body:()=>Promise<void>):Promise<void>{
  ...
  expect(out.completion.kind).toBe("ok");expect(out.cleanupFailed).toBe(false);
}
const handler=(seen:unknown[])=>async(v:unknown)=>{seen.push(v);return success(undefined);};

test("toml decodes typed configuration and fails its native violations",async()=>{
  const s=schema("config",[
    {identity:"config",kind:"record",name:"t::config",fields:[{name:"title",type:"str"},{name:"server",type:"server"}]},
    {identity:"server",kind:"record",name:"t::server",fields:[{name:"host",type:"str"},{name:"ports",type:"ports"}]},
    {identity:"ports",kind:"array",name:"int[]",element:"int"},
  ]);
  const c=createCodec(s,domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
  const out=value(await c.decodeToml(from('title = "t"\n[server]\nhost = "h"\nports = [80, 443]\n'))) as any;
  expect(out.title).toBe("t");expect(out.server.host).toBe("h");
  expect(dataArray(out.server.ports)).toEqual([80n,443n]);
  check(await c.decodeToml(from('title = "t"\n[server]\nhost = "h"\nports = [80, 9007199254740993]\n')),1110,{path:"",reason:"invalid_toml"});
  check(await c.decodeToml(from('title = "a"\ntitle = "b"\n[server]\nhost = "h"\nports = []\n')),1110,{path:"",reason:"invalid_toml"});
  check(await c.decodeToml(from('[1,2]')),1110,{path:"",reason:"invalid_toml"});
  const f=createCodec(schema("float"),domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
  check(await f.decodeToml(from('a = nan')),1110,{path:"",reason:"type"});
  const d=createCodec(schema("d",[{identity:"d",kind:"record",name:"t::d",fields:[{name:"d",type:"str"}]}]),domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
...
test("yaml projects core scalars and pins rounding, sharing and cycles",async()=>{
  ...
  expect(out.name).toBe("r");expect(out.n).toBe(7n);
  ...
  const strs=createCodec(schema("w",[{identity:"w",kind:"record",name:"t::w",fields:[{name:"a",type:"str"},{name:"b",type:"str"},{name:"c",type:"str"}]}]),domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
  const w=value(await strs.decodeYaml(from('a: 2024-01-02T03:04:05Z\nb: yes\nc: on\n'))) as any;
  expect(w.a).toBe("2024-01-02T03:04:05Z");expect(w.b).toBe("yes");expect(w.c).toBe("on");
  // Shared aliases project per occurrence.
  const pair=createCodec(schema("p",[{identity:"p",kind:"record",name:"t::p",fields:[{name:"a",type:"int"},{name:"b",type:"int"}]}]),domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
  const shared=value(await pair.decodeYaml(from('a: &x 4\nb: *x\n'))) as any;
  expect(shared.a).toBe(4n);expect(shared.b).toBe(4n);
  // Genuine cycles fail instead of recursing forever.
  const loop=createCodec(schema("a",[{identity:"a",kind:"record",name:"t::a",fields:[{name:"b",type:"a"}]}]),domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
  check(await loop.decodeYaml(from('b: &x\n  b: *x\n')),1110,{path:"/b/b",reason:"cycle"});
  // Multi-document streams project against array schemas.
  const rows=createCodec(schema("rows",[{identity:"rows",kind:"array",name:"t::row[]",element:"row"},row]),domain,{invalidData:identity("can.std.codec@1::invalid_data"),readFailed:identity("can.std.stream@1::read_failed"),cancelled:identity("can.std.stream@1::cancelled")});
...
test("json5 accepts extensions and pins rounding and duplicates",async()=>{
  ...
  check(await codec.decodeJson5(from('{name: "r", n: 9007199254740993}')),1110,{path:"/n",reason:"integer_token"});
  ...
  expect(dup.name).toBe("b");
  check(await codec.decodeJson5(from('{name: "a", n:}')),1110,{path:"",reason:"invalid_json5"});
});

test("jsonl bounded decode is exact and strict",async()=>{
  const rows=value(await codec.decodeJsonl(from('{"name":"a","n":9007199254740993}\n{"name":"b","n":-0}\n'))) as unknown;
  ...
  const framed=value(await codec.decodeJsonl(from('\n{"name":"a","n":1}\r\n\r\n{"name":"b","n":2}'))) as unknown;
  ...
  check(await codec.decodeJsonl(from('{"name":"a","n":1}\nng\n')),1110,{path:"/1",reason:"invalid_json"});
  check(await codec.decodeJsonl(from('{"name":"a","n":1}\n{"name":"b","n":')),1110,{path:"/1",reason:"invalid_json"});
  // Record faults carry the record index.
  check(await codec.decodeJsonl(from('{"name":"a","n":1}\n{"name":"b","n":1,"n":2}\n')),1110,{path:"/1/n",reason:"duplicate_member"});
  check(await codec.decodeJsonl(from('{"name":"a","n":1}\n{"name":"b","n":"x"}\n')),1110,{path:"/1/n",reason:"type"});
});

test("consume matches bounded decode at every split point",async()=>{
  await owned(async()=>{
    const text='{"name":"héllo ❄","n":1}\n\n{"name":"b","n":9007199254740993}\r\n{"name":"c","n":-12}';
    const raw=new TextEncoder().encode(text);
    const want=dataArray(value(await codec.decodeJsonl(ownBytes(raw))) as unknown).map(r=>[dataProperty(r,"name"),dataProperty(r,"n")]);
    expect(want.length).toBe(3);
    for(let split=1;split<raw.length;split++){
      const seen:unknown[]=[];
      const count=value(await codec.consume(byteReader([raw.slice(0,split),raw.slice(split)]),handler(seen))) as bigint;
...
test("consume stops on cancellation, read failure and record faults",async()=>{
  await owned(async()=>{
    ...
    const gated=new ReadableStream<Uint8Array>({async pull(c){started=true;await gate;c.enqueue(enc.encode('{"name":"a","n":1}\n'));c.close();}});
    const cancellable=registerReader(openByteCell(gated,65536n),fail,identity("can.std.stream@1::close_failed"),{scopeManaged:true});
    const seen:unknown[]=[];
    ...
    const boom=new ReadableStream<Uint8Array>({pull(){throw Object.assign(new Error("reader boom"),{code:"reader_boom"});}});
    const failing=registerReader(openByteCell(boom,65536n),fail,identity("can.std.stream@1::close_failed"),{scopeManaged:true});
    check(await codec.consume(failing,handler([])),1316,{reason:"reader_boom"});
    const partial:unknown[]=[];
    check(await codec.consume(byteReader([new TextEncoder().encode('{"name":"a","n":1}\n{"name":"b","n":"x"}\n')]),handler(partial)),1110,{path:"/1/n",reason:"type"});
...
test("oversized records fail the framing byte cap",()=>{
  ...
  fails(()=>{framer.push(new TextEncoder().encode('{"0123456789abcdef"'));},"byte_limit","/1");
});
