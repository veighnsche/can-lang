...
const responses = createResponses(domain, {
  ...
  invalidData: errors[0]!.identity,
  close: "unused",
  writeFailed: "unused",
  limit: "unused",
});
const routing = createRouter(domain, {
  invalid: errors[1]!.identity,
  duplicate: errors[2]!.identity,
  ambiguous: errors[3]!.identity,
});
test("route path corpus matches checked static validation", async () => {
  const corpus = (await Bun.file(new URL("./http-paths.json", import.meta.url)).json()) as {
    accept: string[];
    reject: string[];
  };
  const status = await responses.ok(),
    headers = await responses.emptyHeaders();
  ...
  for (const path of corpus.reject) {
    const made = await routing.get(path, callback);
    expect(made.kind).not.toBe("ok");
  }
