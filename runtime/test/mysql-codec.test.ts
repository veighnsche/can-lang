...
const rowsOf = (fields: Array<{ name: string; kind: string; inner?: string; some?: string; none?: string }>) => ({
  root: "app::row",
  fields: fields as Array<{ name: string; kind: "bool" | "int" | "float" | "str" | "bytes" | "option"; inner?: "bool" | "int" | "float" | "str" | "bytes"; some?: string; none?: string }>,
});
const decoded = (fields: Parameters<typeof rowsOf>[0], row: unknown): unknown => {
  const outcome = codec.decodeRow(rowsOf(fields), row);
  if (!outcome.ok) throw new Error("expected row, got " + JSON.stringify(domainOutcome(outcome.failure, "sql::schema_mismatch")));
  return outcome.value;
};
const mismatched = (fields: Parameters<typeof rowsOf>[0], row: unknown): Record<string, unknown> => {
  const outcome = codec.decodeRow(rowsOf(fields), row);
...
describe("mysql codec", () => {
  test("datetimes render naive UTC wall text", () => {
    expect(dataProperty(decoded([{ name: "t", kind: "str" }], { t: new Date("2026-01-15T12:00:00.000Z") }), "t")).toBe("2026-01-15 12:00:00");
    expect(dataProperty(decoded([{ name: "t", kind: "str" }], { t: new Date("2026-03-04T05:06:07.890Z") }), "t")).toBe("2026-03-04 05:06:07.890");
    expect(dataProperty(decoded([{ name: "t", kind: "str" }], { t: new Date("1999-12-31T23:59:59.001Z") }), "t")).toBe("1999-12-31 23:59:59.001");
    expect(mismatched([{ name: "t", kind: "str" }], { t: new Date(NaN) })).toEqual({ path: "/t", reason: "type" });
  });
  ...
  test("out-of-range and malformed integers reject without coercion", () => {
    const f = [{ name: "v", kind: "int" }] as Parameters<typeof rowsOf>[0];
    expect(mismatched(f, { v: "18446744073709551615" })).toEqual({ path: "/v", reason: "int_range" });
    expect(mismatched(f, { v: 18446744073709551615n })).toEqual({ path: "/v", reason: "int_range" });
  ...
  test("classification maps verified codes and falls back with errno", () => {
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_CONNECTION_REFUSED"), failures, contracts), "sql::connection_failed")).toEqual({ phase: "query" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_CONNECTION_CLOSED"), failures, contracts), "sql::connection_failed")).toEqual({ phase: "query" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_SERVER_ERROR", 1045), failures, contracts), "sql::connection_failed")).toEqual({ phase: "auth" });
    expect(domainOutcome(classifyMySQL("execute", mysqlError("ERR_MYSQL_SERVER_ERROR", 1062), failures, contracts), "sql::constraint_failed")).toEqual({ constraint: "ER_DUP_ENTRY" });
    expect(domainOutcome(classifyMySQL("execute", mysqlError("ERR_MYSQL_SYNTAX_ERROR", 1064), failures, contracts), "sql::query_failed")).toEqual({ operation: "execute", code: "ER_PARSE_ERROR" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_SERVER_ERROR", 1146), failures, contracts), "sql::query_failed")).toEqual({ operation: "query_one", code: "ER_NO_SUCH_TABLE" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_SERVER_ERROR", 1213), failures, contracts), "sql::query_failed")).toEqual({ operation: "query_one", code: "ER_LOCK_DEADLOCK" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_SERVER_ERROR", 1205), failures, contracts), "sql::query_failed")).toEqual({ operation: "query_one", code: "ER_LOCK_WAIT_TIMEOUT" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_SERVER_ERROR", 1054), failures, contracts), "sql::query_failed")).toEqual({ operation: "query_one", code: "mysql_errno_1054" });
    expect(domainOutcome(classifyMySQL("query_one", mysqlError("ERR_MYSQL_SOMETHING"), failures, contracts), "sql::query_failed")).toEqual({ operation: "query_one", code: "ERR_MYSQL_SOMETHING" });
    expect(() => classifyMySQL("query_one", new Error("boom"), failures, contracts)).toThrow("boom");
  });
  test("affected rows read the mysql count field", () => {
    ...
    expect(domainOutcome(mysqlAffectedRows("execute", { count: 0 }, failures), "sql::query_failed")).toEqual({ operation: "execute", code: "bad_count" });
    expect(domainOutcome(mysqlAffectedRows("execute", { affectedRows: -1 }, failures), "sql::query_failed")).toEqual({ operation: "execute", code: "bad_count" });
    expect(domainOutcome(mysqlAffectedRows("execute", null, failures), "sql::query_failed")).toEqual({ operation: "execute", code: "bad_count" });
  });
});
