...
const lookup = (name: string): string | undefined =>
  MYSQL_URL !== undefined && name === "CAN_TEST_MYSQL" ? MYSQL_URL : undefined;
...
const table: Record<string, Record<string, SQLDescriptorEntry>> = {
  "": {
    drop_cover: {
      ...
      segments: [{ text: "DROP TABLE IF EXISTS cover" }],
      params: [],
      paramType: "p",
      rowType: "r",
      limit: 0,
    ...
    set_lock_timeout: {
      ...
      cardinality: "execute",
      kind: "set_statement",
      segments: [{ text: "SET SESSION innodb_lock_wait_timeout = 1" }],
      params: [],
      paramType: "p",
      rowType: "r",
      limit: 0,
    ...
    set_wait_timeout: {
      ...
      cardinality: "execute",
      kind: "set_statement",
      segments: [{ text: "SET SESSION wait_timeout = 1" }],
      params: [],
      paramType: "p",
      rowType: "r",
      limit: 0,
...
const mk = (idn: bigint, flag: boolean) =>
...
describe("mysql transactions", () => {
  live("transactions commit across awaits and roll back", async () => {
    await owned(async () => {
      ...
      const dup = await transactions.withTransaction(
        token,
        async (handle: unknown) => {
          value(await transactions.execute(insert, insertParams, handle, mk(5n, true)));
  ...
  live("opposite-order updates deadlock exactly one attempt", async () => {
    await owned(async () => {
      ...
      const flag = (on: boolean, idn: bigint) =>
      ...
      const [a, b] = await Promise.all([
        attempt(first, 101n, 102n, aLocked, bGate),
        attempt(second, 102n, 101n, bLocked, aGate),
      ]);
      const codes = [a, b].map((outcome) =>
        outcome.kind === "domain" ? domainOutcome(outcome, "sql::query_failed") : outcome,
      );
      expect(
        codes.filter(
          (code) =>
            typeof code === "object" &&
            (code as Record<string, unknown>)["code"] === "ER_LOCK_DEADLOCK",
        ),
      ).toHaveLength(1);
      expect(
        codes.filter(
  ...
  live("lock waits time out with ER_LOCK_WAIT_TIMEOUT", async () => {
    await owned(async () => {
      ...
      value(
        await pools.execute(
          descriptors.declareDescriptor("", "set_lock_timeout"),
          emptyParams,
          second,
          record("app::empty", []),
        ),
      ...
      ).toEqual({ operation: "execute", code: "ER_LOCK_WAIT_TIMEOUT" });
  ...
  live("idle expiry self-heals on next use", async () => {
    await owned(async () => {
      ...
      value(
        await pools.execute(
          descriptors.declareDescriptor("", "set_wait_timeout"),
          emptyParams,
          token,
          record("app::empty", []),
        ),
  ...
  dockerized("restart kills in-flight work and the pool recovers", async () => {
    await owned(async () => {
      ...
      try {
        const flight = pools.queryOne(
          sleep,
          sleepParams,
          token,
          record("app::sleep_parameters", [["secs", 10]]),
        );
        await Bun.sleep(500);
      ...
      for (let i = 0; i < 30; i++) {
        const probe = await pools.queryOne(
          descriptors.declareDescriptor("", "session_tz"),
          tzParams,
          token,
          record("app::empty", []),
