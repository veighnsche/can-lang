...
const api = createNumbers(domain, {
  inexact: errors[0].identity,
  invalidNumber: errors[1].identity,
...
test("native formatting preserves bigint digits and IEEE spellings without locale formatting", async () => {
  for (const input of [0n, -1n, 9007199254740993n, 10n ** 400n])
    expect(value(await api.fromInt(input))).toBe(String(input));
  for (const input of [
    NaN,
    Infinity,
    -Infinity,
    ...
    expect(value(await api.fromFloat(input))).toBe(String(input));
...
test("Math and Number predicates agree with qualified native operations on boundaries", async () => {
  ...
  for (const operation of ["floor", "ceil", "trunc", "round"] as const)
    for (const input of inputs)
      expect(Object.is(value(await api[operation](input)), Math[operation](input))).toBe(true);
  for (const input of inputs) {
    expect(value(await api.isFinite(input))).toBe(Number.isFinite(input));
    expect(value(await api.isNaN(input))).toBe(Number.isNaN(input));
  }
...
test("integer parsing requires the entire exact decimal grammar", async () => {
  for (const input of ["0", "-0", "1", "-1", "9007199254740993", "9".repeat(400)])
    expect(value(await api.toInt(input))).toBe(BigInt(input));
  for (const input of [
    "",
    " ",
    " 1",
...
test("float parsing admits only decimal source numbers and finite native results", async () => {
  for (const input of [
    ...
    expect(Object.is(value(await api.toFloat(input)), Number(input))).toBe(true);
  }
  for (const input of [
    "",
    " ",
...
test("boolean conversions accept exact spellings and only the integer range zero through one", async () => {
  for (const [input, expected] of [
    ...
    expect(value(await api.boolToInt(input))).toBe(expected);
    expect(value(await api.intToBool(expected))).toBe(input);
