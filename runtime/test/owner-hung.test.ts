...
for (const leased of [false, true])
  test(`external supervisor kills a nonsettling owner (leased=${leased})`, async () => {
    const directory = mkdtempSync(join(tmpdir(), "can-owner-supervisor-"));
    const runtime = fileURLToPath(new URL("../", import.meta.url));
    const file = join(directory, "hung.ts");
    writeFileSync(
      file,
      `import {runOwnedRoot,launchOwned,registerResource,closeResource} from ${JSON.stringify(join(runtime, "owner.ts"))};
...
await runOwnedRoot(async()=>{
 ...
 console.log("owned-pending");return success(undefined);
    ...
    child.stdout.on("data", (chunk) => {
      output += chunk;
      if (output.includes("owned-pending") && !killTimer)
    ...
    try {
      ...
      expect(output).toContain("owned-pending");
      expect(output).not.toContain("drained");
      expect(output).not.toContain("native-close");
      expect(errors).toBe("");
