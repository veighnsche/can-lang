...
test("published groups inspect each losing outcome once while preserving occurrence dedup", async () => {
  const count = 256,
    ready = deferred<void>(),
    release = deferred<Completion>();
  const occurrence = captureStandard(new Error("shared losing occurrence"), origin),
  ...
  try {
    const root = runOwnedRoot(
      async () => {
        ...
        const selected = await group.promises[0];
        group.publish([0]);
        ready.resolve();
        return selected;
      },
...
test("closing preserves existing-owner subleases and deadlines never force close", async () => {
  ...
  const result = await runOwnedRoot(async () => {
    token = registerResource("test.pool", { value: 9 }, () => {
    ...
    const group = launchOwned([
      {
        ...
        run: async () => {
          ...
          const used = await useResource(token, "test.pool", (native) => {
    ...
    group.publish([]);
    const closing = closeResource(token, "test.pool", {
      milliseconds: 0,
    ...
    const refused = await invoke(() => {
      launchOwned([{ captures: [token], run: empty }]);
      return empty();
    }, origin);
...
test("reverse automatic cleanup reports omissions even when native closes succeed", async () => {
  ...
  const result = await runOwnedRoot(
    () => {
      registerResource("first", {}, () => {
        events.push("first");
        return empty();
      });
      registerResource("second", {}, () => {
        events.push("second");
        return empty();
      });
      return empty();
  ...
  expect(result.cleanupFailed).toBe(true);
  expect(diagnostics.map((d) => d.category)).toEqual(["cleanup", "cleanup"]);
});

test("scoped handles reject escapes and host callbacks block user code", async () => {
  let token!: Resource;
  let callback!: () => Promise<Completion>;
  let guarded!: () => Promise<Completion>;
  let executions = 0;
  const result = await runOwnedRoot(async () => {
    const scoped = await withScope(async (scope) => {
      token = registerResource("transaction", {}, empty, { scopeManaged: true });
      callback = registerCallableCaptures(
        async () =>
          useResource(token, "transaction", () => {
            executions++;
            return empty();
          }),
        [token],
      );
      guarded = guardCallback(scope, async () => {
        executions++;
        return empty();
      });
      return empty();
    ...
    for (const run of [callback, guarded, () => useResource(token, "transaction", empty)]) {
      const refused = await invoke(run, origin);
      expect(refused.kind).toBe("standard");
...
test("resource preparation fails before every participant and rolls back leases", async () => {
  ...
  const result = await runOwnedRoot(async () => {
    ...
    const expired = registerResource("pool", {}, empty);
    await closeResource(expired, "pool");
    const refused = await invoke(() => {
      launchOwned([
        {
          captures: [token],
...
test("interleaved roots preserve native async context and reject cross-root handles", async () => {
  ...
  const first = runOwnedRoot(async () => {
    a = registerResource("pool", { root: "a" }, empty);
    ready.resolve(a);
    await release.promise;
    const read = await useResource(a, "pool", (native) =>
      success((native as { root: string }).root),
    );
    expect(value(read)).toBe("a");
    await closeResource(a, "pool");
    return success("first");
  });
  const second = runOwnedRoot(async () => {
    const foreign = await ready.promise;
    const refused = await invoke(() => useResource(foreign, "pool", empty), origin);
    expect(refused.kind).toBe("standard");
    const b = registerResource("pool", { root: "b" }, empty);
    await Promise.resolve();
    expect(
      value(await useResource(b, "pool", (native) => success((native as { root: string }).root))),
    ).toBe("b");
    await closeResource(b, "pool");
    release.resolve();
      ...
      const group = launchOwned([
        {
          ...
          run: async () => {
            await gate.promise;
            return useResource(token, "transaction", empty);
          },
        },
      ]);
      group.publish([]);
...
test("automatic close faults propagate cleanup from a managed scope", async () => {
  const root = await runOwnedRoot(() =>
    withScope(async () => {
      registerResource(
        ...
        {},
        () => {
          throw new Error("private native close fault");
        },
        { scopeManaged: true },
      );
      return empty();
  ...
  expect(root.cleanupFailed).toBe(true);
  expect(root.completion.kind).toBe("standard");
  if (root.completion.kind === "standard")
    expect(standardFailureKind(root.completion.value)).toBe("cleanup");
});
...
test("resources opened by a loser retain its lease across registered shutdown expiry", async () => {
  ...
  const root = runOwnedRoot(
      ...
      const group = launchOwned([
        {
          ...
          run: async () => {
            ...
            await use.promise;
            return useResource(token, "pool", empty);
          },
        },
      ]);
      group.publish([]);
    ...
    (diagnostic) => {
      if (diagnostic.phase === "cleanup") expired.resolve();
    },
  );
  create.resolve();
...
test("a participant-created managed child scope releases its initial lease at callback exit", async () => {
  ...
  const result = await runOwnedRoot(async () => {
    ...
    const completed = await group.promises[0];
    group.publish([0]);
    return completed;
  });
    ...
    const group = launchOwned([
      {
        captures: [],
        run: async () => {
          const token = registerResource("pool", {}, () => {
            closed++;
            return empty();
          });
          await useResource(token, "pool", empty);
          return closeResource(token, "pool");
        },
      },
    ]);
...
test("idempotent reentrant close shares the installed native close attempt", async () => {
  let calls = 0;
  let reentered: Promise<Completion<void>> | undefined;
  const result = await runOwnedRoot(async () => {
    let token!: Resource;
    token = registerResource(
      "pool",
      ...
      () => {
        calls++;
        if (calls === 1) reentered = closeResource(token, "pool");
        return empty();
      },
      { idempotent: true },
    );
    expect((await closeResource(token, "pool")).kind).toBe("ok");
    expect((await reentered!).kind).toBe("ok");
    return empty();
  });
  expect(calls).toBe(1);
  expect(result.cleanupFailed).toBe(false);
  ...
  const result = await runOwnedRoot(() =>
    withScope(async (scope) => {
      ...
      const callback = guardCallback(
        scope,
        registerCallableCaptures(async () => {
          await gate.promise;
          return useResource(token, "pool", empty);
        }, [token]),
      );
      ...
      group.publish([]);
      expect(resourceStatus(token).leases).toBe(1);
      const closing = closeResource(token, "pool");
      await Promise.resolve();
      expect(closed).toBe(false);
      gate.resolve();
      await closing;
...
test("selected standard outcomes are not late and repeated losing occurrences report once", async () => {
  const occurrence = captureStandard(new Error("one occurrence"), origin);
  const diagnostics: OwnerDiagnostic[] = [];
  const result = await runOwnedRoot(
    async () => {
      const selected = launchOwned([{ captures: [], run: () => failure(occurrence) }]);
      await selected.promises[0];
      selected.publish([0]);
      const losers = launchOwned([
        { captures: [], run: () => failure(occurrence) },
        { captures: [], run: () => failure(occurrence) },
      ]);
      losers.publish([]);
      return empty();
  ...
  expect(result.cleanupFailed).toBe(false);
  expect(diagnostics.length).toBe(1);
  expect(diagnostics[0].message).toBe("Error: one occurrence");
});
...
test("native callbacks bind their operation scope and drain without ambient context", async () => {
  ...
  const root = runOwnedRoot(() =>
    withScope(async (scope) => {
      callback = guardCallback(scope, async () => {
        entered++;
        token = registerResource("callback", {}, empty, { scopeManaged: true });
        await finish.promise;
        return useResource(token, "callback", empty);
      });
      ready.resolve();
  ...
  expect((await root).cleanupFailed).toBe(false);
  expect(resourceStatus(token).state).toBe("closed");
  const refused = await invoke(callback, origin);
  expect(refused.kind).toBe("standard");
  expect(entered).toBe(1);
});
