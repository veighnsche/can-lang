...
const ids = {
  ...
  credential: errors[1].identity,
  transport: errors[2].identity,
  timeout: errors[3].identity,
  limit: errors[4].identity,
  status: errors[5].identity,
  failed: errors[6].identity,
  invalidData: errors[7].identity,
  invalidAnswer: errors[8].identity,
};
...
test("native injection skips the operation and selects by leaf identity", async () => {
  ...
  const invoked = await policyBase(context, operation, origin, async () => {
    throw new Error("wrapped operation ran");
  });
...
test("emitted injection is not normalized and keeps its own identity", async () => {
  ...
  expect(policyKey(invoked, ids.failed)).toEqual({
    origin: "emitted",
    identity: ids.invalidAnswer,
  });
  closeContext(context);
...
test("policyBase invokes the operation without context or injection", async () => {
  ...
  const live = await policyBase(context, operation, origin, async () =>
    failure(domain.create(ids.failed, record(ids.failed, [["detail", statusPayload()]]), origin)),
  );
