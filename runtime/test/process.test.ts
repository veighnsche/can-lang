...
const str=shape("primitive","str"),int=shape("primitive","int"),bool=shape("primitive","bool");
const declarations=catalogue.errors.filter(e=>[1300,1301,1310,1311,1312,1313,1314,1315].includes(e.id));
const errors=declarations.map(e=>shape("error",e.identity,e.fields.map(f=>({name:f.name,type:f.type==="int"?int.identity:f.type==="bool"?bool.identity:str.identity}))));
const domain=createDomainRuntime({declarations:declarations.map(e=>({...e,parameters:0})),shapes:[str,int,bool,...errors]});
const identity=(name:string)=>errors.find(e=>e.declaration===name)!.identity;
const resultId=hash("record","can.std.process@1::result");
const processes=createProcesses(domain,{notFound:identity("can.std.files@1::not_found"),denied:identity("can.std.files@1::denied"),spawnFailed:identity("can.std.process@1::spawn_failed"),timeout:identity("can.std.process@1::timeout"),outputLimit:identity("can.std.process@1::output_limit"),nonzero:identity("can.std.process@1::nonzero"),invalidConfig:identity("can.std.process@1::invalid_config"),ioError:identity("can.std.process@1::io_error"),result:resultId});
const origin={source:"test",start:0,end:0,invocation:[]};
type Over=Partial<{cwd:string;inherit:boolean;env:string[];stdin:Uint8Array;out:bigint;err:bigint;deadline:bigint;grace:bigint}>;
const options=(over:Over={})=>record("test-options",[["cwd",over.cwd??""],["inherit_env",over.inherit??false],["env",array(over.env??[])],["stdin",ownBytes(over.stdin??new Uint8Array())],["stdout_limit",over.out??1000000n],["stderr_limit",over.err??1000000n],["deadline_ms",over.deadline??0n],["grace_ms",over.grace??100n]]);
...
test("echo round-trips stdout with exit zero",async()=>{
 ...
 expect(bare.kind).toBe("ok");if(bare.kind!=="ok")throw Error();
...
test("nonzero exits and signals report distinctly",async()=>{
 ...
 expect(failed.kind).toBe("ok");if(failed.kind!=="ok")throw Error();
 ...
 expect(signaled.kind).toBe("ok");if(signaled.kind!=="ok")throw Error();
 expect(dataProperty(signaled.value,"code")).toBe(-1n);
 expect(dataProperty(signaled.value,"signal")).toBe("SIGTERM");
 check(await owned(()=>processes.requireSuccess(signaled.value)),1313,{code:-1n,signal:"SIGTERM"});
});
test("stdin reaches the child; streams drain concurrently past pipe buffers",async()=>{
 const upper=await owned(()=>processes.run("/usr/bin/tr",["a-z","A-Z"],options({stdin:new TextEncoder().encode("hello")})));
 expect(upper.kind).toBe("ok");if(upper.kind!=="ok")throw Error();
 expect(textOf(upper.value,"stdout")).toBe("HELLO");
 const big=await owned(()=>processes.run("/bin/sh",["-c","head -c 200000 /dev/zero | tr '\\0' 'x'; echo marker >&2"],options()));
 expect(big.kind).toBe("ok");if(big.kind!=="ok")throw Error();
...
test("arguments stay literal without a shell",async()=>{
 await withTemp(async root=>{
  ...
  const result=await owned(()=>processes.run("/bin/echo",["$(touch "+sentinel+")","a;b","`id`"],options()));
  expect(result.kind).toBe("ok");if(result.kind!=="ok")throw Error();
...
test("missing executables and denied directories are declared failures",async()=>{
 await withTemp(async root=>{
  check(await owned(()=>processes.run("/nonexistent-bin-xyz",[],options())),1300,{path:"/nonexistent-bin-xyz"});
  check(await owned(()=>processes.which("definitely-not-a-binary-xyz")),1300,{path:"definitely-not-a-binary-xyz"});
  ...
  const locked=join(root,"locked");await mkdir(locked);
  await chmod(locked,0o000);
  try{
   check(await owned(()=>processes.run("/bin/echo",["x"],options({cwd:locked}))),1301,{operation:"spawn"});
  }finally{await chmod(locked,0o700);}
  check(await owned(()=>processes.run("/bin/echo",["x"],options({cwd:join(root,"nodir")}))),1300,{path:join(root,"nodir")});
 });
});
test("invalid configs fail before spawning",async()=>{
 await withTemp(async root=>{
  const sentinel=join(root,"spawned");
  check(await owned(()=>processes.run("",[],options())),1314,{field:"executable",reason:"empty"});
  check(await owned(()=>processes.run("a\0b",[],options())),1314,{field:"executable",reason:"nul_byte"});
  check(await owned(()=>processes.run("/bin/echo",["a\0b"],options())),1314,{field:"args",reason:"nul_byte"});
  check(await owned(()=>processes.run("/bin/echo",[],options({env:["NOEQUALS"]}))),1314,{field:"env",reason:"entry"});
  check(await owned(()=>processes.run("/bin/echo",[],options({env:["1BAD=x"]}))),1314,{field:"env",reason:"name"});
  check(await owned(()=>processes.run("/bin/echo",[],options({env:["K=a\0b"]}))),1314,{field:"env",reason:"nul_byte"});
  check(await owned(()=>processes.run("/bin/echo",[],options({out:-1n}))),1314,{field:"stdout_limit",reason:"negative"});
  check(await owned(()=>processes.run("/bin/echo",[],options({deadline:-1n}))),1314,{field:"deadline_ms",reason:"negative"});
  check(await owned(()=>processes.run("/bin/echo",[],options({deadline:2147483648n}))),1314,{field:"deadline_ms",reason:"too_large"});
  check(await owned(()=>processes.run("/usr/bin/touch",[sentinel],options({env:["BAD"]}))),1314,{field:"env",reason:"entry"});
  expect(await Bun.file(sentinel).exists()).toBe(false);
 });
});
test("output caps terminate runaway producers",async()=>{
 const flood=await owned(()=>processes.run("/bin/sh",["-c","yes | head -c 1000000"],options({out:1000n,grace:50n})));
...
test("deadlines terminate and reap, escalating past SIGTERM traps",async()=>{
 ...
 const result=await owned(()=>processes.run("/bin/sleep",["30"],options({deadline:300n,grace:50n})));
 check(result,1311,{deadline_ms:300n});
 expect(Date.now()-started).toBeLessThan(10000);
 const trapped=await owned(()=>processes.run("/bin/sh",["-c","trap '' TERM; sleep 30"],options({deadline:300n,grace:100n})));
...
test("group termination reaches grandchildren",async()=>{
 await withTemp(async root=>{
  ...
  const result=await owned(()=>processes.run("/bin/sh",["-c","sleep 60 & echo $! > "+pidfile+"; wait"],options({deadline:500n,grace:50n})));
...
test("environment inheritance is explicit; cwd applies",async()=>{
 await withTemp(async root=>{
  const bare=await owned(()=>processes.run("/usr/bin/env",[],options({env:["CAN_MARK=yes"]})));
  expect(bare.kind).toBe("ok");if(bare.kind!=="ok")throw Error();
  const plain=textOf(bare.value,"stdout");
  expect(plain).toContain("CAN_MARK=yes\n");
  expect(plain).not.toContain("PATH=");
  const inherited=await owned(()=>processes.run("/usr/bin/env",[],options({inherit:true,env:["CAN_MARK=yes"]})));
  expect(inherited.kind).toBe("ok");if(inherited.kind!=="ok")throw Error();
  ...
  expect(where.kind).toBe("ok");if(where.kind!=="ok")throw Error();
...
test("unsupplied assertions cannot spawn",async()=>{
 await withTemp(async root=>{
  ...
  for(const [name,actual] of [
   ...
   const result=await runAssertion({root:{package:"test",declaration:"boundary",name},actual:actual as never,expected:async()=>success(undefined)});
   expect(result.passed).toBe(false);
  }
  expect(await Bun.file(ghost).exists()).toBe(false);
  const good=record(resultId,[["stdout",ownBytes(new Uint8Array())],["stderr",ownBytes(new Uint8Array())],["code",0n],["signal",""]]);
  const pure=await runAssertion({root:{package:"test",declaration:"boundary",name:"require"},actual:()=>processes.requireSuccess(good),expected:async()=>success(good)});
...
test("unknown spawn failures stay standard; classification is exact",async()=>{
 ...
 expect(classifySpawnError(Object.assign(new Error("x"),{code:"ENOENT"}))).toEqual({kind:"not_found",code:"ENOENT"});
 expect(classifySpawnError(Object.assign(new Error("x"),{code:"EACCES"}))).toEqual({kind:"denied",code:"EACCES"});
 expect(classifySpawnError(Object.assign(new Error("x"),{code:"ETXTBSY"}))).toEqual({kind:"failed",code:"ETXTBSY"});
...
test("abandoned runs die with their scope",async()=>{
 ...
 await runOwnedRoot(()=>withScope(async()=>{
  const pending=processes.run("/bin/sleep",["30"],options({grace:50n}));
  void pending.then(completion=>{observed=completion;},cause=>{observed=cause as Completion<unknown>;});
