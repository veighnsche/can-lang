...
const domainDetails=(result:Completion)=>{expect(result.kind).toBe("domain");if(result.kind!=="domain")throw Error("expected domain failure");return domainFailureDiagnostics(result.value);};
...
test("native transport failures carry their producing operation",async()=>{
 ...
 const root=await runOwnedRoot(async()=>{
  const failed=await api.request(connection,request,()=>success(0n),origin,"test:provenance/fetch-a");
  const details=domainDetails(failed);
  expect(details.declaration.id).toBe(1102);
  expect(details.provenance).toEqual({boundary:"native",operation:"test:provenance/fetch-a"});
...
test("native and authored codec failures with identical payloads stay distinguishable",async()=>{
 ...
 const root=await runOwnedRoot(async()=>{
  ...
  expect(key(native.provenance.boundary,native.declaration.name)).not.toBe(key(authoredDetails.provenance.boundary,authoredDetails.declaration.name));
...
test("unknown native exceptions keep the standard channel and are never relabelled",async()=>{
 const api=createTransport(domain,{...httpTypes,header:header.identity},()=>undefined);
 const root=await runOwnedRoot(async()=>{
  const defect=await api.request(connection,{...request,exchange:async()=>{throw Error("adapter defect");}},()=>success(0n),origin,"test:provenance/fetch-c");
...
test("cause and occurrence identity survive forwarding while equal reconstructions differ",()=>{
 const cause=domain.create(ids.transport,record(ids.transport,[["phase","connect"]]),origin,undefined,{boundary:"native",operation:"test:provenance/fetch-d"});
 ...
 const same=domain.create(ids.transport,record(ids.transport,[["phase","connect"]]),origin,undefined,{boundary:"native",operation:"test:provenance/fetch-d"});
 expect(domainFailureDiagnostics(same).occurrenceID).not.toBe(first.occurrenceID);
 expect(same).not.toBe(cause);
 const wrapped=domain.create(ids.invalid,record(ids.invalid,[["reason","url"]]),origin,cause,{boundary:"native",operation:"test:provenance/fetch-d"});
...
test("provenance validation rejects unattributed native and malformed tags",()=>{
 ...
 expect(domainFailureDiagnostics(domain.create(ids.transport,payload,origin)).provenance).toEqual({boundary:"emitted",operation:""});
 for(const provenance of [{boundary:"native",operation:""},{boundary:"bogus",operation:"x"},{boundary:"native",operation:7},"native",null,42]) {
  expect(()=>domain.create(ids.transport,payload,origin,undefined,provenance)).toThrow(TypeError);
 }
});
