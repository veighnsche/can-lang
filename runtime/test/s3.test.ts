...
import {createS3,isS3Value,S3_CLIENT_KIND,S3_UPLOAD_KIND,S3_PRESIGNED_KIND,S3_CONTINUATION_KIND} from "../platform/s3.ts";
...
const decls=catalogue.errors.filter(e=>[1305,1316,1318,1319,1329,1330,1331,1332,1343,1344,1345,1346,1347,1348].includes(e.id));
const declarations=decls.map(e=>({...e,parameters:0}));
const errors=decls.map(e=>shape("error",e.identity,e.fields.map(f=>({name:f.name,type:f.type==="int"?int.identity:str.identity}))));
...
const metaOf=async(meta:unknown)=>({size:dataProperty(meta,"size") as bigint,etag:dataProperty(meta,"etag") as string,type:dataProperty(meta,"content_type") as string,modified:value(await times.instantEpochMillis(dataProperty(meta,"last_modified"))) as bigint});
async function owned(body:()=>Promise<void>):Promise<void>{
  ...
  if(out.completion.kind!=="ok"||out.cleanupFailed){
    const cause=out.completion.kind==="standard"&&isStandardFailure(out.completion.value)?standardFailureDiagnostics(out.completion.value as never).cause as {message?:unknown;expected?:unknown;actual?:unknown;diff?:unknown;stack?:unknown}:undefined;
    const detail=out.completion.kind==="domain"?JSON.stringify(domainFailureDiagnostics(out.completion.value).payload):cause!==undefined?JSON.stringify({message:String(cause.message),stack:String(cause.stack??"").slice(0,600)}):out.completion.kind;
...
const ENDPOINT=process.env["CAN_TEST_S3_ENDPOINT"],ACCESS=process.env["CAN_TEST_S3_ACCESS_KEY"],SECRET=process.env["CAN_TEST_S3_SECRET_KEY"];
...
const openLive=async()=>value(await s3.clientOpen(ENDPOINT!,"us-east-1",BUCKET,ACCESS!,SECRET!)) as object;
const refusedEndpoint=()=>{const match=ENDPOINT!.match(/^(https?:\/\/[^:]+)(:\d+)?(.*)$/);return match![1]+":9"+(match![3]??"");};
    ...
    try{await body(keys);}
      ...
      if(keys.length>0){
        try{
          ...
          for(const key of keys){try{value(await s3.remove(client,key));}catch{/* best effort */}}
        }catch{/* best effort */}
      }
    }
  });
...
const byteReader=(chunks:Uint8Array[]):object=>{
  ...
  return registerReader(openByteCell(stream,65536n),fail,identity("can.std.stream@1::close_failed"),{scopeManaged:true});
};
test("client_open validates endpoint, region, bucket and credentials",async()=>{
  await owned(async()=>{
    check(await s3.clientOpen("not a url","us-east-1","b","a","s"),1343,{reason:"endpoint"});
    ...
    check(await s3.clientOpen("http://127.0.0.1:9","us-east-1","","a","s"),1343,{reason:"bucket"});
    check(await s3.clientOpen("http://127.0.0.1:9","us-east-1","b","","s"),1343,{reason:"credentials"});
    check(await s3.clientOpen("http://127.0.0.1:9","us-east-1","b","a",""),1343,{reason:"credentials"});
...
test("presign mints locally and describe reveals method and expiry",async()=>{
  await owned(async()=>{
    ...
    for(const leaf of [METHOD_GET,METHOD_PUT,METHOD_DELETE,METHOD_HEAD] as const){
      const token=value(await s3.presign(client,record(leaf,[]),"dir/obj.bin",300n,none())) as object;
      ...
      const expires=value(await times.instantEpochMillis(dataProperty(info,"expires_at"))) as bigint;
      const now=BigInt(Date.now());
      expect(expires-now>290000n&&expires-now<=300000n).toBe(true);
    }
    let thrown:unknown;
    try{value(await s3.describe(Object.freeze({})));}catch(cause){thrown=cause;}
...
test("upload terminal states reject reuse without wire calls",async()=>{
  await owned(async()=>{
    ...
    check(await s3.uploadWrite(upload,buf("x")),1347,{operation:"upload_write",state:"cancelled"});
    ...
    let thrown:unknown;
    try{value(await s3.uploadWrite(Object.freeze({}),buf("x")));}catch(cause){thrown=cause;}
    expect(thrown instanceof TypeError).toBe(true);
  });
});
test("write_stream rejects non-bytes readers before uploading",async()=>{
  await owned(async()=>{
    const client=value(await s3.clientOpen("http://127.0.0.1:9","us-east-1","b","a","s")) as object;
    const stream=new ReadableStream<Uint8Array>({start(c){c.enqueue(new TextEncoder().encode("hi\n"));c.close();}});
    const lines=registerReader(openLineCell(stream,1024n),fail,identity("can.std.stream@1::close_failed"),{scopeManaged:true});
    check(await s3.writeStream(client,"k",lines,writeOpts(none()),1024n,5000n),1343,{reason:"reader"});
    check(await s3.writeStream(client,"",byteReader([]),writeOpts(none()),1024n,5000n),1343,{reason:"key"});
    check(await s3.writeStream(client,"k",byteReader([]),writeOpts(none()),0n,5000n),1343,{reason:"max_bytes"});
    check(await s3.writeStream(client,"k",byteReader([]),writeOpts(none()),1024n,0n),1343,{reason:"deadline"});
  });
});
test("supplied operations deny the live assertion boundary",async()=>{
  await owned(async()=>{
    const client=value(await s3.clientOpen("http://127.0.0.1:9","us-east-1","b","a","s")) as object;
    const context=assertionContext({package:"can.project.root/app",declaration:"can.project.root/app::probe",name:"sample"});
    try{
      for(const run of [
        ...
        let thrown:unknown;
        try{await run();}catch(cause){thrown=cause;}
        expect(thrown).toBeDefined();
      }
      // Real operations run under assertion context.
      const local=value(await s3.clientOpen("http://127.0.0.1:9","us-east-1","b","a","s",context)) as object;
      expect(isS3Value(S3_CLIENT_KIND,local)).toBe(true);
      const token=value(await s3.presign(client,record(METHOD_GET,[]),"k",60n,none())) as object;
...
live("read_range serves exact windows and pins range faults",async()=>{
  await liveOwned(async(keys)=>{
    ...
    check(await s3.readRange(client,key,100n,10n),1346,{code:"InvalidRange",operation:"read_range"});
    check(await s3.readRange(client,PREFIX+"absent.bin",0n,4n),1344,{key:PREFIX+"absent.bin"});
  });
});
live("read_stream reassembles bytes and cancels terminally",async()=>{
  await liveOwned(async(keys)=>{
    ...
    let thrown:unknown;
    try{value(await reads.readMany(token2,1n));}catch(cause){thrown=cause;}
...
live("write_stream pumps readers under byte budgets",async()=>{
  await liveOwned(async(keys)=>{
    ...
    keys.push(overKey);
    check(await s3.writeStream(client,overKey,byteReader([new TextEncoder().encode("abcdef")]),writeOpts(none()),4n,30000n),1348,{limit:4n,size:6n});
    await settled();
...
live("write_stream cancels the upload when the reader fails",async()=>{
  await liveOwned(async(keys)=>{
    ...
    keys.push(key);
    const stream=new ReadableStream<Uint8Array>({pull(){throw Object.assign(new Error("reader boom"),{code:"reader_boom"});}});
    const reader=registerReader(openByteCell(stream,65536n),fail,identity("can.std.stream@1::close_failed"),{scopeManaged:true});
    check(await s3.writeStream(client,key,reader,writeOpts(none()),1024n,30000n),1316,{reason:"reader_boom"});
    await settled();
    expect(value(await s3.exists(client,key))).toBe(false);
  });
});
live("write_stream fails timeout past the deadline",async()=>{
  await liveOwned(async(keys)=>{
    ...
    keys.push(key);
    let pulls=0;
    const stream=new ReadableStream<Uint8Array>({pull:async(controller)=>{pulls++;await new Promise(resolve=>setTimeout(resolve,500));controller.enqueue(new TextEncoder().encode("x"));}});
    const reader=registerReader(openByteCell(stream,65536n),fail,identity("can.std.stream@1::close_failed"),{scopeManaged:true});
    check(await s3.writeStream(client,key,reader,writeOpts(none()),1024n,50n),1346,{code:"timeout",operation:"write_stream"});
...
live("multipart upload finishes, guards terminals and abandons cleanly",async()=>{
  await liveOwned(async(keys)=>{
    ...
    check(await s3.uploadWrite(upload,ownBytes(new Uint8Array([1]))),1347,{operation:"upload_write",state:"finished"});
    ...
    keys.push(emptyKey);
    const emptyMeta=await metaOf(value(await s3.uploadFinish(value(await s3.beginUpload(client,emptyKey,uploadOpts(none(),none()))) as object)));
    expect(emptyMeta.size).toBe(0n);
    const dropKey=PREFIX+"multi-drop.bin";
    keys.push(dropKey);
    const dropped=value(await s3.beginUpload(client,dropKey,uploadOpts(none(),none()))) as object;
...
live("list pages, continues and groups with opaque tokens",async()=>{
  await liveOwned(async(keys)=>{
    ...
    expect(value(await times.instantEpochMillis(dataProperty(entry,"last_modified"))) as bigint>0n).toBe(true);
...
live("presigned urls roundtrip and fail closed on misuse",async()=>{
  await liveOwned(async(keys)=>{
    ...
    const urlOf=async(method:string,k:string,expires:bigint)=>{
      const token=value(await s3.presign(client,record(method,[]),k,expires,none())) as object;
    ...
    keys.push(putKey);
    const put=await fetch(await urlOf(METHOD_PUT,putKey,300n),{method:"PUT",body:"via-url"});
    ...
    expect((await fetch(getUrl.replace(/X-Amz-Signature=[^&]+/,"X-Amz-Signature=00"))).status).toBe(403);
    const delKey=PREFIX+"signed-del.bin";
    keys.push(delKey);
    value(await s3.writeBytes(client,delKey,buf("d"),writeOpts(none())));
    expect((await fetch(await urlOf(METHOD_DELETE,delKey,300n),{method:"DELETE"})).status).toBe(204);
    ...
    expect((await fetch(await urlOf(METHOD_GET,PREFIX+"absent-signed.bin",300n))).status).toBe(404);
  });
});
live("denied credentials, refused endpoints and missing keys map exactly",async()=>{
  await liveOwned(async(keys)=>{
    const client=await openLive();
    const bad=value(await s3.clientOpen(ENDPOINT!,"us-east-1",BUCKET,"nope","nope")) as object;
    ...
    const refused=value(await s3.clientOpen(refusedEndpoint(),"us-east-1",BUCKET,ACCESS!,SECRET!)) as object;
    check(await s3.stat(refused,"k"),1346,{code:"ConnectionRefused",operation:"stat"});
    check(await s3.readBytes(refused,"k",8n),1346,{code:"ConnectionRefused",operation:"read_bytes"});
    check(await s3.stat(client,PREFIX+"absent.bin"),1344,{key:PREFIX+"absent.bin"});
    check(await s3.readBytes(client,PREFIX+"absent.bin",8n),1344,{key:PREFIX+"absent.bin"});
    expect(value(await s3.exists(client,PREFIX+"absent.bin"))).toBe(false);
    value(await s3.remove(client,PREFIX+"absent.bin"));
    const noBucket=value(await s3.clientOpen(ENDPOINT!,"us-east-1","no-such-bucket-zzz",ACCESS!,SECRET!)) as object;
    check(await s3.stat(noBucket,"k"),1344,{key:"k"});
    check(await s3.list(noBucket,listOpts("",5n,none(),none())),1346,{code:"NoSuchBucket",operation:"list"});
    expect(keys).toEqual([]);
  });
});
