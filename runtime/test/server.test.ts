...
async function standardKind(call: () => Promise<Completion<unknown>>): Promise<string> {
  ...
  if (completion.kind !== "standard") throw new Error("wrong outcome");
...
test("config validation rejects out-of-range numbers with field reasons", async () => {
  expect(
    domainOutcome(
      await server.makeConfig("127.0.0.1", 8080n, 0n, 1000n),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "body_limit" });
  expect(
    domainOutcome(
      await server.makeConfig("127.0.0.1", 8080n, 67108865n, 1000n),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "body_limit" });
  expect(
    domainOutcome(
      await server.makeConfig("127.0.0.1", 8080n, 1024n, -1n),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "shutdown_ms" });
  expect(
    domainOutcome(
      await server.makeConfig("127.0.0.1", 8080n, 1024n, 2147483648n),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "shutdown_ms" });
  for (const [limit, shutdown] of [
    [1n, 0n],
    [67108864n, 2147483647n],
    [1024n, 5000n],
  ] as const) {
...
test("bind failures echo the attempted address", async () => {
  const owned = await runOwnedRoot(async () => {
    ...
    expect(domainOutcome(await server.start(config, table), "http::bind_failed")).toMatchObject({
      address: "127.0.0.1:18341",
    });
    const badPort = value(await server.makeConfig("127.0.0.1", 70000n, 1024n, 1000n));
    expect(domainOutcome(await server.start(badPort, table), "http::bind_failed")).toMatchObject({
      address: "127.0.0.1:70000",
    });
    const huge = value(await server.makeConfig("127.0.0.1", 2n ** 100n, 1024n, 1000n));
    expect(domainOutcome(await server.start(huge, table), "http::bind_failed")).toMatchObject({
      address: "127.0.0.1:" + (2n ** 100n).toString(),
    });
    const badHost = value(await server.makeConfig("a\0b", 18342n, 1024n, 1000n));
    expect(domainOutcome(await server.start(badHost, table), "http::bind_failed")).toMatchObject({
      address: "a\0b:18342",
    });
    expect((await server.stop(first.token)).kind).toBe("ok");
    return success(undefined);
...
test("serves routed requests with keep-alive reuse", async () => {
  const owned = await runOwnedRoot(async () => {
    ...
    for (let round = 0; round < 3; round++) {
      ...
      expect(await got.text()).toBe("got");
...
test("rejects over-limit bodies with 413", async () => {
  const owned = await runOwnedRoot(async () => {
    ...
    const tooLarge = await fetch("http://127.0.0.1:18345/x", {
      method: "POST",
      body: "nine-bytes",
    });
    ...
    expect(await exact.text()).toBe("small");
...
test("dispatch failures become sanitized 500s and release leases", async () => {
  ...
  const owned = await runOwnedRoot(async () => {
    ...
    expect(body).not.toContain("plum-falcon");
...
test("stop drains delayed handlers before settling", async () => {
  const owned = await runOwnedRoot(async () => {
    const { token } = await startOn(18347, async () => {
      await Bun.sleep(150);
  ...
  await withoutUnhandled(async () => {
    ...
    const owned = await runOwnedRoot(
      async () => {
        ...
        expect(domainOutcome(stopped, "http::shutdown_failed")).toMatchObject({
          phase: "deadline",
        });
    ...
    expect(diagnostics[0]).toMatchObject({ phase: "cleanup" });
  });
});
...
test("simultaneous stop callers split ok and resource_state", async () => {
  const owned = await runOwnedRoot(async () => {
    ...
    const [first, second] = await Promise.all([
      invoke(() => server.stop(token), origin),
      invoke(() => server.stop(token), origin),
    ]);
...
test("early-winning coordination keeps a late server waiter", async () => {
  await withoutUnhandled(async () => {
    const owned = await runOwnedRoot(async () => {
      ...
      if (late.kind !== "ok") throw new Error("wrong outcome");
...
for (const [signal, port] of [
  ["SIGINT", 18355],
  ["SIGTERM", 18356],
] as const)
  test(`signals settle wait gracefully in a child (${signal})`, async () => {
    const script =
      childPrelude([
        "http::invalid_server_config",
        "http::bind_failed",
...
await runOwnedRoot(async ()=>{
 ...
 return success(undefined);
    ...
    expect(result.output).toContain("settled:ok");
...
await runOwnedRoot(async ()=>{
 ...
 return success(undefined);
  ...
  expect(result.output).toContain("listening");
  expect(result.output).toContain("stop:domain");
  expect(result.output).toContain("deadline");
  expect(result.output).toContain("wait-pending");
  expect(result.output).not.toContain("settled-late");
  expect(result.errors).toBe("");
...
test("TLS config validates PEM structure before serving", async () => {
  ...
  expect((await server.makeTlsConfig(tlsBytes(tlsCert + tlsCert), tlsBytes(tlsKey))).kind).toBe(
    "ok",
  );
  for (const key of [
  ...
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes("garbage"), tlsBytes(tlsKey)),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_cert" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(tlsCert), tlsBytes("garbage")),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_key" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(tlsKey), tlsBytes(tlsKey)),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_cert" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(tlsCert), tlsBytes(tlsCert)),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_key" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(""), tlsBytes(tlsKey)),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_cert" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(tlsCert), tlsBytes(tlsKey + tlsKey)),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_key" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(
        tlsBytes("-----BEGIN CERTIFICATE-----\nAA==\n-----END X-----\n"),
        tlsBytes(tlsKey),
      ),
      "http::invalid_server_config",
    ),
  ...
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(tlsCert), ownBytes(new Uint8Array([255, 254]))),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_key" });
  expect(
    domainOutcome(
      await server.makeTlsConfig(tlsBytes(tlsCert), tlsBytes("x".repeat(1048577))),
      "http::invalid_server_config",
    ),
  ).toMatchObject({ reason: "tls_key" });
});
...
function localChain(dir: string): { cert: Uint8Array; key: Uint8Array } {
  ...
  if (generated.exitCode !== 0)
    throw new Error("openssl chain failed: " + generated.stderr.toString());
  return {
    cert: new Uint8Array(readFileSync(certPath)),
...
test("TLS serves a generated chain and rejects untrusted clients", async () => {
  ...
  try {
    ...
    const owned = await runOwnedRoot(async () => {
      ...
      const trusted = await fetch("https://127.0.0.1:18358/x", {
        tls: { ca: cert },
      } as RequestInit);
      expect(trusted.status).toBe(200);
      expect(await trusted.text()).toBe("secure");
...
test("TLS start maps unparseable DER to bind failures", async () => {
  const owned = await runOwnedRoot(async () => {
    ...
    const table = value(await router.make([route]));
    expect(
      domainOutcome(await server.startTls(config, table, tls), "http::bind_failed"),
    ).toMatchObject({ address: "127.0.0.1:18359" });
    return success(undefined);
  });
  expect(owned.cleanupFailed).toBe(false);
  expect(owned.completion.kind).toBe("ok");
...
test("stream routes read incrementally under per-request scope", async () => {
  const seen: { status: number; head: string; leases: number } = {
    status: 0,
    head: "",
    leases: -1,
  };
  const owned = await runOwnedRoot(async () => {
    ...
    const upload = value(
      await router.stream(
        value(
          await router.post("/u", async (request) => {
    ...
    const skip = value(
      await router.stream(value(await router.post("/skip", async () => textResult("skipped")))),
    );
    const table = value(await router.make([upload, skip]));
    const token = value(await server.start(config, table));
    const streamed = await fetch("http://127.0.0.1:18360/u", {
      method: "POST",
      body: "hello-world",
    });
    seen.status = streamed.status;
    seen.head = await streamed.text();
    const ignored = await fetch("http://127.0.0.1:18360/skip", {
      method: "POST",
      body: "x".repeat(1000),
    });
    if (ignored.status !== 200 || (await ignored.text()) !== "skipped")
      throw new Error("abandoned body broke the route");
    const again = await fetch("http://127.0.0.1:18360/u", { method: "POST", body: "abcd" });
    if (again.status !== 200 || (await again.text()) !== "abcd")
      throw new Error("connection reuse broke after abandon");
...
test("dropped connections abort live reads", async () => {
  ...
  const owned = await runOwnedRoot(async () => {
    ...
    const settled = fetch("http://127.0.0.1:18361/u", {
      method: "POST",
      body,
      signal: controller.signal,
      duplex: "half",
    } as RequestInit & { duplex: "half" }).then(
