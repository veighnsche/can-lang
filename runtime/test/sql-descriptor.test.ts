...
describe("sql descriptors", () => {
  test("declare and template a checked descriptor", () => {
    ...
    expect([...(descriptor.strings.raw ?? [])]).toEqual(["SELECT id FROM accounts WHERE x = ", " LIMIT ", ""]);
    ...
    for (let i = 0; i < built.values.length; i++) text += `$${descriptor.numbers[i]}${built.strings[i + 1]}`;
  ...
  test("unknown names, forged values, and arity breaks throw", () => {
    ...
    expect(() => sql.template(descriptor, "nope" as unknown as unknown[])).toThrow("invalid sql values");
  });
