...
const table: Record<string, Record<string, SQLDescriptorEntry>> = {
  "": {
    account_by_id: {
      ...
      kind: "SelectStmt",
      segments: [
        { text: "SELECT id, display_name FROM t WHERE id = " },
        { param: 1 },
        { text: " LIMIT " },
        { param: 2 },
      ],
    ...
    accounts_by_term: {
      ...
      kind: "SelectStmt",
      segments: [
        { text: "SELECT id, display_name FROM t WHERE display_name ILIKE " },
        { param: 1 },
        { text: " LIMIT " },
        { param: 2 },
      ],
...
function installFake(hooks?: { connect?: () => Promise<void> }): {
  constructed: { url: unknown; opts: unknown }[];
  clients: FakeHandle[];
} {
  ...
  (Bun as unknown as { SQL: unknown }).SQL = function (url: unknown, opts: unknown) {
    ...
    Object.assign(callable, {
      ...
      close: async (options?: unknown) => {
        handle.closeCalls.push(options);
        return handle.close(options);
      },
    });
    clients.push(handle);
    return callable;
...
describe("sql pools", () => {
  test("open reads the named credential once and constructs a postgres client", async () => {
    ...
    try {
      ...
      expect(fake.constructed).toEqual([
        { url: "postgres://fake/x", opts: { adapter: "postgres", bigint: true, max: 5 } },
      ]);
  ...
  test("open rejects bad names, missing credentials, and bad config", async () => {
    ...
    try {
      env.clear();
      await owned(async () => {
        expect(domainOutcome(await pools.open("lower", 5n), "http::credentials_missing")).toEqual({
          variable: "lower",
        });
        expect(
          domainOutcome(await pools.open("CAN_MISSING", 5n), "http::credentials_missing"),
        ).toEqual({ variable: "CAN_MISSING" });
      });
      env.set("CAN_TEST_POSTGRES", "postgres://fake/x");
      await owned(async () => {
        expect(
          domainOutcome(await pools.open("CAN_TEST_POSTGRES", 0n), "sql::connection_failed"),
        ).toEqual({ phase: "config" });
        expect(
          domainOutcome(
            await pools.open("CAN_TEST_POSTGRES", 2147483648n),
  ...
  test("open surfaces refused connections and cleans up the client", async () => {
    const fake = installFake({
      connect: async () => {
        throw postgresError("ERR_POSTGRES_CONNECTION_REFUSED");
      },
    });
    try {
      env.set("CAN_TEST_POSTGRES", "postgres://fake/x");
      await owned(async () => {
        expect(
          domainOutcome(await pools.open("CAN_TEST_POSTGRES", 5n), "sql::connection_failed"),
      ...
      await owned(async () => {
        ...
        await expect(
          pools.execute(
            ...
            termParams,
            token,
            record("app::search_parameters", [["term", "x"]]),
            context,
          ),
        ).rejects.toThrow();
...
describe("sql parameters", () => {
  async function withPool(
    run: (token: unknown, client: FakeHandle) => Promise<void>,
  ): Promise<FakeHandle[]> {
    const fake = installFake();
    try {
      env.set("CAN_TEST_POSTGRES", "postgres://fake/x");
      await owned(async () => {
        const token = value(await pools.open("CAN_TEST_POSTGRES", 5n));
        try {
          await run(token, fake.clients[0]!);
        } finally {
          value(await pools.close(token, 1000n));
        }
      });
  ...
  test("int range and unicode scalars validate before launch", async () => {
    await withPool(async (token, client) => {
      ...
      expect(
        domainOutcome(await pools.queryOne(byId, idParams, token, over), "sql::unsupported_value"),
      ).toEqual({ path: "/id", reason: "int_range" });
      const lone = record("app::search_parameters", [["term", "\ud800"]]);
      expect(
        domainOutcome(
          await pools.queryRows(byTerm, termParams, token, lone, 10n),
      ...
      expect(
        domainOutcome(
          await pools.queryOne(byId, idParams, token, mistyped),
          "sql::unsupported_value",
        ),
  ...
  test("options encode to null or the inner scalar", async () => {
    ...
    await withPool(async (token, client) => {
      ...
      expect(
        domainOutcome(await pools.queryOne(byId, plan, token, forged), "sql::unsupported_value"),
  ...
  test("hostile text travels only as a bound parameter", async () => {
    await withPool(async (token, client) => {
      ...
      expect(call.strings).toEqual([
        "SELECT id, display_name FROM t WHERE display_name ILIKE ",
        " LIMIT ",
        "",
      ]);
    });
  ...
  test("bytes parameters are copied before launch", async () => {
    ...
    await withPool(async (token, client) => {
      let sent: unknown;
      client.query = async (_strings, values) => {
        sent = values[0];
        return [{ id: 1n, display_name: "n" }];
      };
  ...
  test("null, unsafe numbers, and binary decode by rule", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        expect(
          domainOutcome(
            await pools.queryOne(byId, coverPlan, token, params),
            "sql::schema_mismatch",
          ),
        ...
        expect(dataProperty(value(await pools.queryOne(byId, idParams, token, params)), "id")).toBe(
          7n,
        );
  ...
  test("decoded binary is an owned copy", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        const row = value(
          await pools.queryOne(byId, coverPlan, token, record("app::id_parameters", [["id", 1n]])),
        );
        backing[0] = 90;
  ...
  test("optional decodes none, some, and observed duplicates", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        client.query = async () => [];
        expect(value(await pools.queryOptional(byTerm, termParams, token, params))).toEqual(
          record("app::option_none", []),
        );
        client.query = async () => [
          { id: 4n, display_name: "Bob" },
          { id: 5n, display_name: "Bob" },
        ];
        expect(
          domainOutcome(
            await pools.queryOptional(byTerm, termParams, token, params),
            "sql::row_count",
          ),
  ...
  test("cardinality counts only observed bounded rows", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        client.query = async () => [];
        expect(
          domainOutcome(await pools.queryOne(byId, idParams, token, params), "sql::row_missing"),
        ).toEqual({ query: "account_by_id" });
        client.query = async () => [
          { id: 4n, display_name: "a" },
          { id: 4n, display_name: "b" },
        ];
        expect(
          domainOutcome(await pools.queryOne(byId, idParams, token, params), "sql::row_count"),
  ...
  test("query_rows binds max_rows+1 and enforces the exact bound", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        client.query = async () => [
          { id: 2n, display_name: "Jason" },
          { id: 3n, display_name: "Mason" },
        ];
        ...
        client.query = async () => [
          { id: 2n, display_name: "a" },
          { id: 3n, display_name: "b" },
          { id: 4n, display_name: "c" },
        ];
        expect(
          domainOutcome(
            await pools.queryRows(byTerm, termParams, token, params, 2n),
            "sql::row_limit",
          ),
        ).toEqual({ limit: 2n });
        expect(
          domainOutcome(
            await pools.queryRows(byTerm, termParams, token, params, -1n),
            "sql::unsupported_value",
          ),
        ).toEqual({ path: "max_rows", reason: "negative" });
        expect(
          domainOutcome(
            await pools.queryRows(byTerm, termParams, token, params, (1n << 63n) - 1n),
            "sql::unsupported_value",
          ),
  ...
  test("execute validates affected counts and binds no limit", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        expect(
          domainOutcome(
            await pools.execute(addAccount, termParams, token, params),
            "sql::query_failed",
          ),
  ...
  test("native failures classify finitely with sanitized payloads", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        client.query = async () => {
          throw postgresError("query", { errno: "23505", constraint: "accounts_name_key" });
        };
        expect(
          domainOutcome(
            await pools.execute(
              ...
              termParams,
              token,
              record("app::search_parameters", [["term", "Zed"]]),
            ),
            "sql::constraint_failed",
          ),
        ).toEqual({ constraint: "accounts_name_key" });
        client.query = async () => {
          throw postgresError("query", { errno: "23503" });
        };
        expect(
          domainOutcome(
            await pools.queryOne(byId, idParams, token, params),
        ...
        client.query = async () => {
          throw postgresError("query", { errno: "42P01" });
        };
  ...
  test("close denies new work, drains the lease, and keeps closing on timeout", async () => {
    ...
    try {
      ...
      const result = await runOwnedRoot(async () => {
        ...
        client.query = async () => {
          await gate;
          return [{ id: 4n, display_name: "Bob" }];
        };
        const params = record("app::id_parameters", [["id", 4n]]);
        const flight = pools.queryOne(byId, idParams, token, params);
        while (client.calls.length === 0) await Bun.sleep(0);
        expect(domainOutcome(await pools.close(token, 20n), "sql::close_failed")).toEqual({
          reason: "timeout",
        });
        ...
        while (resourceStatus(token).state !== "closed" && Date.now() - begin < 5000)
          await Bun.sleep(10);
        expect(resourceStatus(token).state).toBe("closed");
