...
import {createStreamReads,drainStream,openByteCell,openLineCell} from "../transport/stream/readable.ts";
...
const declarations=catalogue.errors.filter(e=>[1300,1301,1303,1304,1305,1308,1316,1317,1318,1319].includes(e.id));
...
const writes=createStreamWrites(domain,{writeFailed:identity("can.std.stream@1::write_failed"),closeFailed:identity("can.std.stream@1::close_failed")});
const files=createFileStreams(domain,{notFound:identity("can.std.files@1::not_found"),denied:identity("can.std.files@1::denied"),invalidPath:identity("can.std.files@1::invalid_path"),unexpectedKind:identity("can.std.files@1::unexpected_kind"),limitExceeded:identity("can.std.files@1::limit_exceeded"),ioError:identity("can.std.files@1::io_error"),closeFailed:identity("can.std.stream@1::close_failed")});
...
test("bytes write/read preserves order across chunks with exact end",async()=>{
  await withTemp(async root=>{
    ...
    const result=await runOwnedRoot(async()=>{
      ...
      expect(await writes.writeSome(writer,ownBytes(new Uint8Array([1,2,3])))).toEqual(success(3n));
...
test("lines frame CRLF, trailing segments and split UTF-8; malformed input fails",async()=>{
  await withTemp(async root=>{
    ...
    const split=new ReadableStream<Uint8Array>({start(c){c.enqueue(new Uint8Array([0xe2]));c.enqueue(new Uint8Array([0x82,0xac,0x0a]));c.close();}});
    const forced=await runOwnedRoot(async()=>{
      const token=registerReader(openLineCell(split,64n),fail,identity("can.std.stream@1::close_failed"));
...
test("line over cap reports limit_exceeded once and ends the reader",async()=>{
  await withTemp(async root=>{
    ...
    await runOwnedRoot(async()=>{
      ...
      let thrown:unknown;
      try{await reads.readMany(reader,4n);}catch(cause){thrown=cause;}
...
test("cancel during a pending read reports cancelled and delivers nothing",async()=>{
  ...
  const stream=new ReadableStream<Uint8Array>({async pull(c){started=true;await gate;try{c.enqueue(new Uint8Array([9]));}catch{}}});
  const out=await runOwnedRoot(async()=>{
    const token=registerReader(openByteCell(stream,8n),fail,identity("can.std.stream@1::close_failed"));
...
test("close twice, use-after-close and foreign handles throw resource-state",async()=>{
  await runOwnedRoot(async()=>{
    ...
    for(const attempt of [()=>reads.closeReader(token),()=>reads.readMany(token,1n),()=>reads.cancelReader(token,"x")]){
      let thrown:unknown;
      try{await attempt();}catch(cause){thrown=cause;}
      expect(standardOf(thrown)).toBe("resource_state");
    }
    const sink={chunks:[] as Uint8Array[],write(c:Uint8Array){this.chunks.push(c);return c.byteLength;},flush(){},end(){}};
    const writer=registerSink(sink,fail,identity("can.std.stream@1::close_failed"));
    let thrown:unknown;
    try{await reads.readMany(writer,1n);}catch(cause){thrown=cause;}
    expect(standardOf(thrown)).toBe("resource_state");
    try{await reads.readMany({},1n);}catch(cause){thrown=cause;}
    expect(standardOf(thrown)).toBe("resource_state");
    expect(await writes.closeWriter(writer)).toEqual(success(undefined));
    return success(undefined);
  });
...
test("native read failure reports once; close still succeeds; end is sticky",async()=>{
  const failing=new ReadableStream<Uint8Array>({pull(){throw Object.assign(new Error("boom"),{code:"EIO"});}});
  await runOwnedRoot(async()=>{
    const token=registerReader(openByteCell(failing,8n),fail,identity("can.std.stream@1::close_failed"));
    check(await reads.readMany(token,4n),1316,{reason:"EIO"});
    let thrown:unknown;
    try{await reads.readMany(token,4n);}catch(cause){thrown=cause;}
    expect(standardOf(thrown)).toBe("resource_state");
    expect(await reads.closeReader(token)).toEqual(success(undefined));
    return success(undefined);
  });
  const ending=new ReadableStream<Uint8Array>({start(c){c.enqueue(new Uint8Array([7]));c.close();}});
  await runOwnedRoot(async()=>{
    const token=registerReader(openByteCell(ending,8n),fail,identity("can.std.stream@1::close_failed"));
...
test("data then error delivers data first, then fails",async()=>{
  let pulls=0;
  const stream=new ReadableStream<Uint8Array>({pull(c){pulls++;if(pulls===1)c.enqueue(new Uint8Array([3]));else throw Object.assign(new Error("late"),{code:"EIO"});}});
  await runOwnedRoot(async()=>{
    const token=registerReader(openByteCell(stream,8n),fail,identity("can.std.stream@1::close_failed"));
...
test("short writes report partial counts; failing ends report close_failed",async()=>{
  let calls=0;
  const short={write(c:Uint8Array){calls++;return calls===1?1:c.byteLength-1;},flush(){},end(){}};
  await runOwnedRoot(async()=>{
    const writer=registerSink(short,fail,identity("can.std.stream@1::close_failed"));
    expect(await writes.writeSome(writer,ownBytes(new Uint8Array([1,2,3])))).toEqual(success(1n));
    expect(await writes.closeWriter(writer)).toEqual(success(undefined));
    return success(undefined);
  });
  const badEnd={write(c:Uint8Array){return c.byteLength;},flush(){},end(){throw Object.assign(new Error("flush lost"),{code:"EIO"});}};
  await runOwnedRoot(async()=>{
...
test("open failures attribute acquisition; writers truncate at open",async()=>{
  await withTemp(async root=>{
    await runOwnedRoot(async()=>{
      check(await files.readStream(join(root,"missing.bin"),8n),1300,{path:join(root,"missing.bin")});
      ...
      check(await files.writeStream(join(root,"nope","f.bin")),1300,{path:join(root,"nope","f.bin")});
...
test("shared drain overflows before retaining and concatenates within cap",async()=>{
  const big=new ReadableStream<Uint8Array>({start(c){c.enqueue(new Uint8Array([1,2,3,4]));c.close();}});
  await runOwnedRoot(async()=>{
    const over=await drainStream(big,3n,fail,identity("can.std.files@1::io_error"));
    expect(over.overflow).toBe(true);
    return success(undefined);
  });
  const small=new ReadableStream<Uint8Array>({start(c){c.enqueue(new Uint8Array([5]));c.enqueue(new Uint8Array([6,7]));c.close();}});
  const result=await runOwnedRoot(async()=>{
