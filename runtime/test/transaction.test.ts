...
function installFake(hooks?: {
  begin?: (run: (tx: unknown) => Promise<unknown>, tx: FakeTx) => Promise<unknown>;
}): { clients: FakeHandle[] } {
  ...
  (Bun as unknown as { SQL: unknown }).SQL = function () {
    const handle: FakeHandle = {
      ...
      beginCalls: 0,
      begin: async () => {
        throw new Error("begin hook missing");
      },
      connect: async () => {},
      close: async () => {},
    };
    const callable = async function () {
      throw new Error("pool template calls are out of scope here");
    };
...
describe("sql transactions", () => {
  test("commit runs scoped queries and returns the committed value", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        expect(client.txs[0]!.calls[0]!.strings).toEqual([
          "INSERT INTO t (display_name) VALUES (",
          ")",
        ]);
          ...
          async (handle: unknown) => {
            ...
            const row = value(
              await transactions.queryOne(
                ...
                getPlan,
                handle,
                record("app::id_parameters", [["id", 4n]]),
              ),
            );
            ...
            const refused = await transactions.queryOne(
              txGet,
              getPlan,
              handle,
              record("app::id_parameters", [["id", 4n]]),
            ...
            expect(
              domainOutcome(
                await transactions.queryOne(txGet, getPlan, handle, over),
  ...
  test("native callback failures classify by phase", async () => {
    installFake({
      begin: async (run) => {
        ...
        throw postgresError("callback broke", { errno: "40001" });
      },
    });
    try {
      env.set("CAN_TEST_POSTGRES", "postgres://fake/x");
  ...
  test("non-postgres begin failures are transactional", async () => {
    installFake({
      begin: async () => {
        throw postgresError("weird", { errno: "XX000" });
      },
    });
    try {
      env.set("CAN_TEST_POSTGRES", "postgres://fake/x");
  ...
  test("foreign throws propagate instead of classifying", async () => {
    ...
    try {
      ...
      await owned(async () => {
        const token = value(await pools.open("CAN_TEST_POSTGRES", 5n));
        const defect = await transactions.withTransaction(
          token,
          async () => {
            return success(record(COMMIT, [["value", 1n]]));
          },
    ...
    try {
      ...
      await owned(async () => {
        ...
        const outcome = await transactions.withTransaction(
          token,
          async () => {
            const inner = await transactions.withTransaction(
              token,
              async () => {
                return success(record(COMMIT, [["value", 1n]]));
              },
      ...
      await owned(async () => {
        ...
        const outcome = await transactions.withTransaction(
          ...
          async (handle: unknown) => {
            const tx = client.txs[0]!;
            tx.query = async () => [];
            const primary = await transactions.queryOne(
              txGet,
              getPlan,
              handle,
              record("app::id_parameters", [["id", 999n]]),
            );
  ...
  test("rejection after a commit decision is commit-unknown with a safe id", async () => {
    installFake({
      begin: async (run, tx) => {
        tx.query = async () => ({ count: 1 });
        const decided = await run(async function (
    ...
    try {
      ...
      await owned(async () => {
        ...
        const first = await transactions.withTransaction(
          token,
          async (handle: unknown) => {
            const affected = value(
              await transactions.execute(
        ...
        const second = await transactions.withTransaction(
          token,
          async () => {
        ...
        expect(domainOutcome(second, "sql::commit_unknown")).toEqual({
          transaction_id: `${poolId}-2`,
        });
        value(await pools.close(token, 1000n));
      });
    } finally {
  ...
  test("foreign throws after a commit decision are still commit-unknown", async () => {
    installFake({
      begin: async (run) => {
        await run(async () => ({ count: 1 }));
        throw new Error("socket died mid-commit");
      },
    });
    try {
      env.set("CAN_TEST_POSTGRES", "postgres://fake/x");
      await owned(async () => {
        ...
        expect(domainOutcome(outcome, "sql::commit_unknown")).toEqual({
          transaction_id: `${poolId}-1`,
        });
        value(await pools.close(token, 1000n));
      });
    } finally {
  ...
  test("compiler-shape violations throw type errors", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        await expect(
          transactions.withTransaction(token, decide, { commit: COMMIT, rollback: COMMIT }),
        ).rejects.toThrow(TypeError);
        // A malformed decision surfaces as a standard completion through
        // the primary path, never as a second begin.
        const malformed = await transactions.withTransaction(
          token,
          async () => success(record("app::other", [["value", 1n]])),
          leaves,
        );
  ...
  test("fixture boundaries never launch transactions", async () => {
    ...
    try {
      ...
      await owned(async () => {
        ...
        const decide = async () => success(record(COMMIT, [["value", 1n]]));
        await expect(
          transactions.withTransaction(token, decide, leaves, context),
        ).rejects.toThrow();
        await expect(
          transactions.queryOne(
            txGet,
            getPlan,
            token,
            record("app::id_parameters", [["id", 4n]]),
