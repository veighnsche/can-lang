...
test("native fetch makes one attempt, preserves request policy, and has no cookie jar", async () => {
  ...
  const server = Bun.serve({
    ...
    fetch(req) {
      ...
      return new Response("ok", { headers: { "set-cookie": "session=secret" } });
    },
  });
  const connection: Connection = { ...base, endpoint: server.url.href, bearerEnvironment: "TOKEN" };
  let reads = 0;
  try {
    await root(async () => {
      expect(
        value(
          await performRequest(
            connection,
            { ...request, query: [{ name: "q", value: ["one", "two"] }] },
            () => {
              reads++;
              return "test-token";
            },
    ...
    expect(seen[0].url).toEndWith("/?q=one&q=two");
    expect(seen.every((r) => r.cookie === null && r.auth === "Bearer test-token")).toBe(true);
  } finally {
  ...
  try {
    await root(async () => {
      try {
        ...
        expect(transportProblem(cause)).toMatchObject({ kind: "status", status: 302 });
      }
      expect(
        value(
          await performRequest(
...
test("stalled native response times out, aborts body, and never enters decode", async () => {
  ...
  try {
    await root(async () => {
      try {
        await performRequest(
          { ...base, endpoint: server.url.href, timeoutMilliseconds: 30 },
          request,
          () => undefined,
          () => {
            decoded = true;
...
test("owner cancellation has its exact transport phase", async () => {
  ...
  await root(async () => {
    try {
      await performRequest(
        { ...base, endpoint: "http://127.0.0.1:1" },
        { ...request, ownerSignal: controller.signal },
        () => undefined,
        () => {
          entered = true;
          return success(undefined);
      ...
      throw new Error("accepted cancelled request");
    } catch (cause) {
      expect(transportProblem(cause)).toEqual({ kind: "transport", phase: "cancelled" });
    }
    return undefined;
  });
...
test("delivered decompressed bytes are bounded and outgoing overflow prevents auth and launch", async () => {
  ...
  const server = Bun.serve({
    ...
    fetch() {
      attempts++;
      return new Response(Bun.gzipSync(new TextEncoder().encode("x".repeat(1000))), {
  ...
  try {
    await root(async () => {
      ...
      try {
        await performRequest(
          connection,
          { ...request, method: "POST", body: new Uint8Array(51) },
          () => {
            reads++;
            return "secret";
          },
        ...
        throw new Error("accepted outgoing overflow");
      } catch (cause) {
        expect(transportProblem(cause)).toEqual({ kind: "limit", limit: 50 });
      }
      expect(reads).toBe(0);
      ...
      try {
        ...
        throw new Error("accepted decoded overflow");
      } catch (cause) {
        expect(transportProblem(cause)).toEqual({ kind: "limit", limit: 50 });
      }
      return undefined;
...
test("decoded then-named data stays boxed across transport promises", async () => {
  ...
  // oxlint-disable no-thenable -- The decoded value deliberately exposes then to catch Promise assimilation.
