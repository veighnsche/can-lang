...
test("late decoder defect is reported while timeout remains selected", async () => {
  const decodeGate = deferred<never>(),
    timed = deferred<void>();
  ...
  try {
    ...
    await timed.promise;
    decodeGate.reject(new TypeError("late decoder defect"));
...
test("native continuation retains captured resource after parent timeout", async () => {
  ...
  const pending = runOwnedRoot(
    async () => {
            ...
            try {
              await nativeOperation(
                async () => {
                  ...
                  const result = await useResource(resource, "connection", () => {
                    used = true;
                    return success(undefined);
                  });
                  if (result.kind !== "ok") throw result.value;
                  return 1;
                },
                [resource],
                deadline,
      ...
      group.publish([0]);
      await group.promises[0];
      const close = closeResource(resource, "connection");
      await Promise.resolve();
      await Promise.resolve();
      prematurelyClosed = closed;
      proceed.resolve();
      await close;
      return success(undefined);
    },
  ...
  expect(prematurelyClosed).toBe(false);
    ...
    async () => {
      ...
      try {
        await nativeOperation(
          async () => {
            await gate.promise;
            throw transportFault({ kind: "transport", phase: "cancelled" });
          },
          [],
          deadline,
        );
      ...
      try {
        await nativeOperation(
          async () => {
            ...
            throw new TypeError("late synchronous fault");
          },
          [],
          deadline,
        );
      ...
      try {
        await nativeCompletion(
          async () => {
            await gate.promise;
            return failure(
              captureStandard(new Error("boxed late fault"), {
                source: "test:late",
                start: 0,
                end: 0,
                invocation: [],
