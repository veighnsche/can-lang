...
function checkDetail(result: Completion, leaf: string, payload: object) {
  ...
  expect(detail).toMatchObject(payload);
  if (!isDomainFailure(d.cause)) throw new Error("expected private original cause");
  expect(domainFailureDiagnostics(d.cause).provenance.boundary).toBe("native");
}
...
test("named fetch validates strict media types and charset without ambiguous parameters", () => {
  for (const media of [
    ...
    expect(() => responseMedia(media, true)).not.toThrow();
  }
  for (const media of [
    undefined,
    "text/plain",
    ...
    expect(() => responseMedia(media, true)).toThrow();
  }
  ...
  for (const media of [
    ...
    expect(jsonRequestMedia(media)).toBe(false);
  }
});
test("named fetch loopback decodes exact JSON, text BOM, bytes and immutable envelopes", async () => {
  const server = Bun.serve({
    ...
    fetch(req) {
      ...
      if (path === "/json")
        return new Response('{"count":9007199254740993}', {
          headers: { "content-type": "application/problem+json" },
        });
      if (path === "/text")
        return new Response(new Uint8Array([239, 187, 191, 65]), {
  ...
  try {
    const root = await runOwnedRoot(async () => {
      const api = createNamedFetch(domain, ids, () => undefined),
        c = { ...connection, endpoint: server.url.href, maxBodyBytes: 1024 };
      const json = value(
      ...
      expect(json.count).toBe(9007199254740993n);
      ...
      const envelope = value(
        await api.request<any>(
          ...
          request,
          undefined,
          { mode: "bytes", envelope: "response-bytes" },
          origin,
          "test:http/fetch",
        ),
      );
      expect(envelope.status).toBe(418n);
      expect([...copyBytes(envelope.body, origin)]).toEqual([0, 255, 1]);
      expect(Object.isFrozen(envelope)).toBe(true);
      expect(Object.isFrozen(envelope.headers)).toBe(true);
      expect(
        envelope.headers.filter((h: any) => h.name === "set-cookie").map((h: any) => h.value),
      ).toEqual(["a=1", "b=2"]);
      expect(envelope.headers.find((h: any) => h.name === "x-repeat").value).toBe("a, b");
      checkDetail(
        await api.request(c, request, undefined, { mode: "bytes" }, origin, "test:http/fetch"),
        errors[5].identity,
        { status: 418n },
      );
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      received.push({
        ...
        query: new URL(req.url).searchParams.getAll("q"),
      });
  ...
  try {
    const root = await runOwnedRoot(async () => {
      ...
      for (const body of [
        {
          mode: "json" as const,
          value: record("sample", [["count", 9007199254740993n]]),
          schema: responseSchema,
        },
        { mode: "text" as const, value: "😀" },
        { mode: "bytes" as const, value: ownBytes(new Uint8Array([0, 255])) },
      ])
        expect(
          value(
            await api.request<string>(
              c,
              { ...request, method: "POST", query: [{ name: "q", value: ["a b", "+"] }] },
              body,
              { mode: "text" },
              origin,
              "test:http/fetch",
      ...
      expect(new TextDecoder().decode(new Uint8Array(received[0].body))).toBe(
        '{"count":9007199254740993}',
      );
      expect(received[1].body).toEqual([...new TextEncoder().encode("😀")]);
      expect(received[2].body).toEqual([0, 255]);
      expect(
        received.every((r) => r.auth === "Bearer token" && r.query.join("|") === "a b|+"),
      ).toBe(true);
      return success(undefined);
    });
    expect(root.completion.kind).toBe("ok");
...
test("named fetch maps invalid bodies, media and malformed replies to exact errors", async () => {
  ...
  const server = Bun.serve({
    ...
    fetch(req) {
      ...
      const path = new URL(req.url).pathname;
      if (path === "/utf8")
        return new Response(new Uint8Array([255]), { headers: { "content-type": "text/plain" } });
      if (path === "/charset")
        return new Response("x", { headers: { "content-type": "text/plain; charset=latin1" } });
      if (path === "/duplicate")
        return new Response('{"count":1,"count":2}', {
          headers: { "content-type": "application/json" },
        });
      return new Response("", {
        headers: { "content-type": path === "/empty" ? "application/json" : "text/plain" },
      });
    },
  });
  try {
    const root = await runOwnedRoot(async () => {
      ...
      for (const [path, mode, reason] of [
        ...
        ["/wrong", "json", "media_type"],
        ["/duplicate", "json", "duplicate_member"],
      ] as const)
        checkDetail(
          await api.request(
            c,
      ...
      checkDetail(
        await api.request(
          c,
          { ...request, method: "POST", headers: [{ name: "content_type", value: "text/plain" }] },
...
test("named fetch raw fixtures handle bodyless requests and unprovided assertions deny authentication", async () => {
  ...
  expect(root.completion.kind).toBe("standard");
  expect(reads).toBe(0);
  expect(contextReport(context).violations).toEqual(["missing fixture"]);
  ...
  for (const status of [200, 299, 300, 418, 599]) {
    const report = await runAssertion({
      root: { package: "test", declaration: "fetch", name: "raw-" + status },
      actual: async (context) => {
        provideHTTP(context, [
          {
            request: { method: "GET", url: c.endpoint, headers: [], body: new Uint8Array() },
            response: {
              status,
        ...
        const result = await api.request<any>(
          ...
          request,
          undefined,
          { mode: "text", envelope: "text-response" },
          origin,
          "test:http/fetch",
          context,
        );
        if (result.kind !== "ok") return result;
        return success([result.value.status, result.value.body]);
      },
...
test("named fetch body Content-Type defaults and overrides preserve explicit encodings", async () => {
  const api = createNamedFetch(domain, ids, () => undefined);
  const c = { ...connection, maxBodyBytes: 1024 };
  let last: RequestInit | undefined;
  const exchange = async (_url: URL, init: RequestInit) => {
    last = init;
    return new Response(new Uint8Array([255]), {
      headers: { "content-type": "bad; charset=latin1" },
    });
  };
  const root = await runOwnedRoot(async () => {
    for (const body of [
      { mode: "text" as const, value: "é" },
      { mode: "bytes" as const, value: ownBytes(new Uint8Array([255])) },
    ]) {
      const result = await api.request(
        c,
        {
          ...request,
          method: "POST",
          headers: [{ name: "content_type", value: "custom/example; charset=latin1" }],
          exchange,
        },
        body,
        { mode: "bytes" },
      ...
      expect(new Headers(last!.headers).get("content-type")).toBe("custom/example; charset=latin1");
      expect([...(last!.body as Uint8Array)]).toEqual(body.mode === "text" ? [195, 169] : [255]);
    }
    const body = {
      mode: "json" as const,
      value: record("sample", [["count", 1n]]),
      schema: responseSchema,
    };
    expect(
      (
        await api.request(
          { ...c, headers: [{ name: "content_type", value: "bad/type" }] },
          { ...request, method: "POST", headers: [{ name: "content_type", value: [] }], exchange },
          body,
          { mode: "bytes" },
          origin,
          "test:http/fetch",
  ...
  expect(root.completion.kind).toBe("ok");
  for (const bad of ['application/json; x="a\u0001b"', "application/json\u00a0; charset=utf-8"])
    expect(() => responseMedia(bad, true)).toThrow("media_type");
});

test("raw HEAD fixtures reject impossible bodies even after authored recovery", async () => {
  const api = createNamedFetch(domain, ids, () => {
    throw Error("fixture read credentials");
  });
  const c = { ...connection, endpoint: "https://fixture.invalid/", maxBodyBytes: 1024 };
  for (const method of ["HEAD", "head"]) {
    let registered = false;
    const report = await runAssertion({
      root: { package: "conformance", declaration: "head", name: method },
      actual: async (context) => {
        try {
          provideHTTP(context, [
            {
              request: { method, url: c.endpoint, headers: [], body: new Uint8Array() },
              response: {
                status: 200,
                headers: [],
                body: new TextEncoder().encode("impossible-body"),
              },
            },
          ]);
          registered = true;
        } catch {
          /* Recovery must not clear malformed fixture evidence. */
        }
        return success(undefined);
      },
  ...
  const report = await runAssertion({
    root: { package: "conformance", declaration: "head", name: "empty" },
    actual: async (context) => {
      provideHTTP(context, [
        {
          request: { method: "HEAD", url: c.endpoint, headers: [], body: new Uint8Array() },
          response: {
            status: 200,
            headers: [["content-type", "text/plain"]],
            body: new Uint8Array(),
...
test("named fetch normalizes invalid, credential, transport and timeout leaves", async () => {
  ...
  try {
    ...
    const root = await runOwnedRoot(async () => {
      ...
      checkDetail(
        await api.request(c, request, undefined, { mode: "text" }, origin, "test:http/fetch"),
        errors[1].identity,
        { variable: "TOKEN" },
      );
      checkDetail(
        await api.request(
          { ...c, endpoint: "http://127.0.0.1:1/", bearerEnvironment: undefined },
          request,
          undefined,
          { mode: "text" },
          origin,
      ...
      checkDetail(
        await authed.request(
          { ...c, timeoutMilliseconds: 20, bearerEnvironment: undefined },
          request,
          undefined,
          { mode: "text" },
          origin,
