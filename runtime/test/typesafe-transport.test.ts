...
test("Noul sends one exact POST and reads credentials only after all input admission", async () => {
  let reads = 0,
    ...
    contentType = "",
    accept = "";
  const api = createTypeSafe(domain, aiTypes, () => {
    reads++;
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      return Response.json({
        model: "resolved",
        answers: { q0: { type: "noul", noul: 0.5 }, q1: { type: "noul", noul: 0.25 } },
      });
    },
  });
  try {
    const root = await runOwnedRoot(async () => {
      ...
      expect(result.value).toEqual([
        { kind: "noul", probability: 0.5 },
        { kind: "noul", probability: 0.25 },
      ]);
      expect(reads).toBe(1);
      ...
      expect(body).toBe(
        '{"model":"jev-latest","state":{"amount":9007199254740993},"questions":{"q0":{"type":"noul","instructions":"Check amount","criteria":{"true":"Yes","false":"No"}},"q1":{"type":"noul","instructions":"Check amount","criteria":{"true":"Yes","false":"No"}}}}',
      );
...
test("Noul malformed responses stay distinct from status failures without retry", async () => {
  ...
  try {
    const root = await runOwnedRoot(async () => {
      ...
      reply =
        '{"model":"resolved","answers":{"q0":{"type":"noul","noul":0.5},"q1":{"type":"noul","noul":2}}}';
      check(
        await api.ask(
          c,
          "jev-latest",
...
test("assertion context refuses a live Noul boundary before authentication", async () => {
  ...
  const root = await runOwnedRoot(() =>
    api.ask(connection, "jev-latest", schema, state, [question], origin, "test:ai/judge", context),
  );
...
test("raw provider assertions cross request encoding, native response parsing and error mapping", async () => {
  ...
  const c = {
    ...
    bearerEnvironment: undefined,
  };
  const api = createTypeSafe(domain, aiTypes, () => {
    throw Error("raw fixture read ambient credential");
  });
  const row: RawHTTPFixture = {
    request: {
      method: "POST",
      url: c.endpoint,
      headers: [
      ...
      body: encode(
        '{"model":"jev-latest","state":{"amount":9007199254740993},"questions":{"q0":{"type":"noul","instructions":"Check amount","criteria":{"true":"Yes","false":"No"}}}}',
      ),
    },
    response: {
      status: 200,
      headers: [["content-type", "application/json"]],
      body: encode('{"model":"resolved","answers":{"q0":{"type":"noul","noul":0.5}}}'),
    },
  };
  for (const malformed of [false, true]) {
    const report = await runAssertion({
      root: {
        package: "conformance",
        declaration: "noul",
        name: malformed ? "raw-invalid" : "raw-valid",
      },
      actual: async (context) => {
        provideHTTP(context, [
          malformed
            ? {
                ...row,
                response: { ...row.response, body: encode('{"model":"resolved","answers":{}}') },
              }
            : row,
        ]);
        const result = await api.ask(
      ...
      expected: async () =>
        malformed
          ? failure(
              domain.create(
                aiTypes.invalidAnswer,
                record(aiTypes.invalidAnswer, [
                  ["question", ""],
                  ["reason", "question_ids"],
                ]),
                origin,
  ...
  const mismatch = await runAssertion({
    root: { package: "conformance", declaration: "noul", name: "wrong-wire-input" },
    actual: async (context) => {
  ...
  expect(mismatch.violations).toEqual(["argument mismatch"]);
  expect(mismatch.evidence).not.toContain("raw-provider-fixture");
  const leftover = await runAssertion({
    root: { package: "conformance", declaration: "noul", name: "unused-wire-input" },
    actual: async (context) => {
      provideHTTP(context, [row]);
      return success(0);
    },
    expected: async () => success(0),
  });
  expect(leftover.passed).toBe(false);
  expect(leftover.violations).toEqual(["unused fixture"]);
});
...
test("invalid raw HTTP configuration stays a harness failure after handling", async () => {
  ...
  for (const response of [
    ...
    const report = await runAssertion({
      root: { package: "conformance", declaration: "noul", name: "bad-configuration" },
      actual: async (context) => {
        try {
          provideHTTP(context, [
            {
              request: { method: "POST", url: "https://fixture.invalid/", headers: [], body },
              response: response as unknown as RawHTTPFixture["response"],
            },
          ]);
          registered = true;
        } catch {
          /* Authored recovery cannot clear the harness violation. */
        }
        return success(0);
      },
      expected: async () => success(0),
