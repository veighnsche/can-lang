...
import {encodeNoulRequest,decodeNoulAnswers,validateNoulQuestions,QuestionIssue,AnswerIssue,type NoulDescriptor} from "../ai/typesafe.ts";
...
const descriptor:NoulDescriptor={instructions:"Evaluate `amount`",trueDescription:"Yes",falseDescription:"No",minimum:0.5};
...
const decode=(text:string,count=1)=>decodeNoulAnswers(bytes(text),count,8192);
...
test("Noul wire request uses the real exact codec and local policy stays local",()=>{
 const encoded=encodeNoulRequest("jev-latest",stateSchema,state,[descriptor,{...descriptor,minimum:1}],8192);
 ...
 expect(()=>encodeNoulRequest("jev-latest",stateSchema,state,[descriptor],16)).toThrow(CodecIssue);
});
...
test("every Noul descriptor is checked, including unused registrations",()=>{
 for(const [change,reason] of [
  ...
  try{validateNoulQuestions([descriptor,{...descriptor,...change}]);throw Error("accepted descriptor");}catch(e){expect(e).toBeInstanceOf(QuestionIssue);expect((e as QuestionIssue).question).toBe("q1");expect((e as QuestionIssue).reason).toBe(reason);}
 }
 for(const count of [0,257])expect(()=>validateNoulQuestions(new Array(count).fill(descriptor))).toThrow(QuestionIssue);
 validateNoulQuestions(new Array(256).fill(descriptor));
 validateNoulQuestions([{...descriptor,minimum:0},{...descriptor,minimum:1}]);
});
...
test("Noul accepts resolved model names, auxiliary metadata and both probability extremes",()=>{
 ...
 expect(Object.is(p[0],-0)).toBe(true);expect(p[1]).toBe(1);expect(Object.isFrozen(p)).toBe(true);
...
test("provider envelopes share bounded UTF8, syntax and duplicate-member admission",()=>{
 ...
 expect(()=>decodeNoulAnswers(ownBytes(new Uint8Array([255])),1,8192)).toThrow(CodecIssue);
});
