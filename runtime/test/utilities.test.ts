...
test("native clock units, monotonic observations and awaited sleep", async () => {
  const before = BigInt(Date.now()),
    wall = value(await clock.wallMillis());
  expect(wall >= before && wall <= BigInt(Date.now())).toBe(true);
  const start = value(await clock.monotonicMillis());
  expect(Number.isFinite(start)).toBe(true);
  await clock.sleepMillis(10n);
  const end = value(await clock.monotonicMillis());
...
test("secure random bounds, immutable bytes and UUID version", async () => {
  for (const n of [0n, 1n, 65536n]) {
    const bytes = value(await random.secureBytes(n));
    expect(byteLength(bytes)).toBe(n);
    expect(Object.isFrozen(bytes)).toBe(true);
    const first = copyBytes(bytes, origin),
      second = copyBytes(bytes, origin);
    first.fill(0);
...
test("SHA-256 known vectors and independent binary oracle", async () => {
  for (const [input, expected] of [
    ...
    const result = value(await sha256(ownBytes(new TextEncoder().encode(input))));
    expect(Buffer.from(copyBytes(result, origin)).toString("hex")).toBe(expected);
  }
  const binary = new Uint8Array([0, 255, 128, 1]);
  expect(
    Buffer.from(copyBytes(value(await sha256(ownBytes(binary))), origin)).toString("hex"),
  ...
  expect((await invoke(() => sha256(forged), origin)).kind).toBe("standard");
  expect(traps).toBe(0);
});
test("logs contain only native JSON string fields and await write", async () => {
  ...
  let done = false;
  const result = log.writeInfo('secret\n"\\\ud800').then((v) => {
    done = true;
    return v;
  });
  ...
  release();
  expect(await result).toEqual(success(undefined));
  expect(lines).toEqual([JSON.stringify({ level: "info", message: 'secret\n"\\\ud800' }) + "\n"]);
  expect(await log.writeError("error text")).toEqual(success(undefined));
  expect(JSON.parse(lines[1]!)).toEqual({ level: "error", message: "error text" });
});
test("log serialization and expected I/O failures disclose only level", async () => {
  for (const serialization of [true, false]) {
    const log = createLog(domain, errors[2]!.identity, {
      encode: () => {
        if (serialization) throw new RangeError("private serialization");
        return "{}";
      },
      write: async () => {
    ...
    expect(
      await runEntry(
        () => {},
        () => log.writeError("private message"),
        [],
        (line) => {
          reports.push(line);
        },
  ...
  expect((await invoke(() => log.writeInfo("private"), origin)).kind).toBe("standard");
  let traps = 0;
  const forged = new Proxy(
    {},
    {
  ...
  expect((await invoke(() => log.writeInfo(forged as string), origin)).kind).toBe("standard");
  expect(traps).toBe(0);
});
test("host utilities require fixtures while hashing runs in ordinary assertions", async () => {
  ...
  const calls: ((context: AssertionContext) => Promise<Completion>)[] = [
    (c) => clock.wallMillis(c),
    (c) => clock.monotonicMillis(c),
    (c) => clock.sleepMillis(0n, c),
    (c) => random.secureBytes(0n, c),
    (c) => random.uuidV4(c),
    (c) => log.writeInfo("secret", c),
    (c) => log.writeError("secret", c),
  ];
  for (const actual of calls) {
    const report = await runAssertion({
      root: { package: "test", declaration: "host", name: "unsupplied" },
      actual,
  ...
  const input = ownBytes(new Uint8Array());
  const expected = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
  const report = await runAssertion({
    root: { package: "test", declaration: "hash", name: "real" },
    actual: async (c) =>
      success(Buffer.from(copyBytes(value(await sha256(input, c)), origin)).toString("hex")),
    expected: async () => success(expected),
  });
