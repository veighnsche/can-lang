...
const reqApi = createRequests(domain, {
  ...
  header: "can.std.http@1::header",
  close: identity("can.std.stream@1::close_failed"),
  writeFailed: identity("can.std.stream@1::write_failed"),
  multipartForm: typeId("http::multipart_form"),
  multipartField: typeId("http::multipart_field"),
  multipartFile: typeId("http::multipart_file"),
});
...
async function textResponse(status: bigint, body: string) {
  return value(
    await responses.text(
      value(await responses.makeBodyStatus(status)),
...
async function untilDone(flag: { done?: boolean }): Promise<void> {
  for (let n = 0; n < 500 && !flag.done; n++)
    await new Promise((resolve) => setTimeout(resolve, 10));
  expect(flag.done).toBe(true);
}
async function serveGet(
  path: string,
  callback: (request: unknown) => Promise<Completion<unknown>>,
): Promise<{ port: number; stop: () => Promise<Completion<unknown>> }> {
...
const connectionOf = (completed: Completion) => {
  const c = value(completed);
  expect(recordIdentity(c)).toBe(CONNECTION);
  return {
    session: dataProperty(c, "session"),
    events: dataProperty(c, "events"),
    protocol: dataProperty(c, "protocol"),
  };
};
const fieldOf = (item: unknown, name: string) => {
  try {
    return dataProperty(item, name);
  } catch {
    return undefined;
  }
};
const eventOf = (item: unknown) => ({
  kind: recordIdentity(item),
  text: fieldOf(item, "text"),
  data: fieldOf(item, "data"),
  code: fieldOf(item, "code"),
  reason: fieldOf(item, "reason"),
});
  ...
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, "127.0.0.1", () => {
      ...
      if (typeof address === "object" && address !== null) {
        ...
        probe.close(() => reject(Error("no port")));
      }
    });
  });
}
async function echoServer(hooks?: {
  onMessage?: (
    socket: { send: (m: string | Uint8Array) => number },
  ...
  const server = Bun.serve({
    ...
    websocket: {
      message(socket, message) {
        if (hooks?.onMessage) hooks.onMessage(socket, message);
        else socket.send(message);
      },
    },
  });
  if (server.port === undefined) throw Error("websocket server has no port");
  return {
    port: server.port,
    close: async () => {
...
test("client echo carries text, binary, protocol and graceful close both ways", async () => {
  ...
  try {
    await owned(async () => {
      ...
      expect(eventOf(first[0])).toMatchObject({ kind: TEXT, text: "hello" });
      expect(value(await ws.sendBytes(client.session, ownBytes(new Uint8Array([1, 2, 3]))))).toBe(
        3n,
      );
      const second = dataArray(value(await reads.readMany(client.events, 1n)));
      expect(second).toHaveLength(1);
      const binary = eventOf(second[0]);
      ...
      expect(eventOf(last[0])).toMatchObject({ kind: CLOSE, code: 1000n, reason: "client-bye" });
...
test("connect validates URL, protocols and every bound before dialing", async () => {
  await owned(async () => {
    ...
    for (const [args, limit] of [
      [[0n, 64n, 1048576n, 5000n], 0n],
      [[65536n, 0n, 1048576n, 5000n], 0n],
      [[65536n, 64n, 0n, 5000n], 0n],
      [[65536n, 64n, 1048576n, 0n], 0n],
      [[67108865n, 64n, 1048576n, 5000n], 67108865n],
      [[65536n, 1025n, 1048576n, 5000n], 1025n],
      [[65536n, 64n, 67108865n, 5000n], 67108865n],
      [[65536n, 64n, 1048576n, 2147483648n], 2147483648n],
    ] as const)
      check(
        await ws.connect(
          "ws://127.0.0.1:9/",
...
test("connect failures distinguish refused, rejected, tls and timeout", async () => {
  ...
  const cert = join(tmpdir(), "can-ws-test-cert.pem"),
    key = join(tmpdir(), "can-ws-test-key.pem");
  ...
  try {
    ...
    if (typeof address !== "object" || address === null) throw Error("no silent port");
    await owned(async () => {
      check(
        await ws.connect(
  ...
  try {
    await owned(async () => {
      ...
      for (let n = 0; n < 5 && !blocked; n++) {
        ...
        if (attempt.kind === "ok") {
          expect(value(attempt)).toBe(8388608n);
          successes++;
          continue;
        }
      ...
      for (let n = 1; n < batch.length; n++)
        expect(copyBytes(eventOf(batch[n]).data, origin)).toHaveLength(8388608);
      ...
      let thrown: unknown;
      try {
        await ws.sendText(client.session, "late");
      } catch (cause) {
        thrown = cause;
      }
      ...
      thrown = undefined;
      try {
        await ws.close(client.session, 1000n, "again");
      } catch (cause) {
        thrown = cause;
      }
      expect(standardOf(thrown)).toBe("resource_state");
...
test("oversized inbound messages fail the read terminally with cleanup intact", async () => {
  ...
  try {
    await owned(async () => {
      ...
      let thrown: unknown;
      try {
        await reads.readMany(client.events, 1n);
      } catch (cause) {
        thrown = cause;
      }
      expect(standardOf(thrown)).toBe("resource_state");
...
test("an unread queue overrun fails instead of growing without bound", async () => {
  const route = await echoServer({
    onMessage: (socket) => {
      for (let n = 0; n < 6; n++) socket.send("filler-" + n);
    },
  });
  try {
    await owned(async () => {
...
test("cancel interrupts a pending event read and two sessions stay apart", async () => {
  ...
  const server = Bun.serve<{ tag: number }>({
    hostname: "127.0.0.1",
    port: 0,
    fetch(request, s) {
      if (s.upgrade(request, { data: { tag: ++next } })) return;
      return new Response("x", { status: 400 });
    },
    websocket: {
      message(socket, message) {
        const tag = socket.data.tag;
        seen.set(socket, (seen.get(socket) ?? 0) + 1);
        socket.send(tag + ":" + seen.get(socket) + ":" + message);
      },
    },
  });
  try {
    await owned(async () => {
      ...
      let thrown: unknown;
      try {
        await reads.closeReader(first.events);
      } catch (cause) {
        thrown = cause;
      }
      expect(standardOf(thrown)).toBe("resource_state");
...
test("accept upgrades inside the handler with an explicit protocol", async () => {
  ...
  await owned(async () => {
    ...
    try {
      ...
      expect(observed.protocol).toBe("chat");
...
test("a denied handshake stays ordinary HTTP with no socket opened", async () => {
  await owned(async () => {
    const route = await serveGet("/ws", async (request) => {
      ...
      const authed = headers.some(
        (entry) =>
          dataProperty(entry, "name") === "x-auth" && dataProperty(entry, "value") === "yes",
      );
...
test("accept validates selection, claims once and refuses plain HTTP", async () => {
  ...
  await owned(async () => {
    const route = await serveGet("/ws", async (request) => {
      ...
      seen.unsupported =
        bad.kind === "domain" ? domainFailureDiagnostics(bad.value).payload : bad.kind;
      ...
      seen.second =
        again.kind === "domain" ? domainFailureDiagnostics(again.value).payload : again.kind;
      const batch = dataArray(value(await reads.readMany(conn.events, 1n)));
...
test("server sends report accepted bytes and close validates where native does not", async () => {
  ...
  await owned(async () => {
    ...
    try {
      ...
      expect(observed.empty).toBe(0n);
      expect(observed.multi).toBe(9n);
      expect(value(await ws.close(client.session, 1000n, "done"))).toBe(undefined);
      expect(dataArray(value(await reads.readMany(client.events, 2n)))).toHaveLength(1);
      expect(await reads.closeReader(client.events)).toMatchObject({ kind: "ok" });
      await untilDone(observed);
...
test("saturated server sends fail fast and drain resumes them", async () => {
  const observed: { blocked?: boolean; drained?: boolean; sent?: number; done?: boolean } = {
    blocked: false,
    drained: false,
    sent: 0,
  };
  await owned(async () => {
    const route = await serveGet("/ws", async (request) => {
      ...
      let resumed = false;
      for (let n = 0; n < 50 && !resumed; n++)
        resumed = (await ws.sendBytes(conn.session, chunk)).kind === "ok";
    ...
    try {
      ...
      while (!marked) {
        for (const item of dataArray(value(await reads.readMany(client.events, 1n)))) {
          if (eventOf(item).kind === TEXT && eventOf(item).text === "done") marked = true;
          else received++;
        }
      }
...
test("server stop fails active reads and prompts peers with 1001", async () => {
  ...
  await owned(async () => {
    const route = await serveGet("/ws", async (request) => {
      ...
      observed.stopped =
        interrupted.kind === "domain"
          ? domainFailureDiagnostics(interrupted.value).payload
          : interrupted.kind;
      value(await reads.closeReader(conn.events));
      value(await ws.close(conn.session, 1000n, "bye"));
      observed.done = true;
    ...
    try {
      ...
      for (let n = 0; n < 300 && observed.stopped === undefined; n++)
        await new Promise((resolve) => setTimeout(resolve, 10));
...
test("an abrupt peer surfaces as a 1006 close with an empty reason", async () => {
  ...
  await owned(async () => {
    ...
    try {
      ...
      expect(observed.code).toBe(1006n);
