...
export function createText(
  ...
  function invalid(reason: string): Completion<never> {
    return failure(
      domain.create(
        identities.invalidUnicode,
        record(identities.invalidUnicode, [["reason", reason]]),
        origin,
      ),
    );
  }
  function empty(identity: string): Completion<never> {
    return failure(domain.create(identity, record(identity, []), origin));
  }
  return Object.freeze({
    async includes(text: string, value: string, _context?: AssertionContext) {
      return success(text.includes(value));
    },
    async startsWith(text: string, value: string, _context?: AssertionContext) {
      return success(text.startsWith(value));
    },
    async endsWith(text: string, value: string, _context?: AssertionContext) {
      return success(text.endsWith(value));
    },
    async toLowerCase(text: string, _context?: AssertionContext) {
      return success(text.toLowerCase());
    },
    async toUpperCase(text: string, _context?: AssertionContext) {
      return success(text.toUpperCase());
    },
    async trim(text: string, _context?: AssertionContext) {
      return success(text.trim());
    },
    async slice(text: string, start: bigint, end: bigint, _context?: AssertionContext) {
      return success(slice(text, start, end));
    },
    async split(
      text: string,
      separator: string,
    ...
    async join(items: readonly string[], separator: string, _context?: AssertionContext) {
      return success(items.join(separator));
    },
    async scalars(
      text: string,
      _context?: AssertionContext,
    ...
    async compileRegex(
      ...
      if (typeof pattern !== "string" || typeof flags !== "string")
        throw new TypeError("invalid compiler regex input");
      ...
      if (!admitted.test(flags))
        return failure(
          domain.create(
            identities.invalidRegex,
            record(identities.invalidRegex, [["reason", "flags"]]),
            origin,
          ),
        );
      try {
        ...
        return failure(
          domain.create(
            identities.invalidRegex,
            record(identities.invalidRegex, [["reason", "syntax"]]),
            origin,
          ),
        );
      }
    ...
    async findMatches(
      ...
      if (limit < 0n || limit > MAX_MATCHES)
        return failure(
          domain.create(
            identities.invalidLimit,
            record(identities.invalidLimit, [["limit", limit]]),
            origin,
          ),
        );
      ...
      for (const match of text.matchAll(scanned)) {
        ...
        out.push(
          record(identities.match, [
            ["text", match[0]],
            ["start", BigInt(match.index)],
            ["end", BigInt(match.index + match[0].length)],
