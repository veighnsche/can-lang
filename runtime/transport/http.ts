...
export type HTTPTypes = Readonly<{
  invalid: string;
  credential: string;
  transport: string;
  timeout: string;
...
export function createTransport(
  ...
  return Object.freeze({
    async request<T>(
      ...
      return invoke(async () => {
        try {
          return await performRequest(connection, request, readEnv, decode);
        } catch (cause) {
          ...
          switch (problem.kind) {
            case "invalid":
              identity = types.invalid;
              fields = [["reason", problem.reason]];
              break;
            case "credential":
              identity = types.credential;
              fields = [["variable", connection.bearerEnvironment!]];
              break;
            case "transport":
              identity = types.transport;
              fields = [["phase", problem.phase]];
              break;
            case "timeout":
            ...
            case "limit":
              ...
              fields = [["limit", BigInt(problem.limit)]];
              break;
            case "status":
              identity = types.status;
              fields = [
                ["status", BigInt(problem.status)],
                [
                  "headers",
                  array(
                    problem.headers.map((h) =>
                      record(types.header, [
                        ["name", h.name],
                        ["value", h.value],
                      ]),
