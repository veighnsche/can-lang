...
export function mediaType(
  value: string,
): Readonly<{ type: string; parameters: ReadonlyMap<string, string> }> {
  // oxlint-disable-next-line no-control-regex -- MIME text permits only tab, visible ASCII, and extended bytes.
  ...
  while (rest) {
    ...
    if (!part) reject("", "media_type");
    const name = part[1].toLowerCase();
    if (params.has(name)) reject("", "media_type");
    let data = part[2];
    if (data.startsWith('"')) data = data.slice(1, -1).replace(/\\(.)/g, "$1");
    params.set(name, data);
    rest = rest.slice(part[0].length);
    if (part[3] === ";" && !rest.trim()) reject("", "media_type");
  }
  return { type: head[1].toLowerCase(), parameters: params };
}
export function responseMedia(value: string | undefined, json: boolean): void {
  if (value === undefined) {
    if (json) reject("", "media_type");
    return;
  }
  const parsed = mediaType(value);
  if (json && parsed.type !== "application/json" && !parsed.type.split("/")[1].endsWith("+json"))
    reject("", "media_type");
  const charset = parsed.parameters.get("charset");
  if (charset !== undefined && charset.toLowerCase() !== "utf-8") reject("", "charset");
}
