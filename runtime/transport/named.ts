...
export function createNamedFetch(
  domain: ReturnType<typeof createDomainRuntime>,
  types: HTTPTypes & Readonly<{ invalidData: string; failed: string }>,
  ...
  return Object.freeze({
    async request<T>(
      ...
      request: Omit<NativeRequest, "body" | "bodyEncoding" | "envelope">,
      ...
      function invalid(cause: CodecIssue): Completion<never> {
        return failure(
          domain.create(
            types.invalidData,
      ...
      function limit(): Completion<never> {
        return failure(
          domain.create(
            types.limit,
            record(types.limit, [["limit", BigInt(connection.maxBodyBytes)]]),
            origin,
            undefined,
            native,
          ),
        ...
        await invoke(async () => {
          ...
          if (body) {
            if (request.method === "GET" || request.method === "HEAD")
              throw new TypeError("checked fetch method cannot have body");
            try {
              if (body.mode === "json")
                encoded = copyBytes(
                  encodeJSON(body.schema!, body.value, connection.maxBodyBytes),
                  origin,
                );
              else if (body.mode === "bytes") encoded = copyBytes(body.value, origin);
              else {
                if (typeof body.value !== "string")
                  throw new TypeError("checked text body must be string");
                if (body.value.length > connection.maxBodyBytes) return limit();
                if (!body.value.isWellFormed()) reject("", "unicode_scalar");
                encoded = new TextEncoder().encode(body.value);
              }
            } catch (cause) {
              if (cause instanceof CodecIssue)
                return cause.reason === "byte_limit" ? limit() : invalid(cause);
              throw cause;
            }
            if (encoded.byteLength > connection.maxBodyBytes) return limit();
          }
          const exchange =
            request.exchange ?? providerHTTP(context, origin, operation, connection.maxBodyBytes);
            ...
            (bytes, metadata) => {
              ...
              try {
                if (result.mode === "bytes") value = ownBytes(bytes);
                  ...
                  responseMedia(
                    metadata.headers.find((header) => header.name === "content-type")?.value,
                    result.mode === "json",
                  );
                  if (result.mode === "json")
                    value = decodeJSON(result.schema!, ownBytes(bytes), connection.maxBodyBytes);
                  else {
                    try {
                ...
                if (cause instanceof CodecIssue) return invalid(cause);
                throw cause;
              }
              if (result.envelope !== undefined)
                value = record(result.envelope, [
