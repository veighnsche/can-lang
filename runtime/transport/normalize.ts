...
export function createNormalizer(
  ...
  return Object.freeze({
    map<T>(outcome: Completion<T>, operation: string): Completion<T> {
      if (outcome.kind !== "domain") return outcome;
      const original = domainFailureDiagnostics(outcome.value);
      if (original.provenance.boundary !== "native" || original.provenance.operation !== operation)
        return outcome;
      if (!leaves.has(original.typeIdentity)) return outcome;
      return failure(
        domain.create(
          types.failed,
          record(types.failed, [["detail", original.payload]]),
          original.origin,
          outcome.value,
