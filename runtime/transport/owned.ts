...
export async function nativeOperation<T>(
  ...
  const group = launchNative({
    captures,
    run: async () => {
      try {
        return success({ ok: true as const, value: await operation() });
      } catch (cause) {
...
async function selected(group: OwnedGroup, deadline?: Deadline): Promise<Completion> {
  ...
  try {
    completion = deadline ? await deadline.wait(group.promises[0]) : await group.promises[0];
  } catch (cause) {
    group.publish([]);
    throw cause;
  }
...
export async function nativeCompletion<T>(
  operation: () => Completion<T> | Promise<Completion<T>>,
  captures: readonly unknown[] = [],
  deadline?: Deadline,
