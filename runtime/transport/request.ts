...
function invalid(reason: RequestReason): never {
  throw transportFault({ kind: "invalid", reason });
}
...
export function headerName(source: string): string {
  ...
  const name = source.replaceAll("_", "-").toLowerCase();
  if (forbidden.has(name)) invalid("header_name");
  return name;
}
function applyHeaders(headers: Headers, entries: Entries, bearer: boolean): void {
  const seen = new Set<string>();
  for (const entry of entries) {
    const name = headerName(entry.name);
    if (seen.has(name) || (bearer && name === "authorization")) invalid("header_name");
    seen.add(name);
    headers.delete(name);
    for (const value of typeof entry.value === "string" ? [entry.value] : entry.value) {
      ...
      try {
        ...
        if (cause instanceof TypeError) invalid("header_value");
        throw cause;
      }
    }
  }
...
export function endpointURL(endpoint: string): URL {
  if (!scalar(endpoint) || !endpoint) invalid("url");
  let url: URL;
  try {
    url = new URL(endpoint);
  } catch (cause) {
    if (cause instanceof TypeError) invalid("url");
    throw cause;
  }
  if (url.protocol !== "http:" && url.protocol !== "https:") invalid("url");
  if (url.username || url.password) invalid("userinfo");
  if (url.href.includes("#")) invalid("fragment");
  if (url.href.includes("?")) invalid("path_query");
  return url;
}
export function prepareRequest(
...
): Readonly<{ url: URL; headers: Headers }> {
  let endpoint = endpoints.get(connection);
  if (!endpoint) {
    endpoint = endpointURL(connection.endpoint);
    endpoints.set(connection, endpoint);
  }
  if (!scalar(path)) invalid("url");
  let url: URL;
  try {
  ...
  for (const entry of query) {
    ...
    for (const value of typeof entry.value === "string" ? [entry.value] : entry.value) {
      if (!scalar(value)) invalid("query_value");
      search.append(entry.name, value);
    }
  }
  ...
  if (bodyEncoding !== undefined) {
    ...
    if (bodyEncoding === "json" && contentType !== null && !jsonRequestMedia(contentType))
  ...
  if (connection.bearerEnvironment !== undefined) {
    ...
    try {
      ...
      if (cause instanceof TypeError) invalid("credential_value");
      throw cause;
    }
  }
...
export function headerSnapshot(
  ...
  for (const name of [...new Set(headers.keys())].sort()) {
    for (const value of name === "set-cookie" ? headers.getSetCookie() : [headers.get(name)!])
      result.push(Object.freeze({ name, value }));
  }
