...
const origin:FailureOrigin=Object.freeze({source:"can:stream-lifecycle",start:0,end:0,invocation:Object.freeze([])});
...
function cellFor(token:unknown):ReaderCell|WriterCell{
  if((typeof token!=="object"&&typeof token!=="function")||token===null||!cells.has(token))throw resourceStateFailure(undefined,origin);
...
export function registerReader(cell:ReaderCell,fail:Fail,closeFailed:string,options:Readonly<{idempotent?:boolean;scopeManaged?:boolean}>={}):object{
  const token=registerResource(READER_KIND,cell,async():Promise<Completion<void>>=>{
    try{await releaseReader(cell,cell.errored===true);return success(undefined);}
    catch(cause){return fail(closeFailed,[["reason","close"]],cause);}
  },options);
  cells.set(token,cell);
  return token;
}
export function registerWriter(cell:WriterCell,fail:Fail,closeFailed:string,options:Readonly<{idempotent?:boolean;scopeManaged?:boolean}>={}):object{
  const token=registerResource(WRITER_KIND,cell,async():Promise<Completion<void>>=>{
    try{
...
function closeDeadline(fail:Fail,closeFailed:string):{milliseconds:number;failure:()=>Completion<void>}{
  return {milliseconds:STREAM_CLOSE_MS,failure:()=>fail(closeFailed,[["reason","close_timeout"]])};
}
export async function closeHandle(token:unknown,kind:string,fail:Fail,closeFailed:string):Promise<Completion<void>>{
  cellFor(token);
...
export async function cancelHandle(token:unknown,kind:string,reason:string,fail:Fail,closeFailed:string,precancel?:(cell:ReaderCell|WriterCell)=>unknown|Promise<unknown>):Promise<Completion<void>>{
  const cell=cellFor(token);
  if(cell.cancelled===undefined)cell.cancelled=reason;
  if(precancel!==undefined)try{await precancel(cell);}catch{}
  return closeResource(token,kind,closeDeadline(fail,closeFailed));
}
export async function useReader<T>(token:unknown,operation:(cell:ReaderCell)=>Completion<T>|Promise<Completion<T>>):Promise<Completion<T>>{
...
export async function useWriter<T>(token:unknown,operation:(cell:WriterCell)=>Completion<T>|Promise<Completion<T>>):Promise<Completion<T>>{
  return useResource(token,WRITER_KIND,async(native:unknown)=>operation(native as WriterCell));
}
