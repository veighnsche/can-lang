...
async function pullBytes(
  ...
  if (source === undefined) throw new TypeError("invalid byte reader cell");
  const items: unknown[] = [];
  const cap = cell.maxChunk > CHUNK_CLAMP ? Number(CHUNK_CLAMP) : Number(cell.maxChunk);
  for (;;) {
    while (
      cell.carry !== undefined &&
      cell.carry.byteLength > 0 &&
      BigInt(items.length) < maxItems
    ) {
    ...
    try {
      ...
      cell.errored = true;
      cell.carry = undefined;
      return fail(contracts.readFailed, [["reason", reasonFor(cause)]], cause);
    }
    if (next.done) {
      cell.ended = true;
      if (cell.cancelled !== undefined)
        return fail(contracts.cancelled, [["reason", cell.cancelled]]);
      continue;
    }
...
async function pullLines(
  ...
  const source = cell.reader;
  if (source === undefined) throw new TypeError("invalid text reader cell");
  ...
  for (;;) {
    ...
    if (newline >= 0) {
      ...
      if (utf8Length(line) > cell.maxLine) {
        cell.errored = true;
        return fail(contracts.limitExceeded, [["limit", cell.maxLine]]);
      }
    ...
    if (cell.ended === true) {
      if (framing.carry.length > 0 || framing.pendingBytes > 0n) {
        let tail: string;
        try {
          tail = framing.decoder.decode();
        } catch (cause) {
          cell.errored = true;
          return fail(contracts.readFailed, [["reason", "utf8"]], cause);
        }
        const line = framing.carry + tail;
        framing.carry = "";
        framing.pendingBytes = 0n;
        const clean = line.endsWith("\r") ? line.slice(0, -1) : line;
        if (utf8Length(clean) > cell.maxLine) {
          cell.errored = true;
          return fail(contracts.limitExceeded, [["limit", cell.maxLine]]);
        }
        items.push(clean);
    ...
    if (framing.pendingBytes > cell.maxLine) {
      cell.errored = true;
      return fail(contracts.limitExceeded, [["limit", cell.maxLine]]);
    }
    let next;
    ...
    let text: string;
    try {
      text = framing.decoder.decode(next.value, { stream: true });
    } catch (cause) {
      cell.errored = true;
      return fail(contracts.readFailed, [["reason", "utf8"]], cause);
    }
...
async function pullEvents(
  ...
  for (;;) {
    ...
    if (take.kind === "failed") {
      cell.errored = true;
      return fail(contracts.readFailed, [["reason", take.reason]]);
    }
    if (take.kind === "end") {
      cell.ended = true;
      continue;
...
export function createStreamReads(
  ...
  return Object.freeze({
    async readMany(
      ...
      if (!validMaxItems(maxItems))
        return fail(types.limitExceeded, [
          ["limit", typeof maxItems === "bigint" ? maxItems : -1n],
        ]);
    ...
    async cancelReader(
      ...
      return cancelHandle(token, READER_KIND, reason, fail, types.closeFailed, async (cell) => {
        if ("events" in cell && cell.events !== undefined) {
          cell.events.interrupt();
          return;
        }
...
export async function drainStream(
...
): Promise<Readonly<{ overflow: boolean; data: Uint8Array }>> {
  const cell: ReaderCell = {
    reader: stream.getReader(),
    item: "bytes",
    maxChunk: CHUNK_CLAMP,
    maxLine: CHUNK_CLAMP,
  };
  const source = cell.reader!;
  ...
  const token = registerReader(cell, fail, terminalFailed, {
    idempotent: true,
    scopeManaged: true,
  });
