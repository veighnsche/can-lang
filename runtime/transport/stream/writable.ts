...
import {WRITER_KIND,registerWriter,useWriter,closeHandle,cancelHandle,type Fail,type WriterCell,type SinkLike} from "./lifecycle.ts";
const origin:FailureOrigin=Object.freeze({source:"can:stream-write",start:0,end:0,invocation:Object.freeze([])});
type Contracts=Readonly<{writeFailed:string;closeFailed:string}>;
function acceptedOf(result:unknown):bigint|undefined{
  if(typeof result==="number"&&Number.isSafeInteger(result)&&result>=0)return BigInt(result);
  return undefined;
}
export function createStreamWrites(domain:ReturnType<typeof createDomainRuntime>,types:Contracts){
  const fail:Fail=(identity,fields,cause)=>failure(domain.create(identity,record(identity,fields),origin,cause));
  return Object.freeze({
    async writeSome(token:unknown,chunk:unknown,context?:AssertionContext):Promise<Completion<bigint>>{
      ...
      return useWriter(token,async cell=>{
        ...
        if(accepted===undefined||accepted>BigInt(bytes.byteLength))return fail(types.writeFailed,[["reason","bad_accepted"]]);
    ...
    async cancelWriter(token:unknown,reason:string,context?:AssertionContext):Promise<Completion<void>>{
      denyLiveBoundary(context,origin);
