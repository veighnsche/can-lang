...
func TestCurrentBundledBytes(t *testing.T) {
	...
	defer cancel()
	bundle, err := distribution.Build(ctx, sourceRoot, t.TempDir(), archive, "bytes-integration")
	if err != nil {
		t.Fatal(err)
	}
	...
	data, err := os.ReadFile(filepath.Join(sourceRoot, "compiler/testdata/current/bytes/roundtrip.can"))
	if err != nil {
		t.Fatal(err)
	}
