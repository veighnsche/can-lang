...
func TestCurrentBundledCollections(t *testing.T) {
	archive := os.Getenv("CAN_BUN_ARCHIVE")
	if archive == "" {
		t.Skip("set CAN_BUN_ARCHIVE for offline collections execution")
	}
	sourceRoot, _ := filepath.Abs("../..")
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	bundle, err := distribution.Build(ctx, sourceRoot, t.TempDir(), archive, "collections-integration")
	if err != nil {
		t.Fatal(err)
	}
	qualified := exec.CommandContext(ctx, "/usr/bin/sandbox-exec", "-p", "(version 1)(allow default)(deny network*)", filepath.Join(bundle, "runtime/bun"), "--no-env-file", "--no-macros", "--no-install", "--config="+filepath.Join(bundle, "tools/runtime/bunfig.toml"), "test", filepath.Join(bundle, "runtime/test/collections.test.ts"))
	qualified.Dir = t.TempDir()
	qualified.Env = []string{"PATH=/nonexistent", "HOME=" + qualified.Dir, "XDG_CONFIG_HOME=" + qualified.Dir}
	if output, err := qualified.CombinedOutput(); err != nil || !strings.Contains(string(output), "4 pass") || !strings.Contains(string(output), "0 fail") {
		t.Fatalf("offline collections traces: %v\n%s", err, output)
	}
	...
	for _, fixture := range []struct {
		...
		data, err := os.ReadFile(filepath.Join(sourceRoot, "std/map/current/src", fixture.name+".can"))
		if err != nil {
			t.Fatal(err)
		}
		...
		if status != 0 || diag != "" {
			t.Fatalf("collections assertions: %d %s %s", status, out, diag)
		}
		var report map[string]any
		if err = json.Unmarshal([]byte(out), &report); err != nil || report["passed"] != true || len(report["assertions"].([]any)) != fixture.rows {
			t.Fatalf("invalid collections report %v %s", err, out)
		}
		status, out, diag = run("run")
		if status != 0 || out != "" || diag != "" {
			t.Fatalf("collections execution: %d %s %s", status, out, diag)
		}
