...
assert.equal((output.match(/<script /g)||[]).length,2);assert.ok(!output.includes('<img'));assert.ok(!output.includes('<script>alert'));
assert.ok(output.includes('/__can/assets/htmx-4.0.0.min.js'));
assert.ok(output.includes('/__can/assets/htmx-guard.js'));
