// I35 test-only live setup harness. Executed by
...
// I35 tables. Query behavior itself is exercised through the compiled
...
try {
  ...
  try {
    ...
    const statements = seed
      ...
      .map((part) => part.trim())
      .filter((part) => part !== "");
    if (statements.length !== 10) throw new Error(`seed file must hold 10 statements, saw ${statements.length}`);
    const [resetCover, resetAccounts, createAccounts, truncateAccounts, insertAccounts, createCover, truncateCover, insertCover, dropCover, dropAccounts] =
      statements as [string, string, string, string, string, string, string, string, string, string];
    if (
      !resetCover.startsWith("DROP TABLE") || !resetAccounts.startsWith("DROP TABLE") ||
      !createAccounts.startsWith("CREATE TABLE") || !truncateAccounts.startsWith("TRUNCATE") ||
      !insertAccounts.startsWith("INSERT INTO") || !createCover.startsWith("CREATE TABLE") ||
      !truncateCover.startsWith("TRUNCATE") || !insertCover.startsWith("INSERT INTO") ||
      !dropCover.startsWith("DROP TABLE") || !dropAccounts.startsWith("DROP TABLE")
    ) {
      throw new Error("seed file shape changed; refusing to run setup/teardown");
    }
    if (mode === "setup") {
      for (const setup of [resetCover, resetAccounts, createAccounts, truncateAccounts, insertAccounts, createCover, truncateCover, insertCover]) {
      ...
      const accounts = (await sql(staticTemplate("SELECT COUNT(*)::int AS count FROM can_i35_accounts"))) as Array<{ count: number }>;
      const cover = (await sql(staticTemplate("SELECT COUNT(*)::int AS count FROM can_i35_cover"))) as Array<{ count: number }>;
      console.log(JSON.stringify({ serverVersion, accounts: accounts[0]?.count ?? -1, cover: cover[0]?.count ?? -1 }));
    } else {
      await sql(staticTemplate(dropCover));
