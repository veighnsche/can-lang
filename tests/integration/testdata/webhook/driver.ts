// F05 test-only setup/inspection harness for the authenticated
// webhook/companion pair. Executed by tests/integration/webhook_test.go
// against a disposable SQLite file database. Modes: "setup" applies the
// example schema.sql through static templates; "inspect" reports the
// ledger, outbox, attempt, dead-letter, and nonce rows as JSON with
// integers stringified. The compiled program serves every HTTP behavior
// itself; this driver only prepares the file the operator would prepare
// and reads back rows the test asserts on.
import { SQL } from "bun";
...
try {
  ...
  try {
    if (mode === "setup") {
      ...
      const outbox = (await sql(staticTemplate("SELECT delivery_id, event, subscription, state, attempts, updated_ms, worker_id, lease_until_ms, version FROM webhook_outbox ORDER BY delivery_id"))) as Array<Record<string, unknown>>;
      ...
      const dead = (await sql(staticTemplate("SELECT delivery_id, event, subscription, reason, attempts, dead_ms FROM webhook_dead_letter ORDER BY delivery_id"))) as Array<Record<string, unknown>>;
      const nonces = (await sql(staticTemplate("SELECT nonce, seen_ms FROM webhook_carrier_nonce ORDER BY nonce"))) as Array<Record<string, unknown>>;
      ...
      console.log(JSON.stringify({ ledger: freeze(ledger), outbox: freeze(outbox), attempts: freeze(attempts), dead: freeze(dead), nonces: freeze(nonces) }));
    } else {
