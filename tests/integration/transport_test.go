...
func TestBundledTransportLoopback(t *testing.T) {
	...
	clean := t.TempDir()
	command := exec.CommandContext(ctx, "/usr/bin/sandbox-exec", "-p", `(version 1)(allow default)(deny network*)(allow network-bind (local ip "localhost:*"))(allow network-inbound (local ip "localhost:*"))(allow network-outbound (remote ip "localhost:*"))`, filepath.Join(bundle, "runtime/bun"), "--no-env-file", "--no-macros", "--no-install", "--config="+filepath.Join(bundle, "tools/runtime/bunfig.toml"), "test", filepath.Join(bundle, "runtime/test/transport-body.test.ts"), filepath.Join(bundle, "runtime/test/transport-request.test.ts"), filepath.Join(bundle, "runtime/test/transport-owned.test.ts"), filepath.Join(bundle, "runtime/test/transport-fetch.test.ts"), filepath.Join(bundle, "runtime/test/transport-http.test.ts"), filepath.Join(bundle, "runtime/test/transport-late.test.ts"))
	command.Dir = clean
	...
	for _, evidence := range []string{"native fetch makes one attempt", "stalled native response", "response decode past", "transport failures construct exact", "native continuations finish", "late decoder defect", "native continuation retains captured resource"} {
