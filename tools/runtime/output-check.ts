...
if (input.schemaVersion !== 1 || !Array.isArray(input.modules))
  throw new Error("invalid output validation request");
...
for (const module of input.modules) {
  if (
    ...
    throw new Error("invalid output module");
  if (input.sourceIndex) {
    const mapped = input.sourceIndex.modules.find((m: any) => m.path === module.path);
    if (mapped) {
      const lines = module.source.split("\n");
      if (
        mapped.segments.some(
          (p: any) => p.line > lines.length || p.column > lines[p.line - 1].length,
        )
      )
        throw new Error("mapping outside generated module");
    }
  }
  try {
    ...
    for (const edge of transpiler.scanImports(module.source)) {
      if (edge.kind !== "import-statement" || !module.imports.includes(edge.path))
        throw new Error("undeclared executable module edge");
    }
  } catch {
