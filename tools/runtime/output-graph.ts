...
export function validateStaticGraph(javascript: string, imports: readonly string[]): void {
  ...
  while (pending.length) {
    ...
    if (node.type === "ImportExpression")
      throw new Error("dynamic imports are not compiler output");
    if (
      ...
      if (source && (typeof source.value !== "string" || !imports.includes(source.value)))
        throw new Error("undeclared static module edge");
    }
