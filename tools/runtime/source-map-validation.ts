...
export function validateIndex(index: any): void {
  ...
  for (const s of index.sources) {
    if (
      ...
      !s.path.endsWith(".can") ||
      !s.spans ||
      typeof s.spans !== "object" ||
      Array.isArray(s.spans)
    )
      throw new Error("invalid source identity");
    for (const [name, span] of Object.entries(s.spans) as [string, any][]) {
      if (
        !/^[a-z_]+:[0-9]+:[0-9]+$/.test(name) ||
        name !== `${span.operation}:${span.start}:${span.end}` ||
        !integer(span.start) ||
        !integer(span.end, span.start) ||
        !integer(span.line, 1) ||
        !integer(span.column) ||
        !integer(span.endLine, span.line) ||
        !integer(span.endColumn) ||
  ...
  for (const m of index.modules) {
    if (
      ...
      throw new Error("invalid mapped module");
    paths.add(m.path);
    let line = 0,
      column = -1;
    for (const p of m.segments) {
      if (
        ...
        !Object.hasOwn(ids.get(p.source)?.spans ?? {}, p.name)
      )
        throw new Error("invalid mapping segment");
      line = p.line;
      column = p.column;
    }
  }
...
export function validateMaps(index: any, maps: Record<string, any>): void {
  validateIndex(index);
  if (Object.keys(maps).length !== index.modules.length)
    throw new Error("source map inventory mismatch");
    ...
    if (
      Object.keys(map)
        ...
        Object.keys(canonical)
          .filter((k) => canonical[k] !== undefined)
          .sort()
          .join() ||
      Object.keys(canonical).some((k) => JSON.stringify(map[k]) !== JSON.stringify(canonical[k]))
    )
      throw new Error("noncanonical source map");
    const decoded = decodedMappings(new TraceMap(map));
    const actual: any[] = [];
    for (let l = 0; l < decoded.length; l++)
      for (const p of decoded[l]) {
        if (p.length !== 5) throw new Error("incomplete source segment");
        const source = map.sources[p[1]],
          name = map.names[p[4]],
          span = sources.get(source)?.spans[name];
        if (!span || p[2] !== span.line - 1 || p[3] !== span.column)
          throw new Error("map/source-index disagreement");
        actual.push({ line: l + 1, column: p[0], source, name });
      }
    if (JSON.stringify(actual) !== JSON.stringify(module.segments))
      throw new Error("map segments differ from compiler input");
  }
}
