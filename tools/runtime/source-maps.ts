...
const raw=await Bun.stdin.text(), index=JSON.parse(raw);
validateIndex(index);
...
for (const module of index.modules) {
  ...
  for (const p of module.segments) {
    ...
    addMapping(map,{generated:{line:p.line,column:p.column},source:p.source,original:{line:span.line,column:span.column},name:p.name});
  }
  maps[module.path]=toEncodedMap(map);
}
validateMaps(index,maps);
console.log(JSON.stringify({schemaVersion:1,kind:"can.source-maps",requestSHA256:createHash("sha256").update(raw).digest("hex"),maps}));
